...
class MigrationQwenTranslationAdapter:
    ...
    def __init__(self, timeout_seconds: float = 180.0, chunk_size: int = 48) -> None:
    ...
    def translate(self, batch: TranslationBatch) -> TranslationBundle:
        ...
        for start in range(0, len(batch.units), self._chunk_size):
            ...
            translated.update(self._translate_chunk(batch, chunk))
        units = tuple(
    ...
    def _translate_chunk(
                    ...
                    "content": json.dumps(
                        {
                            ...
                            "units": [
                                {"unit_id": unit.unit_id, "source_text": unit.source_text}
                                for unit in units
                            ],
                        },
                        ensure_ascii=False,
        ...
        return self._normalize(parsed, unit_ids)

    @staticmethod
    def _normalize(value: dict[str, Any] | None, unit_ids: tuple[str, ...]) -> dict[str, str]:
        """拒绝缺失、重复、新增、空译文和身份漂移。"""
