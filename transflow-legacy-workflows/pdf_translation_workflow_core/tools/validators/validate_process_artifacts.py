...
def product_quality_passed(decisions: list[dict[str, Any]]) -> bool:
    d9 = decision_by_id(decisions, "D9_final_acceptance")
    if not d9 or not isinstance(d9.get("model_output"), dict):
        return False
    if d9.get("next_state") == "S_DONE_PRODUCT_ACCEPTED":
        return True
    output = d9["model_output"]
    return output.get("product_quality_verdict") == "PASS" or output.get("terminal_state") == "S_DONE_PRODUCT_ACCEPTED"
...
def validate_product_quality_visual_closure(
    ...
    if "Lx_RepairLoop" in states_seen and d8 is None:
        errors.append("Lx_RepairLoop was entered but D8_minimal_repair_selection is missing")
...
def validate(run_dir: Path) -> dict[str, Any]:
    ...
    if not (states_seen & terminal_states) and not product_quality_passed(decisions):
    ...
    if "Lx_RepairLoop" in states_seen:
        missing_decisions |= {"D8_minimal_repair_selection"} - decision_ids
