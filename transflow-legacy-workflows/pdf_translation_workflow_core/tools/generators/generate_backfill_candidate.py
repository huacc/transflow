...
def sample_fill_detail(page: fitz.Page, rect: fitz.Rect) -> dict[str, Any]:
    ...
    inner_samples = [sample for sample in samples if sample[3] == "inner"]
    inner_clusters: dict[tuple[int, int, int], list[tuple[int, int, int, str]]] = {}
    for rgb in inner_samples:
        key = tuple(int(round(channel / 8) * 8) for channel in rgb[:3])
        inner_clusters.setdefault(key, []).append(rgb)

    def cluster_saturation(key: tuple[int, int, int]) -> int:
        return max(key) - min(key)

    def cluster_is_light_page_background(key: tuple[int, int, int]) -> bool:
        return min(key) >= 232 and cluster_saturation(key) <= 16

    inner_candidate: tuple[tuple[int, int, int], list[tuple[int, int, int, str]]] | None = None
    if inner_clusters:
        inner_candidate = max(inner_clusters.items(), key=lambda item: (len(item[1]), -sum(item[0])))
        inner_key, inner_cluster = inner_candidate
        inner_ratio = len(inner_cluster) / max(1, len(inner_samples))
        if inner_ratio >= 0.38 and not cluster_is_light_page_background(inner_key):
            representative_samples = inner_cluster

            def inner_median_channel(channel_index: int) -> int:
            ...
            r = inner_median_channel(0)
            g = inner_median_channel(1)
            b = inner_median_channel(2)
            return {
                "fill_color": (r / 255, g / 255, b / 255),
                "method": "inner_bbox_pixel_cluster",
                "sample_count": len(samples),
                "selected_cluster_count": len(inner_cluster),
                "selected_cluster_key": list(inner_key),
                "selected_cluster_inner_count": len(inner_cluster),
                "selected_cluster_near_ring_count": 0,
                "selected_cluster_outer_ring_count": 0,
                "selected_cluster_representative_source": "inner",
                "inner_cluster_ratio": round(inner_ratio, 3),
                "quantization_step": 8,
            }
