...
CASES = [
    {
        "case_id": "R28_AIA_ZH_TO_EN_pages_001_020",
    ...
    {
        "case_id": "R28_AIA_EN_TO_ZH_pages_001_020",
...
def run_case(root: Path, case: dict[str, str]) -> dict[str, Any]:
    ...
    verdict = load_json_if_exists(case_dir / "reports" / "round28_final_verdict.json")
    ...
    (case_dir / "round28_case_run_record.json").write_text(json.dumps(record, ensure_ascii=False, indent=2), encoding="utf-8")
    return record
...
def main() -> None:
    ...
    reports.mkdir(parents=True, exist_ok=True)
    (reports / "stage_a_seed_evidence.json").write_text(json.dumps(stage_a_seed, ensure_ascii=False, indent=2), encoding="utf-8")
    (reports / "round28_batch_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
    ...
    (reports / "round28_batch_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    (reports / "round28_final_verdict.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
