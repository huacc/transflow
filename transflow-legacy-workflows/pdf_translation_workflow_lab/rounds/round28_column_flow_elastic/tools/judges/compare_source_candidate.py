...
def summarize(signals: list[dict[str, Any]], overlaps: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    causal_priority = ["text_fit_overflow", "font_size_regression", "cross_slot_overlap"]
    selected_failure = next((failure for failure in causal_priority if failure_counts.get(failure)), None)
    if selected_failure is None and failure_counts:
        selected_failure = failure_counts.most_common(1)[0][0]
    ...
    return {
        ...
        "triage_result": {
            ...
            "triage_rule": "causal_priority_then_current_run_blocking_failure_count",
        ...
        "tool_selection_reason": (
            f"先由 Triage 按因果优先级选出 {selected_failure}，再由静态 Dispatch 表映射到 {selected_repair}。"
