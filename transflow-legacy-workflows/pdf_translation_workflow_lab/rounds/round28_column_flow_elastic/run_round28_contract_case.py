...
def boundary_report(root: Path, planned_paths: list[Path]) -> dict[str, Any]:
    ...
    for path in planned_paths:
        ...
        if not inside:
            verdict = "FAIL"
    return {
        "tool": "round28_workspace_boundary_preflight",
...
def main() -> None:
    ...
    page_profiles = reports / "page_profiles.json"
    raw_layout_plan = reports / "layout_plan.raw.json"
    layout_plan = reports / "layout_plan.json"
    column_flow_evidence = reports / "column_flow_elastic_evidence.json"
    ...
    run_request = {
        ...
        "run_mode": "round28_column_flow_elastic",
    ...
    run_cmd(root, [sys.executable, "tools/planners/classify_pages.py", "--role-plan", rel(root, role_plan), "--source-language", args.source_language, "--target-language", args.target_language, "--output", rel(root, page_profiles)], "S6_LayoutPlan:S6D_ClassifyPages", operation_log, decision_log)
    run_cmd(root, [sys.executable, "tools/planners/plan_layout.py", "--source-pdf", args.source_pdf, "--role-plan", rel(root, role_plan), "--output", rel(root, raw_layout_plan)], "S6_LayoutPlan:S6E_BuildRawLayoutPlan", operation_log, decision_log)
    run_cmd(root, [sys.executable, "tools/planners/apply_column_flow_elastic.py", "--layout-plan", rel(root, raw_layout_plan), "--page-profiles", rel(root, page_profiles), "--output", rel(root, layout_plan), "--evidence", rel(root, column_flow_evidence)], "S6_LayoutPlan:S6F_ColumnFlowElastic", operation_log, decision_log)
    prompt_log(model_log, "S6_LayoutPlan", "S6_page_type_and_column_flow.prompt.json", ["role_plan", "page_profiles", "raw_layout_plan"], "page class, fixed column widths, vertical elastic flow", "Local deterministic page classifier and column-flow postprocessor executed this template contract without backend model call.")
    log_state(trace, state="S6_LayoutPlan", purpose="生成角色计划、页面类别、raw layout，并按列宽不变/纵向弹性策略生成 generator-consumable 布局计划", input_artifacts=["reports/source_structure.json", translations_arg, "reports/page_strategy.json"], output_artifacts=["reports/role_plan.json", "reports/page_profiles.json", "reports/layout_plan.raw.json", "reports/column_flow_elastic_evidence.json", "reports/layout_plan.json"], decision="layout_plan_ready_with_column_flow_elastic", next_state="S7_GenerateCandidate")
    ...
    run_cmd(
        ...
        [
            sys.executable,
            "tools/validators/materialize_round28_artifacts.py",
            "--round-root",
            ".",
            "--reports-dir",
            "reports",
    ...
    log_state(
        ...
        purpose="按 round28 执行契约物化七产物、多轮记忆台账、页面分类证据、trace 卡，并校验分发图一致性",
        ...
        decision="round28_decision_artifacts_materialized",
        next_state="S9_VerifyProcessContract",
    )
    write_json(reports / "state_trace.json", trace)
    ...
    (reports / "round28_column_flow_elastic_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
