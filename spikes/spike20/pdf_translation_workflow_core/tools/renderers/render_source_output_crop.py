...
def render_contact_sheet(
    ...
    source_label: str,
    output_label: str,
    backrotate_output_degrees: float | None = None,
    backrotate_output_path: Path | None = None,
) -> dict[str, Any]:
    ...
    sheet.save(out_png)
    backrotated_ref = None
    if backrotate_output_degrees is not None and backrotate_output_path is not None:
        ensure_dir(backrotate_output_path.parent)
        output_crop = crops[1][1]
        output_crop.rotate(backrotate_output_degrees, expand=True, resample=Image.Resampling.BICUBIC).save(backrotate_output_path)
        backrotated_ref = rel(backrotate_output_path)
    result = {
        ...
        "backrotate_output_degrees": backrotate_output_degrees,
        "backrotated_output": backrotated_ref,
    }
...
def main() -> int:
    ...
    parser.add_argument("--backrotate-output-degrees", type=float, default=None)
    parser.add_argument("--backrotate-output-out", default=None)
    ...
    result = render_contact_sheet(
        ...
        args.source_label,
        args.output_label,
        args.backrotate_output_degrees,
        Path(args.backrotate_output_out) if args.backrotate_output_out else None,
    )
