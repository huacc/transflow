...
class Runner:
    ...
    def prepare_inputs(self) -> None:
        ...
        for semantic_file in semantic_files:
            ...
            shutil.copy2(semantic_file, target)
    ...
    def select_translation_for_case(self, case: dict[str, Any], extraction: Path) -> Path:
        ...
        for index, candidate in enumerate(candidates, start=1):
            out = match_dir / f"candidate_{index:03d}_{candidate.stem}.validation.json"
            result = self.run_cmd(
                "S5_TranslationPlan",
    ...
    def run_case(self, case: dict[str, Any]) -> dict[str, Any]:
        ...
        if quality_data.get("product_quality_verdict") == "FAIL" and int(repair_data.get("blocking_repair_count") or 0) > 0:
            loop_records.append(self.write_repair_loop_record(case_id, case_dir, quality, repair_plan, repair_data))
        case.update(
            {
                "report_dir": case_dir,
    ...
    def write_repair_loop_record(
        ...
        deferred = plans[1:]
        record_path = case_dir / "repair_loop_0001.json"
        record = {
            "loop_id": f"{case_id}_L001",
            "loop_iteration": 1,
    ...
    def run(self) -> dict[str, Any]:
        ...
        if failed_cases:
            self.decision(
                ...
                "Lx_RepairLoop",
                "Record selected repair atoms and unrepairable execution boundary.",
                [case["visual_repair_plan"] for case in failed_cases],
                "D8_repair_selection.prompt.json",
                ["repair_loop_record_path", "execution_status", "unrepairable_reason", "deferred_failures"],
                {
                    "verdict": "fail",
                    "backend_model_call_made": False,
                    "repair_loop_records": [item for case in failed_cases for item in case.get("repair_loop_records", [])],
                    "unrepairable_reason": "No generic repair atom executor is wired for the remaining failures in this round.",
                },
                "S_FAIL_QUALITY",
            )
            self.transition(
