...
def plan_end_layout(
    ...
    *,
    ...
    for region in regions:
        use_bold = _is_bold(region.font_name) and region.role not in {"contact", "contact_block", "disclaimer"}
            ...
            findings.append(
                EndFinding(
                    ...
                    evidence={
                        ...
                        "source_font_size": region.font_size,
                        "minimum_font_size": round(_minimum_font_size(region.font_size, region.role), 4),
                    },
                )
            )
        placements.append(
    ...
    return EndLayoutPlan(template.page_id, TOOLBOX_KEY, template.structure_sha256, tuple(placements)), tuple(findings)
