...
def plan_cover_layout(
    ...
    *,
    ...
    eligible = [container.container_id for container in template.containers if container.translatable]
    ...
    for container in template.containers:
        if container.translatable and container.container_id not in translated:
            continue
        text = translated[container.container_id] if container.translatable else container.source_text
    ...
    return CoverLayoutPlan(template.page_id, TOOLBOX_KEY, template.structure_sha256, tuple(placements)), tuple(findings)
...
def _fit_text(
    ...
    minimum = max(8.0 if role == "title" else 5.0, source_font_size * (0.68 if role == "title" else 0.60))
    sizes: list[float] = []
    ...
    for size in sizes:
        ...
        if _probe(page_width, page_height, output, text, size, font_file, font_resource, alignment) >= 0:
...
def _minimum_fitting_height(
    ...
    spare_height = _probe(page_width, page_height, allowed, text, font_size, font_file, font_resource, alignment)
    if spare_height < 0:
        return None
...
def _is_bold(font_name: str) -> bool:
    ...
    return any(token in lowered for token in ("bold", "black", "heavy", "semibold", "xbold")) or lowered.endswith(
        ("-bd", "-demi")
    )
...
def _native_bilingual_companions(
    ...
    source_best = {
        ...
        if (
            candidates := [candidate for candidate in preserved if _is_structural_bilingual_pair(source, candidate)]
        )
    }
    target_best = {
        ...
        if (
            candidates := [candidate for candidate in requested if _is_structural_bilingual_pair(candidate, target)]
        )
    }
    return {
...
def _is_companion_group(container, group: list, *, maximum_gap_factor: float = 1.5) -> bool:
    ...
    return vertical_gap <= max(container.font_size, *(item.font_size for item in group)) * maximum_gap_factor
...
def _horizontally_related(container, group: list) -> bool:
    ...
    narrower_width = min(container.source_bbox[2] - container.source_bbox[0], group_bbox[2] - group_bbox[0])
