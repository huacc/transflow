...
from .engine import run_p17_page
...
class RecordedTranslationProvider:
    ...
    model_name = "recorded-p17-bundle"

    def __init__(self, bundle_path: Path) -> None:
        self.bundle_path = bundle_path
        audit_path = bundle_path.parent.parent / "reports" / "translation_provider_audit.json"
        self.last_audit = (
            json.loads(audit_path.read_text(encoding="utf-8"))
            if audit_path.is_file()
            else {"status": "NOT_AVAILABLE", "confirmed_proper_name_ids": []}
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run P17 body.composite.flow_text_chart page packages")
    ...
    parser.add_argument("--latin-font-file", default="C:/Windows/Fonts/DejaVuSans.ttf")
    parser.add_argument("--latin-bold-font-file", default="C:/Windows/Fonts/DejaVuSans-Bold.ttf")
    ...
    write_json(
        ...
        {
            "toolbox_key": "body.composite.flow_text_chart",
            "run_id": args.run_id,
            "provider": args.provider,
            "sample_ids": [record["sample_id"] for record in selected],
            "holdout_accessed": holdout_accessed,
            "holdout_integrity": (
                "NON_BLIND_SOURCE_POOL_PREVIEWED_BEFORE_PARTITION" if holdout_accessed else None
            ),
            "holdout_purpose": "non_blind_final_validation" if holdout_accessed else None,
        },
    )
    ...
    if args.provider == "qwen":
        try:
            ...
            qwen_config = replace(
                qwen_config,
                chunk_max_units=max(64, qwen_config.chunk_max_units),
            )
    ...
    for record in selected:
        ...
        use_latin_font = target_language.casefold().startswith("en")
        result = run_p17_page(
            source_pdf=source,
            page_id=sample_id,
            run_dir=run_root / "cases" / sample_id,
            provider=provider,
            font_file=args.latin_font_file if use_latin_font else args.font_file,
            bold_font_file=args.latin_bold_font_file if use_latin_font else args.bold_font_file,
    ...
    write_json(
        run_root / "reports" / "batch_result.json",
        {
            ...
            "promotion_blockers": [
                "P4_PROMOTION_MANIFEST_ABSENT",
                "P5_GATE_NOT_EVALUATED",
                "P13_PROMOTION_MANIFEST_ABSENT",
                "P17_HOLDOUT_NON_BLIND",
            ],
        },
    )
