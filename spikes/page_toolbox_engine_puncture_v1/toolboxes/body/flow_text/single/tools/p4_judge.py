...
def judge_p4_candidate(
    *,
    ...
    for placement in plan.placements:
        ...
        if abs(placement.output_bbox[0] - source.source_bbox[0]) > 0.01:
        ...
        if placement.output_bbox[3] > plan.content_bottom + 0.01 and source.role != "margin":
            findings.append(ToolboxFinding("P4_VERTICAL_PAGE_ESCAPE", "HARD", "p4_quality_judge", placement.container_id, "正文越过可用纵向底边"))
        if placement.font_size < 6.0 or placement.font_size + 0.01 < placement.source_font_size * 0.72:
