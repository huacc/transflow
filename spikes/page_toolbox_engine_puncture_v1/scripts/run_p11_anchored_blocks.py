...
TOOLBOX_ROOT = ROOT / "toolboxes" / "body" / "anchored_blocks"
...
from toolboxes.body.anchored_blocks.tools.engine import run_p11_page
from toolboxes.body.anchored_blocks.tools.translation_guard import RequiredLiteralRetryProvider


class RecordedTranslationProvider:
    provider_name = "recorded"
    model_name = "p11-recorded-bundle"
    ...
    def translate(self, request):
        ...
        audit_path = self.bundle_path.parent.parent / "reports" / "translation_retry.json"
        self.last_audit = json.loads(audit_path.read_text(encoding="utf-8")) if audit_path.is_file() else {}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run P11 body.anchored_blocks page packages")
    ...
    selection.add_argument(
        "--regression",
        action="store_true",
        help="select development and regression samples without opening holdout",
    )
    ...
    if args.sample_id:
        ...
        selected = [record for record in records if record.get("validation_phase") in {"initial", "initial_expansion"}]
    elif args.regression:
        selected = [record for record in records if record["split"] != "holdout"]
    else:
        selected = records
    ...
    for record in selected:
        ...
        if args.provider == "qwen":
            ...
            provider = RequiredLiteralRetryProvider(
                ...
                QwenPageTranslationProvider(
                    config,
                    (TOOLBOX_ROOT / "prompts" / "required_literal_retry.zh-CN.md").read_text(encoding="utf-8"),
                ),
            )
        else:
    ...
    summary = {
        "schema_version": "p11-anchored-blocks-batch/v1",
        "toolbox_key": "body.anchored_blocks",
        "run_id": args.run_id,
        "provider": args.provider,
