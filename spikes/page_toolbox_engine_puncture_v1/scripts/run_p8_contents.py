...
TOOLBOX_ROOT = ROOT / "toolboxes" / "contents"
...
from toolboxes.contents.tools.engine import run_p8_page


class RecordedTranslationProvider:
    provider_name = "recorded"
    model_name = "p8-recorded-bundle"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run P8 contents page packages")
    ...
    run_id = args.run_id or datetime.now(timezone.utc).strftime("p8-%Y%m%dT%H%M%SZ")
    ...
    summary = {
        "schema_version": "p8-contents-batch/v1",
        "toolbox_key": "contents",
        "run_id": run_id,
        "provider": args.provider,
