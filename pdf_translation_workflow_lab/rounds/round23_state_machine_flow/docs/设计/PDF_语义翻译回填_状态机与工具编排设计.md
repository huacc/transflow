...
| `failure_class` | `visual_repair_plan.json`、`visual_adjudication.json`、QualitySignal |
...
### 8.2 failure class 到目标状态

| failure class 类型 | 优先目标状态 | 说明 |
|---|---|---|
| 语义缺失、placeholder、伪译文、compact variant 缺失 | `S5_TranslationPlan` | 只有语义证据失败才回翻译 |
| 源证据、unit_id、font/linkage 缺失 | `S3_SourceExtract` 或 `S7_GenerateCandidate` | 重新提取或修 generation evidence |
| 布局分类、字体层级、body flow、表格/矩阵角色错误 | `S6_LayoutPlan` | 重新规划 policy/role/layout |
| 背景擦除、图片保护、redaction fill、probe residue | `S7_GenerateCandidate` | 重新生成候选 |
| 无法映射 repair atom | `Ax_AdaptiveChange` 或 `S_FAIL_QUALITY` | 看是否是方法论缺口 |
...
```json
{
  ...
  "repair_atom": "body_flow_region_reflow",
