...
def boundary_report(root: Path, planned_paths: list[Path]) -> dict[str, Any]:
    ...
    for path in planned_paths:
        ...
        if not inside:
            verdict = "FAIL"
    return {
        "tool": "round24_workspace_boundary_preflight",
...
def validate_presupplied_translations(path: Path, output: Path) -> dict[str, Any]:
    ...
    result = {
        "tool": "round24_presupplied_translation_validation",
        ...
        "note": "Round24 validates pre-supplied semantic translations and records that no backend translation model is invoked.",
    }
    write_json(output, result)
    return result
...
def main() -> None:
    ...
    parser.add_argument("--case-id", default="R24_GEN_ZH_TO_EN_00005_pages_001_020")
    ...
    run_request = {
        ...
        "run_mode": "state_machine_layered_judge_repair_patch",
    ...
    boundary = boundary_report(root, [reports, output, previews, source_pdf, translations_json])
    ...
    prompt_log(model_log, "S5_TranslationPlan", "pre_supplied_translation_validation", [args.translations_json], "semantic_translation_validation", "No backend translation model invoked in round24.")
    ...
    if translation_report["translation_validation_verdict"] != "PASS":
        write_json(reports / "state_trace.json", trace)
        raise SystemExit(3)
    log_state(trace, state="S5_TranslationPlan", purpose="校验预供应语义译文是否满足产品质量输入前提", input_artifacts=[args.translations_json], output_artifacts=["reports/semantic_translation_validation.json"], decision="semantic_translation_pass", next_state="S6_LayoutPlan")

    run_cmd(root, [sys.executable, "tools/planners/plan_roles.py", "--source-structure", rel(root, source_structure), "--translations-json", args.translations_json, "--output", rel(root, role_plan)], "S6_LayoutPlan:S6D_BuildRolePlan", operation_log, decision_log)
    ...
    log_state(trace, state="S6_LayoutPlan", purpose="生成角色计划和 generator-consumable 布局计划", input_artifacts=["reports/source_structure.json", args.translations_json, "reports/page_strategy.json"], output_artifacts=["reports/role_plan.json", "reports/layout_plan.json"], decision="layout_plan_ready", next_state="S7_GenerateCandidate")
    ...
    if repair_after_count == 0:
        loop_verdict = "PASS"
    elif repair_after_count < before_count:
    ...
    repair_loop = {
        ...
        "acceptance_rule": "accept only if repaired blocking failure count is zero or lower than the pre-repair count",
    ...
    report = [
        ...
        (
            ...
            if not repair_accepted and before_adj.get("selected_repair_family") == "vertical_flow_relayout"
    ...
    (reports / "round24_state_machine_repair_patch_report.md").write_text("\n".join(report) + "\n", encoding="utf-8")
