...
from transflow.domain.classification import NodeJudgement

LOGGER = logging.getLogger("transflow.classification.rules")
...
def decide_page_role(evidence: dict[str, Any]) -> NodeJudgement:
    ...
    identity_words = bool(
        re.search(r"ANNUAL REPORT|INTERIM REPORT|年報|年报|年度報告|年度报告", upper)
    )
    ...
    if chars == 0:
        return _inconclusive(
            "page.role",
            "原生文字为空，规则无法区分图片封面、结束页、正文或纯视觉页",
            ("IMG1", "TEXT1", "PAGE1"),
        )
    if position["is_last"] and chars <= 250:
        return _inconclusive(
            "page.role",
            "末页且文字较少，但规则无法证明其承担结束功能",
            ("IMG1", "TEXT1", "PAGE1"),
        )
    return _inconclusive(
        "page.role",
    ...
    for block in sorted(candidates, key=lambda item: float(item["bbox"][0])):
        ...
        cluster = next(
            (
                ...
                if abs(x0 - median(float(item["bbox"][0]) for item in items)) <= threshold
            ),
            None,
        )
        if cluster is None:
...
def decide_rule(node_key: str, evidence: dict[str, Any]) -> NodeJudgement:
    ...
    LOGGER.info("调用分类规则，意图=生成独立工程裁决 node=%s", node_key)
...
def main() -> int:
    """记录规则入口只消费匿名 typed evidence。"""

    logging.basicConfig(level=logging.INFO, format="%(levelname)s %(name)s %(message)s")
    LOGGER.info("分类规则示例，意图=说明规则不读取文件名、路径或人工标签")
    return 0
