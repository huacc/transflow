...
def build_cover_template(facts: PageFacts, source_pdf: Path | None = None) -> CoverTemplate:
    ...
    visible_objects = tuple(
        item for item in facts.text_objects if item.object_id not in occluded_ids
    )
    ...
    if semantic:
        ...
        for order, line in enumerate(materialized):
            ...
            role = (
                _role(line.font_size, median_size, hierarchy) if translatable else "literal_repaint"
            )
    ...
    return CoverTemplate(
        ...
        visual_only=visual_only,
        visual_only_reason=(
            "NO_VISIBLE_SEMANTIC_NATIVE_TEXT"
            if visual_only and occluded_ids
            else "NO_SEMANTIC_NATIVE_TEXT"
        )
        if visual_only
        else None,
...
def _logical_lines(objects: tuple[TextObjectFact, ...]) -> tuple[_Line, ...]:
    ...
    for fragment in sorted(fragments, key=lambda item: (item.cy, item.bbox[0])):
        ...
        if compatible:
            min(
                compatible,
                key=lambda row: abs(fragment.cy - statistics.mean(item.cy for item in row)),
            ).append(fragment)
        else:
    ...
    for item in sorted(items, key=lambda candidate: (candidate.bbox[0], candidate.span_index)):
        compatible = next(
            (
                ...
                and min(run[0].font_size, item.font_size) / max(run[0].font_size, item.font_size)
                >= 0.62
            ),
            None,
        )
...
def _occluded_text_object_ids(source_pdf: Path, facts: PageFacts) -> tuple[str, ...]:
    ...
    for item in facts.text_objects:
        ...
        if not trace_candidates:
            continue
        text_trace = max(
            trace_candidates, key=lambda candidate: _intersection_coverage(candidate[1], item.bbox)
        )
...
def _same_row(fragment: _Line, row: list[_Line]) -> bool:
    ...
    overlap = max(
        0.0, min(fragment.bbox[3], reference.bbox[3]) - max(fragment.bbox[1], reference.bbox[1])
    )
    overlap_ratio = overlap / max(0.1, min(fragment.height, reference.height))
    size_ratio = min(fragment.font_size, reference.font_size) / max(
        fragment.font_size, reference.font_size
    )
    return (
        overlap_ratio >= 0.55 and size_ratio >= 0.62 and fragment.color_srgb == reference.color_srgb
    )
...
def _join_text(items: list[tuple[str, Rect, float]]) -> str:
    ...
    for raw_text, bbox, size in items:
        ...
        if previous is not None:
            ...
            if gap > max(previous_size, size) * 0.18 and not (
                _ends_cjk(previous_text) and _starts_cjk(text)
            ):
                pieces.append(" ")
        pieces.append(text)
...
def _allowed_bbox(
    line: _Line, lines: tuple[_Line, ...], anchor: str, width: float, height: float
) -> Rect:
    ...
    above = [
        other.bbox[3] for other in lines if other is not line and other.bbox[3] <= line.bbox[1]
    ]
    below = [
        other.bbox[1] for other in lines if other is not line and other.bbox[1] >= line.bbox[3]
    ]
