...
def build_end_template(
    ...
    translatable_groups = _translation_groups(
        tuple(item for item in classified if item.disposition == "translate")
    )
    ...
    drafts.sort(
        key=lambda row: (
            _union(tuple(item.line.bbox for item in row[0]))[1],
            _union(tuple(item.line.bbox for item in row[0]))[0],
        )
    )
    ...
    for reading_order, (group, role, disposition, protection_reason) in enumerate(drafts):
        ...
        allowed_bbox = (
            _allowed_bbox(
                source_bbox, group_lines, lines, facts.width, facts.height, alignment, font_size
            )
        ...
        regions.append(
            EndTextRegion(
                ...
                source_object_ids=tuple(
                    object_id for line in group_lines for object_id in line.object_ids
                ),
...
def _looks_like_address(text: str) -> bool:
    ...
    return digit_count > 0 and (
        word_count >= 5 or any(token in text for token in ("香港", "北京", "中環", "中环"))
    )


def _translation_groups(
    items: tuple[_ClassifiedLine, ...],
) -> tuple[tuple[_ClassifiedLine, ...], ...]:
...
def _can_group(previous: _ClassifiedLine, current: _ClassifiedLine) -> bool:
    ...
    same_row = (
        abs(current.line.cy - previous.line.cy)
        <= max(previous.line.height, current.line.height) * 0.55
    )
    if same_row:
        return False
...
def _allowed_bbox(
    ...
    obstacle_bottom = (
        min(lower_obstacles) - 1.5
        if lower_obstacles
        else page_height - max(6.0, page_height * 0.02)
    )
