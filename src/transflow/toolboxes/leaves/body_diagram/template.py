...
from transflow.domain.common import content_sha256 as canonical_sha256
from transflow.pdf_kernel.facts import ExtractedPageFacts

from .constants import TOOLBOX_KEY
from .facts import (
    DiagramPageFacts,
    adapt_diagram_page_facts,
)
from .facts import (
    DiagramTextFact as TextObjectFact,
)
...
_PURE_NUMBER = re.compile(
    r"^[\s+\-−–—$¥￥€£HKDUSDRMB,CNY()（）]*\d[\d\s,.]*\+?(?:%|％|‰)?$", re.IGNORECASE
)
...
def build_diagram_template(
    facts: ExtractedPageFacts | DiagramPageFacts,
    source_pdf: Path,
) -> DiagramTemplate:
    if isinstance(facts, ExtractedPageFacts):
        facts = adapt_diagram_page_facts(facts, source_pdf)
    ...
    shapes = _body_node_shapes(
        _node_shapes(details, groups, facts.width, facts.height), facts.height
    )
    node_ids = {index: f"node-{index:03d}" for index in range(len(shapes))}
    group_nodes = {
        index: _containing_shape(group.bbox, shapes) for index, group in enumerate(groups)
    }
    ...
    dense_unowned = len(groups) >= 60 or (len(details) >= 80 and median_size < 6.5)
    ...
    ordered_group_indices = sorted(
        range(len(groups)), key=lambda index: (groups[index].bbox[1], groups[index].bbox[0])
    )
    ...
    if _is_coordinate_locked_map(nodes, connectors, tuple(containers)):
        ...
        promoted_ids = {
            object_id for container in promoted for object_id in container.source_object_ids
        }
        ...
        containers = list(
            _expand_tiny_map_anchor_labels(tuple(containers), facts.width, facts.height)
        )
        nodes = tuple(
            replace(
                node,
...
def _empty_template(
    ...
    return DiagramTemplate(
        ...
        topology_sha256,
        canonical_sha256(
            {"toolbox_key": TOOLBOX_KEY, "mode": "passthrough", "topology_sha256": topology_sha256}
        ),
    )
...
def _text_groups(
    ...
    for fragment in sorted(fragments, key=lambda item: (item.bbox[1], item.bbox[0])):
        ...
        if compatible:
            target = min(
                compatible, key=lambda group: _rect_gap(_make_group(group).bbox, fragment.bbox)
            )
...
def _is_coordinate_locked_map(
    ...
    repeated_labels_per_node = (
        bool(node_owner_counts) and statistics.median(node_owner_counts.values()) > 1
    )
...
def _split_coordinate_locked_containers(
    ...
    *,
    ...
    for container in containers:
        ...
        ordered_blocks = _coalesced_coordinate_blocks(
            tuple(tuple(items) for items in blocks.values())
        )
        for block_index, items in enumerate(ordered_blocks):
            ordered = tuple(
                sorted(items, key=lambda item: (item.line_index, item.bbox[0], item.span_index))
            )
    ...
    ordered_split = sorted(
        split, key=lambda item: (item.source_bbox[1], item.source_bbox[0], item.container_id)
    )
...
def _merge_coordinate_locked_fragments(
    ...
    *,
    ...
    for container in sorted(containers, key=lambda item: item.reading_order):
        ...
        compatible = [
            ...
            and (
                ...
                or any(
                    _coordinate_fragments_are_word_continuation(
                        source_by_id[left_id], source_by_id[right_id]
                    )
                    for existing in group
    ...
    for group in groups:
        ...
        if len(group) == 1 and not _container_uses_map_coordinates(
            base, coordinate_region, page_height
        ):
            merged.append(base)
            continue
        object_ids = tuple(
            dict.fromkeys(
                object_id for container in group for object_id in container.source_object_ids
            )
        )
...
def _coalesced_coordinate_blocks(
    ...
    ordered = sorted(
        blocks,
        key=lambda items: (
            _union([item.bbox for item in items])[1],
            _union([item.bbox for item in items])[0],
        ),
    )
    groups: list[list[TextObjectFact]] = []
    for block in ordered:
        compatible = [
            ...
            or any(
                _coordinate_fragments_are_word_continuation(left, right)
                for left in group
                for right in block
            )
        ]
...
def _coordinate_fragments_are_word_continuation(
    left: TextObjectFact, right: TextObjectFact
) -> bool:
...
def _make_group(items: list[TextObjectFact] | tuple[TextObjectFact, ...]) -> _TextGroup:
    ...
    return _TextGroup(
        ordered,
        _union([item.bbox for item in ordered]),
        max(item.font_size for item in ordered),
        style.color_srgb,
    )


def _can_merge_group(left: _TextGroup, right: _TextGroup) -> bool:
    size_ratio = max(left.font_size, right.font_size) / max(
        0.1, min(left.font_size, right.font_size)
    )
    if size_ratio > 1.35 or left.color_srgb != right.color_srgb:
        return False
    same_baseline = abs(left.bbox[1] - right.bbox[1]) <= max(
        0.75, max(left.font_size, right.font_size) * 0.15
    ) and abs(left.bbox[3] - right.bbox[3]) <= max(
        0.75, max(left.font_size, right.font_size) * 0.15
    )
    horizontal_gap = max(0.0, max(left.bbox[0], right.bbox[0]) - min(left.bbox[2], right.bbox[2]))
    ...
    if (
        same_source_block
        and max(left.font_size, right.font_size) * 0.6
        <= top_delta
        <= max(left.font_size, right.font_size) * 1.8
        and (
            overlap >= minimum_width * 0.35
            or anchor_gap <= max(8.0, max(left.font_size, right.font_size) * 1.5)
        )
    ...
    if vertical_gap > max(6.0, max(left.font_size, right.font_size)):
        return False
    return overlap >= minimum_width * 0.35 or anchor_gap <= max(
        8.0, max(left.font_size, right.font_size) * 1.5
    )
...
def _node_shapes(
    details: list[_DrawingDetail], groups: list[_TextGroup], width: float, height: float
) -> list[_NodeShape]:
    ...
    for detail in selected:
        cluster = next(
            (row for row in clusters if any(_iou(detail.bbox, item.bbox) >= 0.65 for item in row)),
            None,
        )
        if cluster is None:
    ...
    for cluster in clusters:
        ...
        bbox = (
            intersection
            if _area(intersection) >= smallest_area * 0.5
            else min(cluster, key=lambda item: _area(item.bbox)).bbox
        )
        shapes.append(
            _NodeShape(_round_rect(bbox), tuple(sorted(item.object_id for item in cluster)))
        )
...
def _connectors(
    details: list[_DrawingDetail], nodes: tuple[DiagramNode, ...]
) -> tuple[DiagramConnector, ...]:
    node_by_drawing = {drawing_id for node in nodes for drawing_id in node.source_drawing_ids}
    arrow_shapes = [
        item
        for item in details
        if item.object_id not in node_by_drawing
        and item.fill is not None
        and 2.0 <= _area(item.bbox) <= 140.0
    ]
        ...
        for start, end in detail.lines:
            ...
            same_node = next(
                (
                    node
                    for node in nodes
                    if _point_in_rect(start, node.boundary_bbox, 1.0)
                    and _point_in_rect(end, node.boundary_bbox, 1.0)
                ),
                None,
            )
            if same_node:
    ...
    for index, (start, end, drawing_id) in enumerate(segments):
        ...
        if (
            ...
            and not _line_near_nodes(start, end, nodes)
        ):
            continue
...
def _constrain_connector_corridors(
    ...
    for container in containers:
        ...
        if horizontal_bands:
            ...
            bottom = max(
                container.source_bbox[3], min(container.allowed_bbox[3], boundary_y - gutter)
            )
            result.append(
                replace(
                    container,
        ...
        right = min(
            container.allowed_bbox[2], min(right_boundaries, default=container.allowed_bbox[2])
        )
        right = max(right, container.source_bbox[2])
        bottom = min(
            container.allowed_bbox[3], min(bottom_boundaries, default=container.allowed_bbox[3])
        )
...
def _is_protected_group(
    ...
    *,
    ...
    if (
        dense_unowned
        and group.font_size < 6.0
        and diagram_bbox
        and _center_inside(group.bbox, diagram_bbox, tolerance=0.0)
    ):
        return True
    return False
...
def _role(
    ...
    if group.font_size >= median * 1.18:
        return "title"
    if (
        _width(group.bbox) >= width * 0.35
        or len(text) >= 120
        or _height(group.bbox) >= group.font_size * 2.2
    ):
        return "independent_paragraph"
    return "independent_label"


def _derived_allowed_bbox(
    source: Rect,
    role: str,
    index: int,
    ...
    horizontal_peers = [
        ...
        and min(current_group.font_size, group.font_size)
        >= max(current_group.font_size, group.font_size) * 0.70
        and _axis_overlap((y0, y1), (group.bbox[1], group.bbox[3]))
        >= min(_height(source), _height(group.bbox)) * 0.35
        and (group.bbox[0] >= x1 or group.bbox[2] <= x0)
    ]
    obstacles = [group.bbox for other_index, group in enumerate(groups) if other_index != index] + [
        shape.bbox for shape in shapes
    ]
    right_blockers = [
        obstacle[0]
    ...
    peer_right_boundaries = [
        group.bbox[0] - horizontal_gutter for group in horizontal_peers if group.bbox[0] >= x1
    ]
...
def _coordinate_locked_region(
    nodes: tuple[DiagramNode, ...], width: float, height: float
) -> Rect | None:
    if not nodes:
        return None
...
def _map_image_title_frame(
    ...
    *,
    ...
    if coordinate_region is None or not _center_inside(
        source_bbox, coordinate_region, tolerance=8.0
    ):
        return None
...
def _recover_source_text_objects(
    ...
    trusted_counts = Counter(
        ...
        if not any(_pdf_font_names_match(item.font_name, basefont) for basefont, _ in contexts)
        for token in re.findall(r"(?<![A-Za-z0-9])[A-Z]{2,6}(?![A-Za-z0-9])", item.text)
    )
    trusted_ascii_tokens = {token for token, count in trusted_counts.items() if count >= 2}
...
def _standard_cid_collection(basefont: str, encoding: str) -> str | None:
    ...
    return next(
        (
            collection
            for tokens, collection in collections
...
def _nearest_node(
    point: Point, nodes: tuple[DiagramNode, ...], tolerance: float = 9.0
) -> str | None:
    ranked = sorted(
        (_point_rect_distance(point, node.boundary_bbox), node.node_id) for node in nodes
    )
...
def _intersection(rects: list[Rect]) -> Rect:
    return (
        max(item[0] for item in rects),
        max(item[1] for item in rects),
        min(item[2] for item in rects),
        min(item[3] for item in rects),
    )
