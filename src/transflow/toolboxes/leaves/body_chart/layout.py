...
_FAILURE_DISPATCH_MANIFEST = (
    "resources/manifests/toolbox_leaf_migration/"
    "body_chart_failure_dispatch.json"
)
_LAYOUT_TOOL = "src/transflow/toolboxes/leaves/body_chart/layout.py"
...
def plan_chart_layout(
    ...
    *,
    ...
    translated = {
        ...
        for container_id, text in translated_by_container.items()
    }
    font_path = str(Path(font_file).resolve())
    bold_path = (
        str(Path(bold_font_file).resolve())
        if bold_font_file and Path(bold_font_file).is_file()
        else font_path
    )
    ...
    for container in template.containers:
        ...
        selected_font = bold_path if _is_bold(container.font_name) else font_path
        resource = "tfchartb" if selected_font == bold_path and bold_path != font_path else "tfchart"
        missing_codepoints = _missing_codepoints(Path(selected_font), text)
        if missing_codepoints:
            placements_by_id[container.container_id] = _unfit(
                container,
                text,
                selected_font,
                resource,
    ...
    for group in fit_groups.values():
        group_placements = _fit_group(template, group, minimum_font_size)
...
def _missing_codepoints(font_file: Path, text: str) -> tuple[str, ...]:
    """Return the stable set of glyphs absent from the controlled font."""
...
def _fit_group(
    template,
    group: list[tuple[object, str, str, str]],
    minimum_floor: float,
) -> list[ChartPlacement]:
    ...
    for profile, scale, line_height in profiles:
        sizes = tuple(
            max(_minimum_font_size(container, minimum_floor), container.font_size * scale)
            for container, _, _, _ in group
        )
        ...
        if fit_count == len(group):
            ...
            for index, (size, (container, text, font_file, resource)) in enumerate(zip(sizes, group)):
                ...
                placements.append(
                    ...
                    resource,
                    round(size, 4),
                    round(_minimum_font_size(container, minimum_floor), 4),
                    line_height,
    ...
    if best is None:
        return [
            _unfit(container, text, font_file, resource, minimum_floor)
            for container, text, font_file, resource in group
        ]
...
def _slot_profiles(container, font_size: float) -> tuple[tuple[str, Rect], ...]:
    ...
    if (
        container.role in _TABLE_ROLES
        and container.alignment == "LEFT"
        and safe_right - safe_left
        > source[2] - source[0] + font_size
    ):
        ...
        records.append(
            {
                ...
                "dispatch_result": {
                    "dispatch_table": _FAILURE_DISPATCH_MANIFEST,
                    "selected_repair_atom": repair_atom,
                    "bound_tool": _LAYOUT_TOOL,
                },
                "evidence": {
    ...
    for placement in plan.placements:
        ...
        selected = next(
            (
                ...
                for font_size, line_height in _diagnostic_typography_profiles(
                    container.font_size,
                    placement.minimum_font_size,
                )
                if _probe(
                    template.width,
                    template.height,
...
def _diagnostic_slot_profiles(template: ChartTemplate, container) -> tuple[tuple[str, Rect], ...]:
    ...
    candidates: tuple[tuple[str, Rect], ...] = (
        ("diagnostic-safe-wrap", container.allowed_bbox),
        ("diagnostic-page-wrap", page_lane),
    )
    below_source_top = container.source_bbox[3] + max(
        1.0,
        container.font_size * 0.20,
    )
    if (
        container.role == "ANNOTATION"
        and container.allowed_bbox[3] - below_source_top
        >= container.font_size * 2.0
    ):
        candidates = (
            (
                "diagnostic-below-source",
                (
...
def _diagnostic_typography_profiles(
...
def _unfit(
    ...
    minimum = _minimum_font_size(container, minimum_floor)
...
def _minimum_font_size(container, minimum_floor: float = 0.0) -> float:
    if container.role == "AXIS_OR_CATEGORY_LABEL" and container.rotation:
        floor = 4.0
        relative_floor = 0.55
    ...
    return max(minimum_floor, floor, container.font_size * relative_floor)
