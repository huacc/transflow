...
def build_contents_template(facts: PageFacts) -> ContentsTemplate:
    ...
    entry_font = statistics.median(
        line.font_size for entry in entries for line in entry.primary_lines
    )
    ...
    for line in nonnumeric_lines:
        ...
        if line == title or key in consumed_lines:
            continue
        if line.bbox[1] < title.bbox[1] - 0.5 or line.bbox[1] > content_bottom + max(
            25.0, entry_font * 3.0
        ):
            continue
        ...
        is_heading = (
            ...
            or (
                _is_upper_heading(line.text)
                and _has_following_entry(line, entries, column_index, entry_font)
            )
        )
        if not is_heading:
            continue
        role = "column_header" if normalized in _COLUMN_HEADERS else "group_heading"
        if role == "group_heading" and not _has_following_entry(
            line, entries, column_index, entry_font
        ):
            following = [
                entry
            ...
            if following:
                column_index = min(
                    following, key=lambda entry: entry.primary_bbox[1] - line.bbox[3]
                ).column_index
    ...
    protected_ids = tuple(
        sorted(object_id for line in protected_lines for object_id in line.object_ids)
    )
...
def _logical_lines(objects: tuple[TextObjectFact, ...]) -> tuple[_Line, ...]:
    ...
    for (block_index, line_index), items in grouped.items():
        ...
        for item in items:
            if (
                ...
                and item.bbox[0] - previous.bbox[2] > max(previous.font_size, item.font_size) * 0.45
            ):
                pieces.append(" ")
    ...
    return tuple(
        sorted(
            lines, key=lambda line: (line.bbox[1], line.bbox[0], line.block_index, line.line_index)
        )
    )
...
def _page_anchor_clusters(
    ...
    candidates = [
        ...
        and (
            line.font_size <= max(18.0, reference * 1.6)
            or line.text.strip().casefold().startswith("p")
        )
    ]
...
def _match_label(
    ...
    for line in candidates:
        ...
        score = (
            vertical * 8.0 + horizontal_gap * 0.03 + side_penalty - (80.0 if same_block else 0.0)
        )
...
def _continuation_lines(
    ...
    for line in aligned:
        ...
        if key in matched_keys:
            break
        if (
            abs(line.font_size - primary.font_size) > font_tolerance
            or line.color_srgb != primary.color_srgb
        ):
            break
        result.append(line)
        previous = line
...
def _column_bands(
    entries: list[_EntryDraft], width: float, height: float
) -> tuple[ContentsColumnBand, ...]:
    ...
    extents = [
        (
            statistics.median(
                min(entry.source_bbox[0], entry.page_anchor.bbox[0]) for entry in group
            ),
            statistics.median(
                max(entry.source_bbox[2], entry.page_anchor.bbox[2]) for entry in group
            ),
        )
    ...
    for left, right in pairwise(extents):
        boundaries.append(
            (left[1] + right[0]) / 2.0 if left[1] < right[0] else (left[1] + right[1]) / 2.0
        )
    ...
    for index, group in enumerate(ordered):
        ...
        bands.append(
            ContentsColumnBand(
                ...
                page_anchor_x=round(
                    statistics.median(entry.page_anchor.bbox[0] for entry in group), 4
                ),
                anchor_side=side,
            )
        )
...
def _nearest_column(
    line: _Line, entries: list[_EntryDraft], bands: tuple[ContentsColumnBand, ...]
) -> int:
    ...
    return min(
        ...
        key=lambda index: min(
            abs(line.cx - entry.primary_bbox[0]) for entry in entries if entry.column_index == index
        ),
    )


def _nearest_same_row_entry(
    line: _Line, entries: list[_EntryDraft], column_index: int
) -> _EntryDraft | None:
    ...
    entry = min(
        candidates,
        key=lambda item: abs(
            item.primary_bbox[1] + item.primary_bbox[3] - line.bbox[1] - line.bbox[3]
        ),
    )
...
def _has_following_entry(
    line: _Line, entries: list[_EntryDraft], column_index: int, entry_font: float
) -> bool:
    return any(
...
def _assign_hierarchy(entries: list[_EntryDraft], structural: list[_ContainerDraft]) -> None:
    for column_index in sorted({entry.column_index for entry in entries}):
        ...
        headings = sorted(
            (
                draft
                for draft in structural
                if draft.column_index == column_index and draft.role == "group_heading"
            ),
...
def _materialize_containers(
    ...
    ordered = sorted(
        drafts,
        key=lambda draft: (
            -1 if draft.role == "title" else draft.column_index,
            draft.bbox[1],
            draft.bbox[0],
        ),
    )
    ...
    for reading_order, draft in enumerate(ordered):
        ...
        if draft.role == "title":
            ...
            later = [
                value for value in row_tops[draft.column_index] if value > source_bbox[1] + 0.5
            ]
            y1 = max(
                source_bbox[3],
                (min(later) - 0.75)
                if later
                else source_bbox[3] + max(4.0, draft.source_lines[0].font_size * 1.2),
            )
        ...
        containers.append(
            ContentsContainer(
                ...
                source_object_ids=tuple(
                    object_id for line in draft.source_lines for object_id in line.object_ids
                ),
...
def _auxiliary_right_boundary(
    draft: _ContainerDraft, lines: tuple[_Line, ...], page_width: float
) -> float:
    source_bbox = draft.bbox
    ...
    for line in lines:
        if source_ids.intersection(line.object_ids):
            continue
        vertically_aligned = (
            min(source_bbox[3], line.bbox[3]) - max(source_bbox[1], line.bbox[1]) > -0.5
        )
...
def _is_upper_heading(text: str) -> bool:
    ...
    return (
        bool(letters)
        and len(text) <= 80
        and all(not char.isascii() or not char.isalpha() or char.isupper() for char in letters)
    )


def _normalized(text: str) -> str:
    return "".join(
        char for char in text.casefold() if char.isalnum() or "\u4e00" <= char <= "\u9fff"
    )
