...
| `planners\build_layout_plan.py` | planner | `role_plan.json`, `layout_policy.json`, output layout plan path | `layout_plan.shadow.json` with projected target rects, erase rects, draw modes, source-relative font profiles, and overlap hints | missing role plan/policy, invalid rects, empty groups | phase-1 shadow planner that makes current-run target layout decisions visible without changing the legacy generator path; later phases may make the generator consume this plan |
...
- `role_plan.json` and `layout_plan.shadow.json` are S6 phase-1 merge artifacts. They prove role/layout planning can be derived before generation, but `layout_plan.shadow.json` is not yet consumed by `generate_semantic_backfill.py`; compare it with generated `layout_plan.json` during regression before enabling the v2 generator path.
