...
> 文档状态：Draft，已补入 P9A/P9B Repair Memory 修复阶段及并发共享、页级软布局边界
> 编制日期：2026-07-20
> 计划范围：Transflow 独立闭环 + MerqFin 联调替换
> 主依据：`docs/设计/Transflow_PDF翻译排版引擎_总体设计_v0.1.md`（下称“【总体设计】”）

本文是后续开发和验收的施工记录，不是第二份总体设计。它冻结阶段顺序、依赖、任务边界、产出物、测试范围和 Gate；类内部算法、函数拆分和少量文件名允许实施时按仓库现状做小幅修正，但不得改变当前批准版本【总体设计】已经确认的系统边界和主流程不变量。每一施工部分都必须绑定明确的设计版本与 SHA-256；设计修订只能由阶段 Gate 的实测证据触发、经项目负责人批准后形成新版本，禁止实施者静默覆盖当前基线后继续施工。
...
当前没有阻塞计划编制的 `[待决策]`。凡【总体设计】§27 已明确要求“实施阶段冻结”的细项，已被安排到对应阶段，不在本计划中静默假定答案。
...
1. **第一部分 P0–P14：Transflow 独立闭环。** 从生产目录和环境开始，依次完成合同、Standalone Adapter、完整 PDF 纵向切片、分类迁移、PdfKernel、逐叶工具箱、P9A/P9B Repair Memory、Freeform、暂停恢复、并发与故障注入，最后在固定翻译 Provider 下完成整本 PDF 闭环。P9A/P9B 是插入 P9 与 P10 之间的修复阶段，不重编号既有下游阶段。
...
- 第一部分使用文件 Checkpoint 和固定/确定性翻译 Provider 完成独立闭环。
...
- 一个阶段暴露的上游缺陷必须回到上游修复并重过 Gate，不在下游加旁路补丁。
...
3. 向项目负责人提问，不静默选择。
4. 收到回复后，若只影响施工细节则更新本计划的决策记录和追溯矩阵；若改变设计目标、边界或主流程，则保留原设计版本并形成经负责人批准的新版本，禁止原地静默改写验收基线。
...
- 测试 ID 格式固定为 `P<阶段>.<子任务>-T<两位序号>`，例如 `P3.1-T01`；字母修复阶段使用 `P9A.1-T01`、`P9B.1-T01`。端到端用例继续使用附录 C 的 `TF-E2E-*`、`MF-E2E-*`。
...
│  ├─ schemas/{model_decision.v1.schema.json,page_translation.v1.schema.json,document_layout_memory.v1.schema.json,page_repair_memory.v1.schema.json}/
...
| P9A | 文档级布局记忆 | 全页屏障、源布局基线、翻译前目标策略、并发只读快照、失效与恢复 | G9 | G9A | T4 Repair Memory A |
| P9B | 页级修复记忆与多轮重排 | 页级有效布局、失败账本、跨文档规则、真实候选、接受/回滚与停止 | G9A | G9B | T4 Repair Memory B |
...
**前置依赖与交付接口：** 输入为 P9.1–P9.6 signed leaf records、G8 mixed baseline 和当前 Catalog；输出为 second-batch mixed E2E、Catalog/outcome matrix、G8 regression diff 和 stable baseline，供 G9/P9A。
...
| 设计追溯 | §4、§7.1–§7.4、§9.1/§9.5、§13.4、§17–§19、§22 T4、§24.1/§24.3–§24.5、§25–§27 |
| 依赖 | G9 |
| 关键产物 | memory Schema/Domain、DocumentLayoutMemoryBuilder、内容寻址 Artifact、Checkpoint 引用、真实完整 PDF 基线、G9A 证据 |

P9A 建立“当前源 PDF 的布局事实 + 目标语言布局政策”快照，不实现页级修复循环，不自动产生跨文档规则。`source_layout_baseline` 从完整源事实生成；`target_layout_policy` 在源基线的角色层级和几何边界上，根据语言对、受控字体和配置派生，因此不需要等待译后页面。源 PDF 中当前页面的 bbox/geometry 可以作为本 run 的事实保存；禁止的是从第一张页面提前冻结、把这些坐标写成适用于其他 PDF 的固定规则，或在并发运行中原地修改全局记忆。

### P9A.1 文档记忆合同、身份与失效

**实现功能：** 定义 `DocumentLayoutMemory`、`SourceLayoutBaseline`、`TargetLayoutPolicy`、`PageLayoutBaseline`、`LayoutRoleProfile`、`LayoutFactProvenance` 和内容寻址引用的领域/JSON Schema 合同。

**任务描述：** 冻结文档级记忆的最小字段、类型、来源标记、哈希算法和兼容规则。identity 必须绑定 source、源/目标语言、页几何、配置、DocumentLayoutMemory Builder、Classification、Catalog/布局事实提供器、Kernel/Patch interpreter、字体和 memory schema 指纹；任一指纹变化都不得静默复用。`TargetLayoutPolicy` 只声明字体回退、字形覆盖和字号/行距/段距/换行等受控调整范围，不保存尚不存在的译后页面事实，也不要求各页参数完全相同。RepairRuleRegistry 不改变源布局事实，不进入 document memory hash，而在 P9B 由 run 快照和 PageRepairMemory 单独绑定。正文/表格文字只保存稳定对象引用和内容哈希，不复制无界原文或 Provider 响应。

**设计追溯：** 【总体设计】§4、§9.5.1、§9.5.5、§13.4、§17、§18.3、§24.1、§25–§27。

**前置依赖与交付接口：** 输入为 G2 Domain/Schema 规范、G5 ClassificationRoute、G6 PageFacts/Preservation、G7 Catalog 和 G9 第二批叶事实；输出为 `document_layout_memory.v1` Schema、领域对象、canonical serializer/hash 和失效判定，供 P9A.2–P9A.4/P9B/P10。

**实施步骤：**

1. 冻结文档/源布局基线/目标布局政策/页面/角色/事实来源模型，明确 `observed | inferred`、confidence、provenance 和软/硬约束语义。
...
- `P9A.1-T01`：构造覆盖 `source_layout_baseline`、`target_layout_policy`、页面、角色、表格和视觉事实的完整合同；序列化再加载；预期字段、类型、provenance 和 hash 一致率 `100%`。
- `P9A.1-T02`：对相同输入以不同 map/事实枚举顺序构建两次；预期 canonical JSON 和 `memory_hash` 完全一致。
- `P9A.1-T03`：逐项改变 source、源/目标语言对、page geometry、config、Builder、classifier、Catalog/布局事实提供器、Kernel/interpreter、font 或 Schema 指纹；预期旧记忆复用全部被拒绝并指出变化字段；只改变 Registry 时 document memory hash 保持不变。
...
**任务描述：** Builder 必须等待全部原始页 `PageFacts`、完整 Route 和公共边缘索引就绪；部分输入只能返回 `NOT_READY`，不得由第一张或首批页面发布文档记忆。源基线覆盖页框/旋转、正文/标题/列表、字体和 baseline、行间距/段落距离、缩进/对齐、栏/gutter/reading order、表/行/列/cell、图片/图表/示意图/Logo/图标 anchor、保护边界、页眉页脚和重复元素。PDF 可直接读出的事实标记 `observed`；根据相邻 baseline、空白和聚类推断的段落距等标记 `inferred + confidence`，低置信结果不得升级为硬约束。目标策略在源基线之上从语言对、字体 manifest 和配置派生，构建阶段不得调用 TranslationPort；视觉对象内部文字不 OCR、不翻译。

**设计追溯：** 【总体设计】§7.1–§7.3、§9.3、§9.5.1、§10、§12、§20.6、§24.3/§24.5。

**前置依赖与交付接口：** 输入为 P9A.1 合同、G6 PageFacts/Kernel、G5 全页 Route、P7 Margin 和 G8/G9 叶级 facts；输出为 `DocumentLayoutMemoryBuilder`、完整 source page baseline/role profile/shared region map 与 target layout policy，供 P9A.3/P9B Repair 和所有后续 Toolbox。

**实施步骤：**

1. 建立全页事实/Route/公共边缘屏障；在页数或任一必需输入未齐时返回可追溯 `NOT_READY`，不创建可消费 Artifact。
2. 聚合完整 PDF 的 page boxes、rotation、文字 span/baseline、drawing/image、table、Route、owner 和重复边缘事实，形成 `source_layout_baseline`。
3. 按语义角色计算字体、字号、颜色、对齐、缩进、行距和段落距分布；保存样本数、统计量、observed/inferred、confidence 和允许区间。
4. 建立列/gutter/reading order、表格 cell、视觉 anchor/slot/protected region 和回退边界，并引用原 PageFacts ID/hash；根据语言对、字体与配置派生 `target_layout_policy`。
5. 执行完整性、所有权、protected 和低置信软约束校验后冻结快照；TranslationPort、候选页和译后布局均不得成为 Builder 输入。

**测试用例：**

- `P9A.2-T01`：输入 `样本/年报` 中一份未经拆页的完整真实 PDF；先仅提交首张/部分页事实，再提交完整 PageFacts/Route/公共边缘索引；预期前者返回 `NOT_READY` 且可消费 Artifact 数为 `0`，后者只构建一次，page baseline 数等于源页数且漏页/重复页为 `0`。
...
- `P9A.2-T03`：选择 `spikes/page_classification_engine_puncture_v1/分类结果` 中 table 对应真实页；实际提取；预期表/行/列/cell、owner、边框/padding 和整表回退边界可追溯。
- `P9A.2-T04`：选择含图表、示意图、Logo/图标或图片的真实页；预期视觉对象 geometry/hash/anchor/protected 被记录，图片 OCR/TranslationUnit/Patch 数均为 `0`。
...
**验收标准：** 部分输入发布数为 `0`；完整 PDF page baseline、可编辑/KEEP_SOURCE owner 和事实 provenance 覆盖率均 `100%`；每 run 构建次数为 `1`，冻结前翻译调用、候选污染、图片 OCR/翻译和身份特判均为 `0`。
...
- `P9A.3-T05`：恢复前改变 source、源/目标语言对、config、font、Builder、classifier、Catalog/布局事实提供器或 Kernel 任一指纹；预期拒绝旧 document memory；只改变 Registry 时保留 document memory 但拒绝旧 PageRepairMemory，陈旧复用数 `0`。
...
**前置依赖与交付接口：** 输入为 P9A.1–P9A.3、G8/G9 真实叶样本和现有 regression harness；输出为 real-document memory manifests、覆盖矩阵、determinism/anti-overfit diff 和 G9A evidence，供 P9B/P10/P14。

**实施步骤：**

1. 登记至少两份完整真实 PDF 的相对路径、source hash、页数、结构覆盖和选择理由。
2. 实际构建并校验 document memory、逐页 baseline、角色画像、shared/protected facts 和 Artifact/Checkpoint。
3. 对同一输入重复运行，并执行重命名、顺序/缩放和结构扰动，输出字段级 diff 与反过拟合扫描。
4. 重跑 G8/G9 mixed PDF，确认只增加只读 Context，不改变 Route/Toolbox/Patch/PageOutcome。

**测试用例：**

- `P9A.4-T01`：对两份完整真实 PDF 运行 Builder；预期两份均产生可校验 memory Artifact，页覆盖率和 hash 验证率 `100%`。
- `P9A.4-T02`：用分类结果真实页补齐 role/leaf 覆盖；预期覆盖矩阵中正文、表格、视觉 anchor、multi、anchored 和公共边缘均有真实 evidence ref。
- `P9A.4-T03`：同一 source/config 连续构建两次；预期 canonical memory diff 为 `0`。
- `P9A.4-T04`：重命名输入并扰动顺序/缩放；预期行为只随事实变化，文件名/sample_id/公司名分支命中 `0`。
- `P9A.4-T05`：检查 memory Artifact 内容和增长；预期 page record 数等于页数，不重复保存完整原文/候选/Provider 响应，未登记路径命中 `0`。
- `P9A.4-T06`：重跑 G8/G9 回归；预期 Route/Toolbox/Patch/PageOutcome 解释差异和第一/二批退化均为 `0`。

**验收标准：** 两份完整真实 PDF 与补充真实页全部实际执行；页/角色/结构覆盖、hash 校验和重复运行确定性均 `100%`；身份特判、无界内容、候选污染和 G8/G9 退化均为 `0`。
...
| G9A-2 | 构建时机/布局事实 | 部分输入发布 `0`；完整页、owner、provenance 覆盖率 `100%`；冻结前翻译、图片 OCR/翻译 `0` | §9.5.1；P9A.2 | P9A.2-T01～T04 |
...
| G9A-6 | 真实证据/回归 | 至少两份完整真实 PDF；Artifact/hash `100%`；G8/G9 退化 `0` | §24.3–§24.5；P9A.4 | P9A.4-T01～T06 |

**验收方法：** 运行 Schema/identity 单测、完整 PDF Builder、并发/崩溃恢复、真实样本覆盖、扰动和 G8/G9 回归；逐项保留命令原始输出与 Artifact hash。

**通过标准：** G9A-1～G9A-6 全部 PASS；只要存在首张/部分页面提前冻结、冻结前翻译调用、可变文档记忆、运行中版本混用、陈旧复用、候选污染或无真实完整 PDF 证据，G9A 即 FAIL。

---

## P9B · 页级修复记忆、经验规则与多轮重排

| 项 | 内容 |
|---|---|
| 阶段目标 | 让每页在同一只读文档记忆下形成可不同的 `PageEffectiveLayout` 并携带可恢复失败经验，执行有限多轮重排；每轮实际产出候选，最终接受安全 Patch 或明确降级，不做全书二次样式归一 |
| 设计追溯 | §7.1–§7.4、§9.1/§9.5、§12–§15、§17–§19、§22 T4、§24–§27 |
| 依赖 | G9A |
| 关键产物 | PageRepairMemory/RepairAttempt Schema、RepairRuleRegistry、RepairCoordinator、每轮 Artifact/Checkpoint、真实样本多轮证据、G9B 结论 |

P9B 不重新设计叶级布局算法。它统一“记住失败、选择未试动作、重排、产出候选、比较、接受/回滚、停止”的协调闭环；具体 Finding 和 Repair 原子仍归各 PageToolbox。

### P9B.1 页级记忆与跨文档规则合同

**实现功能：** 定义 `PageEffectiveLayout`、`PageRepairMemory`、`RepairAttempt`、`RepairProposal`、`RepairBudget` 和只读 `RepairRuleRegistry` 的领域/Schema/版本合同。

**任务描述：** 页级记忆绑定 source/page/Route/Toolbox/config/document-memory/registry hash，保存初始 `PageEffectiveLayout`、已尝试 action、已见 state、每轮页级有效布局差异、前后质量、候选证据、接受/回滚和终止原因。Registry 只保存 `failure_class + applicability + repair_atom + guards + evidence`，不得保存样本身份、历史原文或跨文档固定坐标；生产运行不能写 Registry。

**设计追溯：** 【总体设计】§4、§9.5.2–§9.5.5、§13.2/§13.4、§15、§17–§18、§24.1/§24.5。

**前置依赖与交付接口：** 输入为 G9A immutable document memory、G2 Domain/Schema、G7 Toolbox Repair 合同和 G3 Checkpoint；输出为 effective-layout/page-memory/attempt/proposal/budget/registry Schema、canonical effective-layout/action/state hash 和状态转换校验，供 P9B.2–P9B.4/P10/P12。

**实施步骤：**

1. 冻结 page memory identity、初始/每轮 `PageEffectiveLayout` 与差异、append-only attempt、候选/质量引用和终止枚举。
2. 定义 deterministic `action_key`、`state_hash`、重复动作/状态循环和非法状态转换规则。
3. 建立 Registry Schema、只读加载/hash、applicability allow-list 和 evidence/promotion 字段。
4. 增加边界扫描：Registry 不含身份/历史内容/绝对坐标，运行时写入和无证据规则均拒绝。

**测试用例：**

- `P9B.1-T01`：构造含初始/每轮 `PageEffectiveLayout` 与页级差异、三次尝试、接受/回滚和终止的完整 page memory；round-trip；预期顺序、引用和 hash 一致率 `100%`。
...
- `P9B.1-T04`：扫描 Registry；预期 sample_id、文件名、公司名、固定页码、历史原文/候选和跨文档绝对坐标命中 `0`。
- `P9B.1-T05`：生产运行尝试新增/修改 Registry 规则或 registry hash；预期只读边界/CAS 拒绝，运行时写入数 `0`。
- `P9B.1-T06`：加载 source/page/Toolbox/document-memory/registry 任一 identity 不匹配的 page memory；预期陈旧记忆拒绝率 `100%`。

**验收标准：** 合同 round-trip、action/state 确定性和陈旧拒绝率均 `100%`；非法转换、身份/历史内容、跨文档固定坐标和运行时 Registry 写入均为 `0`。

### P9B.2 RepairCoordinator 多轮重排

**实现功能：** 实现页级有效布局解析、首轮候选登记、未尝试动作选择、源页重排、真实候选生成、前后裁决、接受/回滚和确定性 fallback。

**任务描述：** 每页固定读取同一 `document_memory_hash`，先以当前页硬事实/Route 为直接依据，以文档角色画像和 `target_layout_policy` 为软先验，再结合 TranslationBundle 的实际长度/密度与 PageRepairMemory 解析 `PageEffectiveLayout`。不同结构页面允许选择不同字号、行距、段距、换行和修复参数；这些决策及变化只记入本页候选/attempt，不回写文档记忆。默认 `max_repair_rounds=3`、`max_no_improvement=2` 并进入 config snapshot。每个实际轮次只执行一个主要动作，从不可变源页和当前最佳完整计划重新生成候选；布局修复必须复用同一有效 TranslationBundle。每轮无论成功失败都保存可打开候选；目标 Finding 改善且无硬回归才接受。重复 action/state、连续无改善、无适用动作、预算耗尽或硬失败必须停止并降级。页面终态后不执行全书二次样式归一、布局重排或质量重判；Document Finalizer 仍执行全部发布硬校验。

**设计追溯：** 【总体设计】§7.2–§7.4、§9.5.2–§9.5.4、§12、§13.2、§15.1–§15.2、§18.2。

**前置依赖与交付接口：** 输入为 P9B.1 合同、G9A 文档记忆、G7 Toolbox/Judge/Repair、G6 Kernel render/constraints 和 G3 Translation/Artifact；输出为 `RepairCoordinator`、候选比较/接受/回滚、终止/fallback 和结构化日志，供 P9B.3/P9B.4/P10。

**实施步骤：**

1. 按“文档软先验 + 当前页硬事实 + TranslationBundle”解析并登记 candidate-0 的 `PageEffectiveLayout`、Finding、质量向量和初始 state，解析适用规则与叶私有 Repair 原子。
2. 过滤已尝试 action/已见 state，按确定性优先级只选一个动作并生成 RepairProposal。
3. 从源页、同一 TranslationBundle 和更新后的完整 layout plan 实际生成 candidate/preview/Patch，再用同一 Judge 比较。
4. 接受无硬回归的改善候选，否则回滚；把有效布局变化只追加到当前页记忆，登记后决定下一轮、定向重译例外或 fallback。
...
- `P9B.2-T02`：让三个不同动作均无法解决实际布局问题；预期 candidate-0 与三个实际候选均可打开，预算耗尽后区域/页面 fallback 并形成最终 PDF。
- `P9B.2-T03`：Registry/Toolbox 返回已尝试 action；预期跳过并选择下一个，已提交 action 实际执行重复数 `0`。
- `P9B.2-T04`：两个不同动作生成相同 state hash；预期第二状态被 `STATE_CYCLE` 拦截，不再循环渲染。
- `P9B.2-T05`：连续两轮质量向量无改善；预期按默认阈值 `NO_IMPROVEMENT` 停止，不执行第三个无意义动作。
...
**验收标准：** 每个实际轮次真实候选/证据产出率、接受/回滚可追溯率和最终 PageOutcome/PDF 完成率均 `100%`；同一文档 hash 下页级差异可追溯且不回写全局；重复已提交动作、状态死循环、布局失败重复翻译、硬回归发布和全书二次样式归一/布局/Judge 调用均为 `0`，发布硬校验跳过数为 `0`。

### P9B.3 Repair Checkpoint、崩溃恢复与 Artifact

**实现功能：** 将每轮 proposal/candidate/Patch/Finding/Decision/page memory 作为确定性、内容寻址、可恢复安全点提交。

**任务描述：** 本轮所有 Artifact（含 `PageEffectiveLayout`/页级差异）校验并原子 rename 后，Checkpoint 才引用新版 PageRepairMemory。Artifact 已写但 Checkpoint 未提交时，恢复器按 action key/hash 复用同一轮并补交，不增加预算、不重复翻译；Checkpoint 已提交后跳过该 action/state。旧 Worker 迟到提交、损坏/陈旧证据和“失败候选直接成为 final”必须被拒绝。

**设计追溯：** 【总体设计】§9.5.2/§9.5.5、§13.4–§13.5、§17、§18.1/§18.3、§24.4。
...
2. 实现候选/证据校验、原子 rename、Checkpoint 引用以及 orphan reuse/cleanup 分支。
3. 实现 committed attempt 跳过、预算恢复、旧 Worker fencing 和损坏/陈旧 memory 拒绝。
4. 让 Page Finalizer 只消费 ACCEPTED/批准 reduced Patch；REJECTED/ROLLED_BACK 候选只能诊断留存。
...
- `P9B.3-T05`：损坏 candidate、decision、page memory 或 registry hash；预期 fail closed 并按已提交边界安全重建/降级，损坏证据被接受数 `0`。
- `P9B.3-T06`：让最后一轮 REJECTED/ROLLED_BACK 后 finalization；预期最终 PDF 只回放最后批准 Patch 或 fallback，失败候选发布数 `0`。

**验收标准：** 已提交 action/翻译不重跑率、合法恢复率和 attempt 证据 hash 验证率均 `100%`；预算重复计算、迟到提交、损坏证据接受和失败候选发布均为 `0`。

### P9B.4 真实样本修复、规则晋级与上游回归

**实现功能：** 用真实分类页面和完整 PDF 证明失败记忆、多轮真实候选、最终降级、规则晋级和既有叶无退化。

**任务描述：** 从 `spikes/page_classification_engine_puncture_v1/分类结果` 为 P9 六叶各选至少一页真实 PDF，并从 `样本/年报` 选择至少两份来源/结构不同的完整 PDF；使用实际 Toolbox/Kernel/render/Judge/Repair 运行，输出不能 mock。可以用可控制的长译文、字号/栏宽等输入制造布局压力。若晋级跨文档规则，必须在两份文档回归后，再用一份未参与规则编写的匿名 holdout 完整 PDF 盲测；未满足证据的经验只留在 PageRepairMemory，不进 Registry。

**设计追溯：** 【总体设计】§9.5.3–§9.5.5、§17–§18、§20.4/§20.6、§22 T4、§24.2–§24.6、§25。

**前置依赖与交付接口：** 输入为 P9B.1–P9B.3、G8/G9 实际叶实现和真实样本、G9A 两份完整文档基线；输出为 real repair runs、每轮 PDF/PNG/JSON、规则 promotion record、holdout evidence、G8/G9 regression diff 和 G9B conclusion，供 P10/P12/P14。

**实施步骤：**

1. 登记 P9 六叶真实页、至少两份完整 PDF 和独立 holdout 的相对路径、hash、Route、选择/隔离规则。
2. 对自然失败或输入压力页实际运行 candidate-0、有限修复、Judge、Checkpoint 和 finalization，保存每轮可打开 PDF/PNG/JSON。
3. 汇总可泛化 failure pattern；只有跨两文档通过且 holdout 不退化的结构化规则才提交 registry diff、版本/hash 和评审证据。
4. 运行完整 PDF、重启恢复、G8/G9 mixed regression 和反过拟合扫描，确认失败仍有最终 PDF，P10 可消费统一 RepairCoordinator。

**测试用例：**

- `P9B.4-T01`：P9 六叶各输入至少一页真实分类 PDF 并执行实际候选/Judge；预期每页 candidate-0 和 PageRepairMemory 均可校验，mock result 数 `0`。
- `P9B.4-T02`：对至少六个真实页施加可控制布局压力并运行多轮；预期每个实际 attempt 都有可打开候选和 JSON 证据，失败页仍有 fallback final。
...
- `P9B.4-T05`：候选规则先跑两份文档，再跑隔离 holdout；预期三者硬约束/目标指标均通过才允许 registry 版本增加，否则晋级数 `0`。
- `P9B.4-T06`：重跑 G8/G9 mixed PDF 与 identity/geometry/text 扰动；预期 Route/Toolbox/Patch/PageOutcome 无非预期差异，样本身份/历史内容分支命中 `0`。

**验收标准：** P9 六叶真实页、两份完整真实 PDF 和一份隔离 holdout 均有实际执行证据；每轮候选/页级记忆/最终 PDF 完成率 `100%`；无证据规则晋级、mock result、重复动作/翻译、身份特判和 G8/G9 退化均为 `0`。

### Gate G9B

**验收目标：** 页级失败经验可跨轮次/重启保留，有限多轮重排真实产出并收敛，跨文档经验只以受控规则发布。

| # | 验收项 | 量化标准 | 设计/任务追溯 | 测试证据 |
|---|---|---|---|---|
| G9B-1 | 页级合同 | effective-layout/round-trip/action/state/identity 验证 `100%`；非法状态 `0` | §9.5.2；P9B.1 | P9B.1-T01～T03、T06 |
| G9B-2 | Registry 边界 | 身份/历史内容/跨文档固定坐标/运行时写入 `0` | §9.5.3；P9B.1 | P9B.1-T04～T05 |
| G9B-3 | 多轮候选 | 每个实际轮次候选/证据和最终结果 `100%`；重复 action/state 循环 `0` | §9.5.4、§15；P9B.2 | P9B.2-T01～T05 |
...
| G9B-6 | Checkpoint/恢复 | 已提交 action/翻译不重跑、合法恢复/hash 验证 `100%`；迟到提交/失败候选发布 `0` | §13、§18.3；P9B.3 | P9B.3-T01～T06 |
| G9B-7 | 真实样本 | P9 六叶真实页、两份完整 PDF 均实际执行；每轮候选和最终 PDF `100%`；mock result `0` | §24.3–§24.4；P9B.4 | P9B.4-T01～T04 |
| G9B-8 | 规则晋级/回归 | 两文档 + 一 holdout 全通过才晋级；无证据晋级、身份分支、G8/G9 退化 `0` | §24.5；P9B.4 | P9B.4-T05～T06 |
| G9B-9 | 页级差异/无二次重排 | 同一 memory hash 下不同页可采用不同有效布局且全局写入 `0`；全书二次样式/布局/Judge 调用 `0`；发布硬校验 `100%` | §9.5.1/§9.5.4、§12.2–§12.3；P9B.2 | P9B.2-T09 |

**验收方法：** 运行 Schema/状态单测、真实候选多轮、循环/无改善/硬回归、同一文档 hash 下的差异页面、TranslationPort 与 Finalizer 调用计数、崩溃窗口、真实六叶/完整 PDF、holdout 晋级和 G8/G9 回归；所有结论引用实际命令输出和 Artifact hash。

**通过标准：** G9B-1～G9B-9 全部 PASS；“只写错误日志但不产出候选”、重复同一失败动作、运行时自动学习、页级优化回写全局、全书二次样式重排、跳过发布硬校验、mock 输出或失败候选进入 final 任一出现，G9B 即 FAIL。
...
| 依赖 | G9B |
| 关键产物 | visual_anchored、五类 composite 的独立证据、全叶 Catalog、全路由 fallback 回归 |
...
**前置依赖与交付接口：** 输入为 G9B Repair Memory/多轮重排闭环、P8–P10 每个叶的签名 Gate/evidence、G7 Catalog schema；输出为 frozen V1 Catalog/resource hash、route-capability matrix、startup report、mixed-route E2E baseline，供 P11/P14。
...
- `P10.7-T07`：重跑 G8/G9 叶基线及 G9A/G9B memory/repair 回归；预期 Route/Toolbox/Patch/PageOutcome、文档记忆和已提交 RepairAttempt 无解释差异。
...
| G10-8 | 文档/上游回归 | 全页终态、页序、完整产物 `100%`；G8/G9/G9A/G9B 差异 `0` | §24.3；P10.7 | P10.7-T05、P10.7-T07、G8/G9/G9A/G9B 回归 |
...
| 项 | 内容 |
|---|---|
| 阶段目标 | 以第一部分 production wiring 完成全量回归、完整年报 E2E、交付审计，并强制复核设计与第二部分计划 |
| 设计追溯 | §22 T7、§24、§25、§26、§27 |
| 依赖 | G13 |
| 关键产物 | 独立闭环验收报告、完整 E2E Artifact、能力矩阵、风险/阈值清单、总体设计与计划复核结论 |

### P14.1 冻结验收基线

**实现功能：** 生成不可变的第一部分验收基线清单和能力矩阵，锁定 G14 所用代码、配置、合同与阈值。

**任务描述：** 汇总 G0–G9、G9A、G9B、G10–G13 Gate 证据，冻结 commit、依赖锁、Schema、Catalog、Document/Page Repair Memory、RepairRuleRegistry、字体、配置、fixture 和测试命令 hash；记录每个分类叶 enabled/disabled/evidence、Freeform 自然/注入样本边界和所有降级能力。最终验收期间不得依据结果临时改阈值，发现问题必须回退相应上游阶段重新过 Gate。

**设计追溯：** 【总体设计】§20.4、§22 T7、§24.2、§24.5、§24.6、§26；本计划 §2.2、§2.4、§5.5。

**前置依赖与交付接口：** 输入为 G0–G9、G9A、G9B、G10–G13 的 GateEvidence、P13 production wheel 和阈值注册表；输出为 `g14_baseline_manifest`、capability/memory matrix、冻结命令/环境和变更阻断规则，供 P14.2–P14.5。

**实施步骤：**

1. 校验 G0–G9、G9A、G9B、G10–G13 每个验收项都有测试证据、Artifact hash、环境和结论，不能只写人工 PASS。
2. 生成代码/lock/schema/catalog/document-memory/page-memory/repair-registry/font/config/fixture/threshold 的不可变 hash 清单。
3. 建立分类叶和 fallback 能力矩阵，区分 enabled、disabled、missing tool、natural evidence、synthetic-only。
4. 启用 baseline drift 检查；任何冻结项变化自动使 G14 失效并指出需重跑的上游 Gate。

**测试用例：**

- `P14.1-T01`：加载 G0–G9、G9A、G9B、G10–G13 全部 GateEvidence；执行完整性校验；预期阶段和验收项缺失数 `0`，结论均为 PASS。
- `P14.1-T02`：对冻结代码、lock、Schema、Catalog、Document/Page Repair Memory、RepairRuleRegistry、字体、配置、fixture、threshold 重算 hash；预期与 baseline manifest 一致率 `100%`。
...
**验收标准：** G0–G9、G9A、G9B、G10–G13 证据齐全且 PASS；冻结 hash 一致率和 Route/Repair Memory 能力矩阵覆盖率均 `100%`；baseline drift 未重验情况下进入后续 G14 测试数 `0`。

### P14.2 全分类与降级矩阵

**实现功能：** 对设计路由表中的所有已知叶与全部 fallback 条件执行生产 wiring 矩阵测试。

**任务描述：** 逐条覆盖 §8.2 已知叶、body.freeform、Unclassified、未注册、disabled、初始化失败、Toolbox 执行失败和 Preservation 失败；每条路径必须确定性进入正常翻译排版、区域/原子回退或页面透传，并最终参与完整 PDF 合并。可读 source 的 run 不得因能力缺失而无结果。

**设计追溯：** 【总体设计】§8.2–§8.3、§11、§12、§20.4、§24.1–§24.5、§24.6。

**前置依赖与交付接口：** 输入为 P14.1 冻结 capability/memory matrix、G8/G9/G9A/G9B/G10/G11 gold/fixture、G12/G13 production Runtime；输出为 route-degradation execution matrix、逐项 PageOutcome 和完整 PDF 证据，供 P14.3/G14。
...
**验收标准：** Route × capability state 矩阵覆盖率 `100%`；无等待人工或无终态路径；可读 source 的完整 PDF 产物率 `100%`；PROCESS_FAILED 误用数 `0`。

### P14.3 文档级最终 E2E

**实现功能：** 以未经拆页的完整 PDF 为唯一业务输入执行最终端到端回归，验证自动分页、全页终态屏障与单一完整 PDF 发布。

**任务描述：** 至少选取一份 `样本/年报` 中完整多页 PDF；服务自行枚举全部页并完成 facts、分类、路由、全页屏障后冻结 DocumentLayoutMemory、翻译、页级有效布局、Toolbox/Freeform、PageRepairMemory、多轮重排/降级、Patch 回放、页最终化和整本合并。覆盖混合分类、结构不同页面采用不同有效布局、最后一页失败、能力缺失、Preservation 特性、暂停重启、应急整本透传和 300 页背压。不得以预拆单页列表替代完整 PDF 输入；全页终态后不得执行全书二次样式归一/重排/Judge，但 Preservation 和 ArtifactIntegrity 硬校验不得省略。

**设计追溯：** 【总体设计】§5.5、§7.2–§7.5、§12.2–§12.3、§13.5、§18.2、§22 T7、§24.3–§24.6；本计划 P3.1、附录 C。

**前置依赖与交付接口：** 输入为 P14.1 冻结 baseline、P14.2 全矩阵和 production wheel；输出为 final E2E run manifests、完整 source/target PDF、逐页证据、reproducibility diff 和容量/恢复报告，供 G14 人工抽查。
...
4. 固定 Provider 重复运行，比较 Route、DocumentLayoutMemory、TranslationUnit、PageRepairMemory、Patch manifest、PageOutcome 和 degradation summary；PDF 只忽略 Preservation Contract 允许的非语义元数据。
...
- `P14.3-T09`：在真实页 candidate-0 失败后执行至少两轮 Repair，并在第一轮提交后重启；预期文档记忆 hash 不变、失败经验保留、已提交 action/翻译不重复、每轮候选可打开且 final 只使用批准 Patch/fallback。
...
**验收标准：** `P14.3-T01`～`T10` 全部通过；完整输入自动全页处理率、最终 PDF 产物率、页数/页序保持率、发布硬校验执行率和 Repair evidence 完整率均 `100%`；固定条件结构化结果差异、文档记忆运行中写入、全书二次样式/布局/Judge 调用和重复已提交 action/翻译均为 `0`。

### P14.4 架构和交付审计

**实现功能：** 完成生产包、依赖方向、运行轨迹、PDF 发布链和独立运维资料审计。

**任务描述：** production wheel、import graph 和真实 E2E trace 不得包含 spikes、MerqFin 源码、HTML/Chrome 渲染、LiteLLM/Provider SDK/模型密钥或通用 Agent 框架；最终 PDF 必须来自 PyMuPDF/PdfKernel 合成链。同步审计三层 Repair Memory 边界、Registry 运行时只读、Artifact 原子发布、Checkpoint 恢复、日志/阈值证据和部署说明。
...
5. 扫描 RepairRuleRegistry、Prompt 和运行写入轨迹，验证历史 PDF 内容/身份未注入、生产 Registry 无写路径。
...
- `P14.4-T06`：扫描 Registry/Prompt/runtime write trace；预期历史 PDF 原文/候选/sample_id/固定页码/跨文档绝对坐标和生产 Registry 写入命中均为 `0`。

**验收标准：** 所有架构禁止项、未知 renderer、历史 PDF Prompt 注入和 Registry 运行时写入命中均为 `0`；抽样证据验证率 `100%`；Runbook 独立复现通过率 `100%`；生产包和文档不泄密。

### P14.5 阶段间强制复盘

**实现功能：** 建立第一、第二部分之间不可绕过的人工复盘和变更影响闭环。

**任务描述：** 向项目负责人提交独立闭环事实，包括正常能力、真实退化、disabled 叶、Freeform 自然/注入效果、性能、资源和工程风险；逐条核对当前批准总体设计版本的假设。任何边界、Schema、AI 合同、迁移顺序变化都必须先形成设计/计划版本化差异，经负责人批准后生成新 baseline hash 并重跑受影响 Gate；不得原地覆盖 P0–P14 的历史验收输入。只有负责人明确选择“进入 P15”才可继续，不能把技术 PASS 等同于生产变更授权。

**设计追溯：** 【总体设计】§22 T7、§23 M0、§25–§27；本计划 §2.3、§5.5、“第一部分结束检查点”。

**前置依赖与交付接口：** 输入为 P14.1–P14.4 全部报告和 G0–G9、G9A、G9B、G10–G13 证据；输出为 review record、decision log、变更影响矩阵和负责人结论（进入 P15/修订后重验/暂停），是 P15 的强制输入。
...
- `P14.5-T01`：审计复盘材料；预期 enabled/disabled/degradation/Freeform evidence/性能/风险字段完整率 `100%`，与 GateEvidence 一致。
...
| # | 验收项 | 量化标准 | 设计/任务追溯 | 测试证据 |
|---|---|---|---|---|
| G14-1 | 上游 Gate | G0–G9、G9A、G9B、G10–G13 记录齐全且 PASS；缺失/伪证据 `0` | §24.6；P14.1 | P14.1-T01 |
...
| G14-11 | 强制复盘 | 报告/决策记录齐全；未关闭 `[待决策]` `0` | §25–§27；P14.5 | P14.5-T01～T02 |
| G14-12 | 人工授权 | 负责人结论为“进入 P15”且绑定 baseline hash；否则 P15 启动 `0` | §23 M0；P14.5 | P14.5-T03～T04 |
| G14-13 | Repair Memory 闭环 | 文档记忆只读同 hash、已提交失败经验保留、每轮候选 `100%`；重复 action/翻译和失败候选发布 `0` | §9.5、§15、§18；P14.3 | P14.3-T09 |

**验收方法：** 执行完整 `G14` 编排、人工抽查 Artifact/预览/PDF、架构审计和复盘会议记录。

**通过标准：** 技术清单全部通过且负责人批准进入第二部分；否则停在 P14，保留原基线，按复盘结论形成经批准的新设计/计划版本并重过受影响 Gate。
...
**任务描述：** 汇总 G0–G23（含 G9A/G9B）、代码/lock/DB Schema/OpenAPI/Prompt profile/字体/Catalog/config/threshold hash、部署/运维/回滚/事故 Runbook 和生产观察报告；验证 MerqFin 保持产品/API/审校/下载与非 PDF Worker，Transflow 唯一调度 PDF/PyMuPDF 引擎，AI Service 只提供两个能力 Port。两个仓库无源码交叉 import、无共享 mutable package/venv。旧 PDF 代码删除必须作为独立项目列前置条件和回滚期，不在本阶段执行。
...
- `P23.4-T01`：校验 G0–G23（含 G9A/G9B）archive；预期每个 Gate 有输入 baseline、测试结果、Artifact/evidence hash、结论和 approver，缺项 `0`。
...
| G23-10 | 交付追溯 | G0–G23（含 G9A/G9B）证据完整、双向追溯和生产 hash 一致率均 `100%` | §26–§28；P23.4 | P23.4-T01～T02、P23.4-T05 |
...
| P9 | G8 第一批 | 第二批叶 | 同上 |
| P9A | G9 普通叶 | 不可变 DocumentLayoutMemory、完整文档布局画像 | PageFacts/Classification/Margin/FilesystemCheckpoint |
| P9B | G9A 文档记忆 | PageRepairMemory、多轮重排、只读 RepairRuleRegistry | Toolbox/Kernel/Artifact/Checkpoint |
...
| P9A | §4、§7、§9.1/§9.5.1/§9.5.5、§13.4、§17–§19、§24 | 文档级布局事实、冻结、恢复、真实 PDF |
| P9B | §7、§9.5.2–§9.5.5、§12–§15、§17–§18、§24–§27 | 页级失败记忆、多轮候选、规则晋级、恢复 |
...
| TF-E2E-15 | 页级多轮失败、差异布局与最终降级 | P9B | P14 | P9B.2-T09、P9B.4-T02～T04、P14.3-T09～T10 | 失败经验保留、页级差异不回写全局、无二次全书重排、每轮有候选且最终 PDF 完整 |
...
| DocumentLayoutMemory Schema/identity/失效字段 | P9A | 全页屏障、翻译前源基线/目标策略、确定性、线程/进程并发和恢复证据 | `[待决策]` 并暂停 G9A |
| PageRepairMemory/RepairRuleRegistry Schema 与晋级规则 | P9B | 多轮真实候选、两文档回归和 holdout 盲测 | 保持规则不晋级并暂停 G9B |
...
- 固定 Provider 下结果可复现，证据/降级诚实。
- 文档级布局记忆在全页屏障后、翻译前冻结，线程/进程并发页共享同一 hash 且跨页无串扰；页级有效布局允许合理差异，页级失败经验可恢复，每轮真实产出候选，跨文档规则无运行时自动学习；全页终态后无二次全书样式重排，发布硬校验完整执行。
