...
> 文档状态：Draft，已补入 P9C 迁移纠偏基线、P9A/P9B Repair Memory 及产品验收边界
> 编制日期：2026-07-20
...
当前没有阻塞 P9C 施工的 `[待决策]`。凡【总体设计】§27 已明确要求“实施阶段冻结”的细项，已被安排到对应阶段，不在本计划中静默假定答案；ProductAcceptance 的分层人工质量阈值由 P14.1 基于 P9C–P13 真实证据冻结，在此之前只能记 `NOT_EVALUATED`，不得静默记 PASS。
...
1. **第一部分 P0–P14：Transflow 独立闭环。** 从生产目录和环境开始，依次完成合同、Standalone Adapter、完整 PDF 纵向切片、分类迁移、PdfKernel、逐叶工具箱、P9C 迁移纠偏、P9A/P9B Repair Memory、Freeform、暂停恢复、并发与故障注入，最后在固定翻译 Provider 下完成整本 PDF 闭环。P9C/P9A/P9B 是插入 P9 与 P10 之间的补充阶段，不重编号既有下游阶段；硬顺序为 `P9 -> P9C -> P9A -> P9B -> P10`。
...
- 尚未完成阶段暴露的上游缺陷必须回到上游修复并重过 Gate，不在下游加旁路补丁。对已经归档的 P5–P9，后续发现的横切合同遗漏由经批准的 P9C 纠偏阶段前向承接：历史代码、报告和 Gate 不回写、不伪造重验；P9C 必须登记历史基线、问题证据、合同优先级、受影响回归和新 Gate，从 G9C 起约束后续阶段。
...
- 测试 ID 格式固定为 `P<阶段>.<子任务>-T<两位序号>`，例如 `P3.1-T01`；字母补充阶段使用 `P9C.1-T01`、`P9A.1-T01`、`P9B.1-T01`。端到端用例继续使用附录 C 的 `TF-E2E-*`、`MF-E2E-*`。
...
| P9A | 文档级布局记忆 | 全页屏障、源布局基线、翻译前目标策略、并发只读快照、失效与恢复 | G9C | G9A | T4 Repair Memory A |
...
**前置依赖与交付接口：** 输入为 G2 Domain/Schema 规范、G5 ClassificationRoute、G6 PageFacts/Preservation、G7 Catalog、G9 第二批叶事实和 G9C Route/owner/corrective contract；输出为 `document_layout_memory.v1` Schema、领域对象、canonical serializer/hash 和失效判定，供 P9A.2–P9A.4/P9B/P10。
...
**前置依赖与交付接口：** 输入为 P9A.1 合同、G6 PageFacts/Kernel、G5 全页 Route、P7 Margin、G8/G9 叶级 facts 和 G9C 路由错配/owner 规则；输出为 `DocumentLayoutMemoryBuilder`、完整 source page baseline/role profile/shared region map 与 target layout policy，供 P9A.3/P9B Repair 和所有后续 Toolbox。
...
**前置依赖与交付接口：** 输入为 P9A.1–P9A.3、G8/G9/G9C 真实叶/纠偏样本和现有 regression harness；输出为 real-document memory manifests、覆盖矩阵、determinism/anti-overfit diff 和 G9A evidence，供 P9B/P10/P14。
...
- `P9A.4-T06`：重跑 G8/G9/G9C 回归；预期 Route/Toolbox/Patch/PageOutcome、SemanticUnitMap/诊断隔离解释差异和第一/二批退化均为 `0`。
...
| G9A-6 | 真实证据/回归 | 至少两份完整真实 PDF；Artifact/hash `100%`；G8/G9/G9C 退化 `0` | §24.3–§24.5；P9A.4 | P9A.4-T01～T06 |
...
| 项 | 内容 |
|---|---|
| 阶段目标 | 让每页在同一只读文档记忆下形成可不同的 `PageEffectiveLayout` 并携带同 run 可恢复、同源新 run 严格匹配后可只读复用的失败经验，执行有限多轮重排；每轮实际产出候选，最终接受安全 Patch 或明确降级，不做全书二次样式归一 |
| 设计追溯 | §7.1–§7.4、§9.1/§9.5–§9.6、§12–§15、§17–§19、§22 T4、§24–§27 |
| 依赖 | G9A |
| 关键产物 | PageRepairMemory/RepairAttempt/PriorRepairEvidenceRef Schema、Rule IR/RepairRuleRegistry、RepairCoordinator、每轮 Artifact/Checkpoint、真实样本多轮证据、G9B 结论 |

P9B 不重新设计叶级布局算法。它统一“记住失败、选择未试动作、重排、产出候选、比较、接受/回滚、停止”的协调闭环；具体 Finding 和 Repair 原子仍归各 PageToolbox。P9C 已冻结 SemanticUnitMap、完整性、诊断候选和路由错配合同，P9B 只能消费，不能用 Repair 绕过它们。

### P9B.1 页级记忆与跨文档规则合同

**实现功能：** 定义 `PageEffectiveLayout`、`PageRepairMemory`、`RepairAttempt`、`PriorRepairEvidenceRef`、`RepairProposal`、`RepairBudget` 和只读 Rule IR/`RepairRuleRegistry` 的领域/Schema/版本合同。

**任务描述：** 页级记忆绑定 source/page/Route/Toolbox/config/document-memory/registry hash，保存初始 `PageEffectiveLayout`、已尝试 action、已见 state、每轮 TranslationBundle hash/页级有效布局差异、前后质量、候选证据、接受/回滚和终止原因。新 run 不续写旧页记忆；只有 source/page/Route/Toolbox/config/document-memory/registry/TranslationBundle/Schema/实现指纹全部精确一致时，才生成只读 PriorRepairEvidenceRef，把旧终态失败 action/state 用作排除集。Registry 使用 `rule_id/salience/required_facts/mechanical_when/optional llm_question/allowed_verdicts/failure_class/repair_atom/bound_tool/acceptance_gates/evidence` Rule IR；事实不足返回 NEED_EVIDENCE，生产运行不能写 Registry。

**设计追溯：** 【总体设计】§4、§9.5.2–§9.5.5、§9.6、§13.2/§13.4、§15、§17–§18、§24.1/§24.5。

**前置依赖与交付接口：** 输入为 G9A immutable document memory、G9C completeness/diagnostic/route contract、G2 Domain/Schema、G7 Toolbox Repair 合同和 G3 Checkpoint；输出为 effective-layout/page-memory/attempt/prior-ref/proposal/budget/Rule IR Schema、canonical effective-layout/action/state hash 和状态转换校验，供 P9B.2–P9B.4/P10/P12。
...
3. 建立 Rule IR Schema、只读加载/hash、required-facts/NEED_EVIDENCE、mechanical/LLM 枚举边界、确定性冲突顺序和 evidence/promotion 字段。
4. 建立 PriorRepairEvidenceRef 完整指纹匹配和只读 action/state 排除集；任一指纹不匹配时只保留审计引用。
5. 增加边界扫描：Registry/PriorRef 不含身份特判、历史原文/候选/绝对坐标，运行时写入和无证据规则均拒绝。
...
- `P9B.1-T07`：构造机械规则、事实不足规则、需一次模型枚举规则和同 salience 冲突规则；预期分别机械裁决、NEED_EVIDENCE、仅返回 allow-list verdict，并按硬约束/salience/rule_id 稳定解析；模型生成坐标/参数/工具被拒绝。
- `P9B.1-T08`：以旧终态 page memory 创建新 run；先保持全部指纹和 TranslationBundle hash 相同，再逐项改变任一指纹；预期前者只读导入失败 action/state，后者导入数 `0` 且旧证据仍可审计。

**验收标准：** 合同 round-trip、action/state 确定性、Rule IR 裁决和陈旧拒绝率均 `100%`；非法转换、事实不足猜测、模型越权、指纹不匹配复用、身份/历史内容、跨文档固定坐标和运行时 Registry 写入均为 `0`。

### P9B.2 RepairCoordinator 多轮重排

**实现功能：** 实现页级有效布局解析、首轮候选登记、未尝试动作选择、源页重排、真实候选生成、前后裁决、接受/回滚和确定性 fallback。

**任务描述：** 每页固定读取同一 `document_memory_hash`，先以当前页硬事实/Route 为直接依据，以文档角色画像和 `target_layout_policy` 为软先验，再结合通过 G9C 完整性门禁的 TranslationBundle、PageRepairMemory/PriorRepairEvidenceRef 解析 `PageEffectiveLayout`。不同结构页面允许选择不同字号、行距、段距、换行和修复参数；这些决策及变化只记入本页候选/attempt，不回写文档记忆。默认 `max_repair_rounds=3`、`max_no_improvement=2` 并进入 config snapshot。每个实际轮次只执行一个主要动作，先按 Rule IR/叶私有 Repair 的确定性顺序裁决，再从不可变源页和当前最佳完整计划重新生成候选；布局修复必须复用同一有效 TranslationBundle。当前页内部段落、表格组或 FlowBand 的最终回流允许作为该轮候选生成步骤，但必须在本页 Judge/FINALIZED 前完成。每轮无论成功失败都保存可打开候选；目标 Finding 改善且无硬回归才接受。重复 action/state、连续无改善、无适用动作、预算耗尽或硬失败必须停止并降级；完整译文下的失败按 G9C 保留诊断候选。页面终态后不执行全书二次样式归一、布局重排或质量重判；Document Finalizer 仍执行全部发布硬校验。

**设计追溯：** 【总体设计】§7.2–§7.4、§9.5.2–§9.6.3、§12、§13.2、§15.1–§15.2、§18.2。

**前置依赖与交付接口：** 输入为 P9B.1 合同、G9A 文档记忆、G9C 完整性/诊断/路由错配合同、G7 Toolbox/Judge/Repair、G6 Kernel render/constraints 和 G3 Translation/Artifact；输出为 `RepairCoordinator`、候选比较/接受/回滚、终止/fallback 和结构化日志，供 P9B.3/P9B.4/P10。

**实施步骤：**

1. 按“文档软先验 + 当前页硬事实 + PASS TranslationBundle”解析并登记 candidate-0 的 `PageEffectiveLayout`、Finding、质量向量和初始 state，解析 Rule IR 与叶私有 Repair 原子；Route 能力错配直接交回 G9C fallback，不进入布局死循环。
2. 合并当前页与精确匹配 PriorRef 的 attempted/seen 集，先校验 required facts，再按硬约束/salience/rule_id 过滤，只选一个动作并生成 RepairProposal。
3. 从源页、同一 TranslationBundle 和更新后的完整 layout plan 实际生成 candidate/preview/Patch；需要的页内 group/FlowBand 回流在本轮完成，再用同一 Judge 比较。
...
- `P9B.2-T10`：让 Rule IR 分别遇到事实齐全、事实不足、机械规则冲突和一次有界模型问题；预期动作选择确定，NEED_EVIDENCE 不执行 Repair，模型输出仅允许 verdict 枚举，坐标/参数/工具越权数 `0`。
...
**验收标准：** 每个实际轮次真实候选/证据产出率、接受/回滚/Rule IR 可追溯率和最终 PageOutcome/PDF 完成率均 `100%`；同一文档 hash 下页级差异和页内最终回流可追溯且不回写全局；重复已提交/历史精确匹配失败动作、状态死循环、事实不足猜测、模型越权、布局失败重复翻译、硬回归发布和 FINALIZED 后全书二次样式/布局/Judge 调用均为 `0`，发布硬校验跳过数为 `0`。

### P9B.3 Repair Checkpoint、崩溃恢复与 Artifact

**实现功能：** 将每轮 proposal/candidate/Patch/Finding/Decision/page memory 作为确定性、内容寻址、可恢复安全点提交。

**任务描述：** 本轮所有 Artifact（含 `PageEffectiveLayout`/页级差异、TranslationBundle/完整性 hash 和诊断状态）校验并原子 rename 后，Checkpoint 才引用新版 PageRepairMemory。Artifact 已写但 Checkpoint 未提交时，恢复器按 action key/hash 复用同一轮并补交，不增加预算、不重复翻译；Checkpoint 已提交后跳过该 action/state。新 run 的 PriorRepairEvidenceRef 作为独立只读引用提交，不与当前 append-only 记忆合并。旧 Worker 迟到提交、损坏/陈旧证据、“失败候选直接成为 final”或诊断候选进入 final 必须被拒绝。
...
5. 提交/恢复 PriorRepairEvidenceRef 的来源 run、完整指纹比较和排除 action/state，不复制旧候选/译文。
...
- `P9B.3-T07`：完成一个终态失败 run 后，以相同 source/page/Route/Toolbox/config/document-memory/registry/TranslationBundle 新建 run，再逐项改变指纹；预期精确匹配 run 跳过旧失败 action/state，变化 run 不受旧排除集影响，旧候选/译文导入数 `0`。

**验收标准：** 已提交 action/翻译不重跑率、合法恢复率、精确匹配 PriorRef 排除率和 attempt 证据 hash 验证率均 `100%`；预算重复计算、指纹不匹配复用、迟到提交、损坏证据接受、旧候选/译文导入和失败/诊断候选发布均为 `0`。

### P9B.4 真实样本修复、规则晋级与上游回归

**实现功能：** 用真实分类页面和完整 PDF 证明失败记忆、多轮真实候选、最终降级、规则晋级和既有叶无退化。

**任务描述：** 从 `spikes/page_classification_engine_puncture_v1/分类结果` 为 P9 六叶各选至少一页真实 PDF，并从 `样本/年报` 选择至少两份来源/结构不同的完整 PDF；使用实际 Toolbox/Kernel/render/Judge/Repair 运行，输出不能 mock，至少一组用迁移测试 Qwen Adapter 的真实译文，其余可用可控制长译文制造布局压力。完整译文下失败必须沿用 G9C 诊断候选，Route 错配不得进入无效 Repair。若晋级跨文档规则，必须在两份文档回归后，再用一份未参与规则编写的匿名 holdout 完整 PDF 盲测；未满足证据的经验只留在 PageRepairMemory，不进 Registry。

**设计追溯：** 【总体设计】§9.5.3–§9.6.3、§17–§18、§20.4/§20.6、§22 T4、§24.2–§24.6、§25。

**前置依赖与交付接口：** 输入为 P9B.1–P9B.3、G8/G9 实际叶实现和真实样本、G9C semantic/diagnostic/route baseline、G9A 两份完整文档基线；输出为 real repair runs、每轮 PDF/PNG/JSON、规则 promotion record、holdout evidence、G8/G9/G9C regression diff 和 G9B conclusion，供 P10/P12/P14。
...
- `P9B.4-T08`：选一个失败页完成 run 后用同一 bundle 重开，再用改变后的 bundle 重开；预期同一 bundle 不重复旧失败 action/state，改变 bundle 重新按当前事实裁决，两个 run 均产出可审计终态。

**验收标准：** P9 六叶真实页、两份完整真实 PDF 和一份隔离 holdout 均有实际执行证据；每轮候选/页级记忆/最终 PDF/三类结论完成率 `100%`；无证据规则晋级、mock result、重复动作/翻译、错路由重复 Repair、诊断发布、身份特判和 G8/G9/G9C 退化均为 `0`。
...
| G9B-2 | Rule IR/Registry 边界 | Rule IR 裁决 `100%`；事实不足猜测、模型越权、身份/历史内容/跨文档固定坐标/运行时写入 `0` | §9.5.3；P9B.1 | P9B.1-T04～T05、T07 |
| G9B-3 | 多轮候选 | 每个实际轮次候选/证据和最终结果 `100%`；重复 action/state 循环、错路由 Repair `0` | §9.5.4、§15；P9B.2 | P9B.2-T01～T05、T10 |
...
| G9B-8 | 规则晋级/回归 | 两文档 + 一 holdout 全通过才晋级；无证据晋级、身份分支、G8/G9/G9C 退化 `0` | §24.5；P9B.4 | P9B.4-T05～T07 |
...
| G9B-10 | 同源新 run 失败经验 | 完整指纹一致时旧失败 action/state 排除 `100%`；任一指纹变化复用、旧候选/译文导入 `0` | §9.5.5、§13.4；P9B.1/P9B.3/P9B.4 | P9B.1-T08、P9B.3-T07、P9B.4-T08 |
| G9B-11 | G9C 结果边界 | 完整译文失败有隔离诊断、错路由安全 fallback、三类结论 `100%`；诊断发布/产品 PASS 冒充 `0` | §9.6、§12、§18；P9B.4 | P9B.4-T07 |

**验收方法：** 运行 Schema/状态单测、真实候选多轮、循环/无改善/硬回归、同一文档 hash 下的差异页面、Rule IR/PriorRef、TranslationPort 与 Finalizer 调用计数、崩溃窗口、真实六叶/完整 PDF、holdout 晋级和 G8/G9/G9C 回归；所有结论引用实际命令输出和 Artifact hash。

**通过标准：** G9B-1～G9B-11 全部 PASS；“只写错误日志但不产出候选”、重复同一失败动作、指纹不匹配仍复用历史、Rule IR 猜测/模型越权、运行时自动学习、页级优化回写全局、FINALIZED 后全书二次样式重排、跳过发布硬校验、mock 输出或失败/诊断候选进入 final 任一出现，G9B 即 FAIL。
...
- `P10.7-T07`：重跑 G8/G9 叶基线及 G9C/G9A/G9B semantic/diagnostic/memory/repair 回归；预期 Route/Toolbox/SemanticUnitMap/diagnostic isolation/Patch/PageOutcome、文档记忆和已提交 RepairAttempt 无解释差异。
...
| G10-8 | 文档/上游回归 | 全页终态、页序、完整产物 `100%`；G8/G9/G9C/G9A/G9B 差异 `0` | §24.3；P10.7 | P10.7-T05、P10.7-T07、G8/G9/G9C/G9A/G9B 回归 |
...
**任务描述：** 汇总 G0–G9、G9C、G9A、G9B、G10–G13 Gate 证据，冻结 commit、依赖锁、Schema、Catalog、Semantic/Completeness/Diagnostic 合同、Document/Page Repair Memory、Rule IR/RepairRuleRegistry、字体、配置、fixture 和测试命令 hash；记录每个分类叶 enabled/disabled/evidence、Freeform 自然/注入样本边界和所有降级能力。以 G9C–G13 真实证据冻结 ProductAcceptance 阈值：硬条件至少包括语义单元与 PDF 物化/可提取覆盖 `100%`、漏单元 `0`、Quality PASS、无区域/页面源文透传掩盖译文；人工质量分层必须给出样本、公式、阈值和批准人。最终验收期间不得依据结果临时改阈值，无法冻结则 G14 为 BLOCKED_BY_DECISION。

**设计追溯：** 【总体设计】§20.4、§22 T7、§24.2、§24.5、§24.6、§26；本计划 §2.2、§2.4、§5.5。

**前置依赖与交付接口：** 输入为 G0–G9、G9C、G9A、G9B、G10–G13 的 GateEvidence、P13 production wheel 和阈值注册表；输出为 `g14_baseline_manifest`、capability/semantic/memory/three-axis matrix、ProductAcceptance 阈值、冻结命令/环境和变更阻断规则，供 P14.2–P14.5。

**实施步骤：**

1. 校验 G0–G9、G9C、G9A、G9B、G10–G13 每个验收项都有测试证据、Artifact hash、环境和结论，不能只写人工 PASS。
2. 生成代码/lock/schema/catalog/semantic/completeness/diagnostic/document-memory/page-memory/rule-registry/font/config/fixture/threshold 的不可变 hash 清单。
...
- `P14.1-T01`：加载 G0–G9、G9C、G9A、G9B、G10–G13 全部 GateEvidence；执行完整性校验；预期阶段和验收项缺失数 `0`，结论均为 PASS。
- `P14.1-T02`：对冻结代码、lock、Schema、Catalog、Semantic/Completeness/Diagnostic、Document/Page Repair Memory、Rule IR/RepairRuleRegistry、字体、配置、fixture、threshold 重算 hash；预期与 baseline manifest 一致率 `100%`。
...
- `P14.1-T05`：从 G9C–G13 冻结样本重算 ProductAcceptance 硬条件、人工质量公式/阈值和三类聚合真值表；预期阈值均有事前证据/批准人，源 PDF final 或 diagnostic candidate 自动推导产品 PASS 的路径数 `0`。

**验收标准：** G0–G9、G9C、G9A、G9B、G10–G13 证据齐全且 PASS；冻结 hash 一致率、Route/semantic/Repair Memory 能力矩阵和三类聚合规则覆盖率均 `100%`；阈值无证据/事后调整、源 final/diagnostic 冒充产品 PASS、baseline drift 未重验情况下进入后续 G14 测试均为 `0`。
...
**前置依赖与交付接口：** 输入为 P14.1 冻结 capability/semantic/memory matrix、G8/G9/G9C/G9A/G9B/G10/G11 gold/fixture、G12/G13 production Runtime；输出为 route-degradation execution matrix、逐项 PageOutcome/diagnostic/three-axis result 和完整 PDF 证据，供 P14.3/G14。
...
- `P14.3-T11`：输入一份未经拆页的完整真实 PDF，通过迁移测试 Qwen TranslationPort 执行全链；预期 SemanticUnitMap、有效译文、PDF 实际物化/可提取 unit 覆盖率均 `100%`、漏项 `0`、Quality PASS、无区域/页面源文透传，发布一个 ProductAcceptance PASS 的完整 target PDF；API Key 只来自环境变量，mock/recorded result 数 `0`。
...
**任务描述：** production wheel、import graph 和真实 E2E trace 不得包含 spikes、MerqFin 源码、HTML/Chrome 渲染、LiteLLM/Provider SDK/模型密钥或通用 Agent 框架；最终 PDF 必须来自 PyMuPDF/PdfKernel 合成链。同步审计 G9C semantic/diagnostic/three-axis 边界、三层 Repair Memory、Rule IR/Registry 运行时只读、Artifact 原子发布、Checkpoint 恢复、日志/阈值证据和部署说明。
...
**前置依赖与交付接口：** 输入为 P14.1–P14.4 全部报告和 G0–G9、G9C、G9A、G9B、G10–G13 证据；输出为 review record、three-axis decision log、变更影响矩阵和负责人结论（进入 P15/修订后重验/暂停），是 P15 的强制输入。
...
| # | 验收项 | 量化标准 | 设计/任务追溯 | 测试证据 |
|---|---|---|---|---|
| G14-1 | 上游 Gate | G0–G9、G9C、G9A、G9B、G10–G13 记录齐全且 PASS；缺失/伪证据 `0` | §24.6；P14.1 | P14.1-T01 |
...
| G14-13 | Repair Memory 闭环 | 文档记忆只读同 hash、同 run/精确匹配同源失败经验保留、每轮候选 `100%`；重复 action/翻译、Rule IR 越权和失败候选发布 `0` | §9.5、§15、§18；P14.3 | P14.3-T09～T10、G9B 回归 |
| G14-14 | 真实翻译产品验收 | 至少一份未经拆页真实 PDF：SemanticUnitMap/有效译文/PDF 物化可提取覆盖 `100%`、漏项 `0`、Quality/ProductAcceptance PASS、源文透传 `0` | §9.6、§12、§16.1.1、§24.6；P14.3 | P14.1-T05、P14.3-T11 |
...
**任务描述：** 汇总 G0–G23（含 G9C/G9A/G9B）、代码/lock/DB Schema/OpenAPI/Prompt profile/字体/Catalog/config/threshold hash、部署/运维/回滚/事故 Runbook 和生产观察报告；验证 MerqFin 保持产品/API/审校/下载与非 PDF Worker，Transflow 唯一调度 PDF/PyMuPDF 引擎，AI Service 只提供两个能力 Port。两个仓库无源码交叉 import、无共享 mutable package/venv。旧 PDF 代码删除必须作为独立项目列前置条件和回滚期，不在本阶段执行。
...
- `P23.4-T01`：校验 G0–G23（含 G9C/G9A/G9B）archive；预期每个 Gate 有输入 baseline、测试结果、Artifact/evidence hash、结论和 approver，缺项 `0`。
...
| G23-10 | 交付追溯 | G0–G23（含 G9C/G9A/G9B）证据完整、双向追溯和生产 hash 一致率均 `100%` | §26–§28；P23.4 | P23.4-T01～T02、P23.4-T05 |
...
| P9A | G9C 纠偏合同 | 不可变 DocumentLayoutMemory、完整文档布局画像 | PageFacts/Classification/Margin/FilesystemCheckpoint |
| P9B | G9A 文档记忆 + G9C 纠偏合同 | PageRepairMemory/PriorRef、多轮重排、只读 Rule IR/RepairRuleRegistry | Toolbox/Kernel/Artifact/Checkpoint |
...
| P9B | §7、§9.5.2–§9.5.5、§9.6、§12–§15、§17–§18、§24–§27 | 页级/同源失败记忆、Rule IR、多轮候选、规则晋级、恢复 |
...
| TF-E2E-15 | 页级多轮失败、同源历史排除、差异布局与最终降级 | P9B | P14 | P9B.2-T09～T11、P9B.3-T07、P9B.4-T02～T04/T08、P14.3-T09～T10 | 同 run/精确匹配同源失败经验保留、页级差异不回写全局、页内回流在 Judge 前、无二次全书重排、每轮有候选且最终 PDF 完整 |
| TF-E2E-16 | 翻译完整性、诊断双轨与路由错配 | P9C | P14 | P9C.2-T02～T06、P9C.3-T01～T07、P9C.4-T02～T06、P14.2-T06、P14.3-T12～T13 | SemanticUnitMap 全覆盖；可物化的完整译文失败有隔离诊断，硬物化失败有状态，无完整译文不伪造；错路由安全 fallback；三类结论诚实 |
...
| DocumentLayoutMemory Schema/identity/失效字段 | P9A | G9C 路由/owner 合同、全页屏障、翻译前源基线/目标策略、确定性、线程/进程并发和恢复证据 | `[待决策]` 并暂停 G9A |
| PageRepairMemory/PriorRepairEvidenceRef/Rule IR 与 Registry 晋级 | P9B | 多轮真实候选、完整指纹同源复用、两文档回归和 holdout 盲测 | 保持规则不晋级并暂停 G9B |
...
| ProductAcceptance 分层质量阈值 | P14 | G9C–G13 真实证据；硬条件为语义/物化 100%、漏项 0、Quality PASS、无源文透传掩盖 | `[待决策]` 并暂停 G14；不得以 final/diagnostic 存在代替 |
...
- P5–P9 历史结论不被纠偏阶段回写；SemanticUnitMap/完整性、final/diagnostic 双轨、路由错配和三类结论合同通过。
- 文档级布局记忆在全页屏障后、翻译前冻结，线程/进程并发页共享同一 hash 且跨页无串扰；页级有效布局允许合理差异，同 run/严格匹配同源失败经验可恢复，每轮真实产出候选，Rule IR/跨文档规则无运行时自动学习；页内最终回流在本页 Judge 前完成，全页终态后无二次全书样式重排，发布硬校验完整执行。
