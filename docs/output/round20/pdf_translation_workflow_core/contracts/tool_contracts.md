...
| expandable text slot | expand only declared `expandable_text_slots.region_kinds` such as headings and explanatory short labels; require current-run target/source length ratio, source width ratio, page type, right-side obstacle, and below-same-column obstacle evidence |
...
| constrained text image fit | after normal textbox probing fails, table cells, compact labels, dense-page hard-slot short labels, legends, and dense-page single-line labels may be inserted as text images with full target text preserved; explanatory `short_label` must first try `expandable_text_slots`; evidence must record `status=constrained_text_image_fit`, `font_size`, target box, and `horizontal_compression_ratio` |
