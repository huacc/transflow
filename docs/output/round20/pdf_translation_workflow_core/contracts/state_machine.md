...
For `expandable_text_slot`, the policy may enable `expandable_text_slots`. The generator may expand headings and explanatory `short_label` regions only when current-run geometry proves the target text is longer, the source bbox is not a hard table/chart/nav slot, and right/down whitespace is available. Evidence must include `expandable_text_slot_applied`, `expandable_text_slot_profile`, and `source_anchor_bbox`. This rule must not branch on filenames, page numbers, literal headings, years, or official reference coordinates.
