...
> 文档状态：经迁移纠偏、翻译完整性与 Repair Memory 架构补充修订，作为 P9C/P9A/P9B 施工基线
> 更新日期：2026-07-20
...
独立闭环不再把“有一个可打开 PDF”当成“翻译成功”。系统在翻译前冻结 SemanticUnitMap，以 TranslationCompletenessDecision 验证全部语义单元；安全 FinalDeliveryArtifact 与非发布 TranslatedDiagnosticCandidate 分轨保存；页、文档和阶段分别报告 EngineeringClosure、ProductAcceptance、PromotionEligibility。源 PDF 透传可以完成工程降级，但不能获得产品验收或晋级资格。
...
- 公共 PDF 机械内核；
- 文档级协调器；
- 文档级布局记忆、页级修复记忆、受控跨文档规则与多轮重排协调器；
- `freeform` 有界恢复；
...
- 不把某份历史 PDF 的原文、候选页、绝对坐标或失败轨迹直接拼入其他文档的运行时 Prompt；跨文档经验只能经受控规则晋级后复用。
...
| RepairAttempt | 一次“选择一个未尝试修复动作 → 从不可变源页重排 → 产出候选 → 裁决”的原子记录 |
| RepairRuleRegistry | 经多文档回归和盲测晋级的版本化跨文档修复规则；生产运行只读，不由运行时自动学习或改写 |
| PriorRepairEvidenceRef | 新 run 对同一 source/page 的历史终态失败证据的只读引用；仅在完整指纹一致时参与重复 action/state 排除，不复制历史候选或替代当前页裁决 |

其中 `DocumentLayoutMemory`、`PageRepairMemory` 和 `RepairRuleRegistry` 三类 Repair Memory 均服务 PDF 布局与修复，不等同于 MerqFin 的 Translation Memory（TM）。TM 管理语义译文复用；Repair Memory 不保存或替代术语、Prompt、译文匹配和审校资产。
...
flowchart TD
    ...
    PM --> I[冻结 SemanticUnitMap 与稳定 TranslationUnit]
    ...
    O -- 是 --> RM[读取页级失败历史与只读 RepairRuleRegistry]
...
这张图冻结十个主流程不变量：只有 Transflow 领取 PDF；分类后才绑定工具箱；全页事实与路由屏障后才冻结文档记忆；并发页只读同一记忆 hash；翻译前冻结 SemanticUnitMap 且完整性 PASS 才进入布局；完整译文下的布局失败保留隔离诊断，源副本不冒充；页级修复历史可恢复且不重复已提交动作；每轮 Repair 都必须产出可裁决候选且有上限；每页必须进入可最终化终态；全部页面终态后只能在源 PDF 副本上串行回放 Patch 和执行硬校验，不做二次全局样式重排。
...
| 9. 构建语义单元分母 | Toolbox / Recovery | SemanticUnitMap、TranslationUnit 列表 |
...
RepairContext = {PageRepairMemory, PriorRepairEvidenceRef?, RepairRuleRegistrySnapshot, RepairBudget}
...
LegacyToolboxAdapter 可以在内部一次性完成 map/request 适配，但必须向协调器交付可校验的 SemanticUnitMap 和 TranslationCompletenessDecision；不得因旧入口只返回 PDF 就跳过完整性分母。
...
classDiagram
    ...
    DocumentCoordinator --> RepairRuleRegistry : pins snapshot
    ...
    RepairCoordinator --> PriorRepairEvidenceRef : reads exact-match failures
    RepairCoordinator --> RepairRuleRegistry
...
- `RepairRuleRegistry` 是版本化只读输入；生产运行不得把本次失败自动写成下一份 PDF 的规则。
...
### 9.5 三层 Repair Memory 模型

Repair Memory 的目标不是让模型自由回忆历史，而是让确定性协调器知道“本份文档的版式是什么、这一页已经失败过什么、哪些跨文档规则已经被正式验证”。V1 固定三层，禁止合并成一个无界向量库或聊天历史：
...
| `RepairRuleRegistry` | 跨文档、版本化发布资产 | 生产运行只读；离线评审后发新版本 | 规则晋级流程/代码评审 | Repair Coordinator、Toolbox 私有 Repair |
...
- 版本指纹：DocumentLayoutMemory Builder、Classification、Toolbox Catalog/布局事实提供器、SharedPdfKernel/Patch interpreter、字体 manifest 的版本与哈希；RepairRuleRegistry 不参与源布局事实哈希，而由 run 快照和 PageRepairMemory 单独绑定；
- `source_layout_baseline`：page identity、MediaBox/CropBox/rotation、栏带、gutter、reading order、可编辑区、保护区、重复边缘区，以及正文、标题、列表、表格文字、页眉页脚、图表/示意图标签等角色的字体族/字体 ID、字号、字重、颜色、baseline、对齐、缩进、行间距和段落距离分布；
- `target_layout_policy`：目标语言允许的字体回退链、字形覆盖要求、角色层级保持规则，以及字号、行距、段距、缩进和换行策略的受控调整范围；除明确配置的硬 guard 外，这些值是软约束，不是所有页面必须相等的固定模板；
- 表格事实：表/行/列/cell identity、合并关系、边框、padding、对齐、可编辑文字 owner 和整表回退边界；
- 视觉事实：图片、图表、示意图、Logo、图标和装饰对象的对象哈希、几何、anchor/slot、保护边界及与文字的空间关系；图片内部文字仍不 OCR、不翻译；
- 事实来源：每个值标记 `observed | inferred`、confidence 和 provenance。PDF 直接提供的 bbox/font/baseline 不得与根据行距聚类推断的段落距离混为一谈；低置信推断只能作为软约束；
- 引用而非复制：正文、表格文字等内容以 object/unit/container ID 和内容哈希引用，不在文档记忆中重复保存无界原文或 Provider 响应。
...
- 每轮实际产出的候选 PDF/PNG、Patch、Finding、前后质量向量、硬约束结果；
- `ACCEPTED | ROLLED_BACK | REJECTED | SKIPPED_DUPLICATE` 及原因；
...
#### 9.5.3 RepairRuleRegistry

跨文档“失败经验”需要保留，但不能把一份年报的绝对坐标、公司名、页码或原文当成通用知识。`RepairRuleRegistry` 只允许保存结构化、可审计规则：

```text
rule_id / salience
applicable_routes / required_facts / excluded_conditions
mechanical_when
optional llm_question + allowed_verdicts
failure_class / repair_atom + bounded parameters / bound_tool
acceptance_gates + hard_regression_guards
evidence_refs + registry_version/hash
```

Registry 采用可机械执行的 Rule IR：先校验 `required_facts`，事实不足只返回 `NEED_EVIDENCE`，不得猜测；`mechanical_when` 能裁决时不调用模型；确需模型时只允许回答一个有界 `llm_question`，返回 `allowed_verdicts` 中的枚举，不能生成坐标、修复参数或任意工具名。多条规则命中时按“硬约束优先、`salience` 降序、`rule_id` 稳定序”解析，任何冲突和最终绑定的 `bound_tool/acceptance_gates` 都必须可追溯。

规则必须先在至少两份来源和结构不同的完整 PDF 上回归，再在一份未参与规则编写的匿名 holdout PDF 上盲测；全部通过后经代码评审和 Gate 才能发布新 registry 版本。生产运行只读取固定快照，禁止：

- 运行中自动写入或覆盖 Registry；
- 保存文件名、sample_id、公司名、固定页码、历史原文或绝对坐标常量；
- 把历史失败页面、完整候选图或无界修复轨迹直接放入当前模型 Prompt；
- 因单一样本改善就自动晋级为跨文档规则。
...
2. 若 Judge 失败，依据当前 Finding、页级历史和 Registry 过滤出适用且未尝试的动作；每轮只选一个主要 Repair 动作，保证因果可解释。
...
4. 每个实际执行的轮次无论成功或失败，都必须先产出可打开候选及 Patch/Finding/Decision 证据，才能判断错误是否改善。
5. 新候选同时比较目标 Finding 改善和所有硬回归项；目标改善且无硬回归才接受，否则回滚到本轮前最佳候选并登记失败经验。
...
- 不存在跨页面共享的可变 RepairMemory；各页可以并发修复。文档级 `document_repair_summary` 只能在页终态后聚合用于审计和未来 Registry 晋级取证，不得反向影响同 run 尚未执行页面。
...
- `DocumentLayoutMemory` 只有 source、源/目标语言对、配置、页几何、Builder/分类器、Catalog/布局事实提供器、Kernel/Patch interpreter、字体和 memory schema 指纹全部相同才可复用；`PageRepairMemory` 还必须额外匹配 Toolbox 和 RepairRuleRegistry hash。任一所需指纹不一致均保留旧 run 审计记录并创建新 run，不偷用陈旧记忆。
- 新 run 不直接续写旧 `PageRepairMemory`。只有 source/page/Route/Toolbox/config/document-memory/registry、TranslationBundle 和相关 Schema/实现指纹全部精确一致时，才可生成 `PriorRepairEvidenceRef`，把旧终态中的失败 action/state hash 作为只读排除集；任一指纹不一致时历史证据只供审计，不参与动作选择。历史候选、历史译文和自然语言总结不得注入当前模型 Prompt。
...
每页在调用 Translation Port 前必须冻结 `SemanticUnitMap`。它覆盖该页全部可翻译原生文字，记录稳定 `unit_id/container_id`、owner、阅读顺序、源文本哈希、必保留字面量和预期处置。表格、段落、列表或复合叶可以保留各自私有分组算法，但必须投影为同一个完整性合同，不能让某个叶用“局部请求成功”代替“整页语义单元完整”。
...
- 新 run 只有在 §9.5.5 所列全部指纹及 TranslationBundle hash 精确一致时，才可通过 `PriorRepairEvidenceRef` 继承旧终态失败 action/state 排除集；否则历史记录只读审计、不影响当前动作选择；
- 未原子提交的步骤可以重放；
- 文档最终化和最终发布必须幂等；
- 恢复前重新校验源文件哈希、配置快照、记忆 Schema/哈希、RepairRuleRegistry、字体、Kernel/Patch interpreter 和工具箱版本。
...
1. 加载冻结的 `DocumentLayoutMemory`、当前 `PageRepairMemory` 和只读 `RepairRuleRegistry` 快照；
2. 为初始候选和每个实际 Repair 轮次生成稳定 `state_hash/action_key`；
3. 排除已经尝试的动作、已见候选状态和不满足 required facts 的规则；
4. 每轮只改变一个主要修复维度，从源页与同一有效 TranslationBundle 重排并实际产出候选；
5. 用同一 Judge 合同比较前后 Finding、质量向量和硬约束，明确接受或回滚；
6. 在选择下一动作前原子提交本轮证据和页级记忆；
7. 达到停止条件后立即进入确定性 fallback，不再把相同失败发送给相同动作。

文档级记忆提供稳定版式参照，不决定某一页的修复动作；页级记忆负责“这一页已试过什么”；跨文档 Registry 只提供已验证候选规则。三层职责不得互换。
...
PageToolbox/Freeform 在发送请求前先按 §9.6.1 冻结 SemanticUnitMap；Translation Port 响应仍先执行 ID/Schema 校验，再执行内容完整性裁决。完整性裁决至少检查缺失/重复/新增 ID、空串/占位/异常回显、无显式 KEEP_SOURCE 的原文照抄、必保留字面量、残留源语言和 unit/container owner。失败时只允许对失败 unit 组成新的定向 TranslationBatch；所有 unit 通过后形成内容寻址 TranslationBundle 和 TranslationCompletenessDecision，二者共同进入候选生成与 Checkpoint。
...
- RepairRuleRegistry 的固定版本、哈希和命中规则 ID；
- PriorRepairEvidenceRef 的来源 run、完整指纹匹配结果和实际排除 action/state；
...
RepairAttempt 使用相同协议，但其 `action_key`、输出路径和候选状态哈希必须确定性生成。候选、Patch、Finding、Decision 和新版 `PageRepairMemory` 均校验并原子 rename 后，Checkpoint 才能引用该轮次。若进程在 Artifact rename 后、Checkpoint 前崩溃，恢复器可以按 action key 和内容哈希复用孤儿证据并补交同一轮，不得把它计成新的尝试；只有 Checkpoint 成功后才视为“已尝试”。
...
│  │  └─ repair_rule_registry.json      # 只读、版本化、经 Gate 晋级的跨文档规则
│  ├─ schemas/
...
5. 冻结 `DocumentLayoutMemory`、`PageRepairMemory`、Rule IR/`RepairRuleRegistry` 和多轮重排协调器；
...
- 每个已执行 Repair 轮次都有真实候选和可恢复失败经验，重复动作/状态循环被确定性拦截；
- 跨文档修复规则只经回归、holdout 盲测和 Gate 晋级，生产运行不自动学习；
...
- `SemanticUnitMap/TranslationCompletenessDecision` 的分母冻结、逐 unit 唯一处置、占位/无理由原文回填/残留拒绝、定向重译和内容寻址；
...
- `PriorRepairEvidenceRef` 只在 source/page/Route/Toolbox/config/document-memory/registry/TranslationBundle 等完整指纹一致时生效，指纹不一致时排除动作数为 `0`；
- `RepairRuleRegistry` 的 Rule IR、事实不足 `NEED_EVIDENCE`、确定性冲突优先级、模型枚举边界、版本/hash、运行时只读和晋级证据完整性；
...
- 某页能力缺失；
- 某页质量失败；
- 同一完整 PDF 的正文、标题、表格、图表/图标 anchor、行间距和段落距离画像能以同一 hash 被跨页读取，且候选页不能污染源布局记忆；文档画像是软先验，不要求不同结构页面解析出完全相同的页级参数；
- 至少两个结构不同页面在同一文档记忆下形成各自 `PageEffectiveLayout` 并通过页级 Gate，Document Finalizer 的全书样式归一/二次 Judge 调用数为 `0`；
- 一页连续多轮失败后，每轮都有真实候选与失败记录，下一轮不重复已提交动作，最终接受安全候选或明确 fallback；
- 对正文、表格、图表/图标标签、旋转文字和复合页的真实样本冻结 SemanticUnitMap，完整 Bundle 的 unit 物化/可提取率为 `100%`、漏单元为 `0`；布局失败仍产出隔离的 TranslatedDiagnosticCandidate，Provider/完整性失败则明确 `NO_TRANSLATED_CANDIDATE`；
...
- Registry 新规则至少经过两个不同来源/结构完整 PDF 回归和一个匿名 holdout PDF 盲测，运行时自写规则数为 `0`；
- 同 source 新 run 只有完整指纹一致才允许导入失败 action/state 排除集；改动任一指纹时历史动作对当前选择影响数为 `0`；
...
- 三层 Repair Memory、Rule IR、真实候选、多轮停止、Checkpoint/同源历史失败证据和跨文档规则晋级 Gate 全部通过。
...
| 只校验响应成功/ID 对齐，遗漏占位、原文回填、未翻译单元或 PDF 中未实际物化的译文 | SemanticUnitMap 冻结完整分母；TranslationCompletenessDecision 与 PDF 物化/可提取探针双重验收 |
...
| 同一 source 新 run 忘记历史终态失败，重复完全相同 action/state | 完整指纹一致时只读导入 PriorRepairEvidenceRef；任一指纹变化即不参与选择，避免陈旧经验污染 |
...
| 单一样本经验自动传播到其他 PDF，形成隐蔽过拟合 | RepairRuleRegistry 生产只读；多文档回归、匿名 holdout 和评审后才能发布新版本 |
...
35. Repair Memory 分为文档级布局事实、页级修复账本和跨文档规则库三层；它们不等同于语义 Translation Memory。
...
38. `RepairRuleRegistry` 是版本化生产只读资产；运行时不自动学习，规则必须经多文档回归、匿名 holdout 和 Gate 晋级。
39. 版式 Repair 复用同一有效 TranslationBundle，并从不可变源页重新生成候选；只有翻译合同/语义失败才允许定向重译。
40. 每个实际 Repair 轮次都必须产出可裁决候选和证据；预算耗尽后仍通过区域/页面/文档 fallback 产出完整 PDF。
...
47. 每页翻译前冻结 SemanticUnitMap，TranslationCompletenessDecision PASS 是进入正常排版和生成翻译诊断候选的前置条件；ID 齐全或 HTTP 成功本身不代表翻译完整。
...
50. 新 run 只在完整身份和 TranslationBundle 指纹一致时，通过 PriorRepairEvidenceRef 排除同源同页已经失败的 action/state；否则历史证据仅供审计。
51. RepairRuleRegistry 使用可机械执行的 Rule IR；事实不足返回 NEED_EVIDENCE，模型只回答允许枚举，不生成坐标、参数或工具。
...
- RepairRuleRegistry 的长期维护人、规则废弃窗口和超出 V1 三文档最低晋级证据后的分层阈值；
...
- PriorRepairEvidenceRef 的审计保留期限、同源多 run 索引和失效清理策略；
