...
> 文档状态：经 Repair Memory 架构补充修订，作为 P9A/P9B 施工基线
> 更新日期：2026-07-20
...
上述三类 Repair Memory 均服务 PDF 布局与修复，不等同于 MerqFin 的 Translation Memory（TM）。TM 管理语义译文复用；Repair Memory 不保存或替代术语、Prompt、译文匹配和审校资产。
...
6. 页面进入独立流水线，可按资源预算并行翻译和排版；每页初始化或恢复自己的 `PageRepairMemory`，每个已提交修复尝试都成为安全点。
7. 每页把本页源事实和路由作为直接依据，把文档角色画像与目标语言策略作为软先验，再结合有效 `TranslationBundle` 和本页失败历史解析 `PageEffectiveLayout`。首轮或修复轮失败时，Repair Coordinator 只选择一个未尝试动作，从不可变源页重新规划、渲染、裁决并登记结果；局部优化只写入页级记忆。
8. 每页必须进入可最终化终态：批准 Patch、区域降级 Patch 或原页透传；不得因某一页失败漏掉该页或提前结束整本任务。
9. 全部原始页都进入终态后，Document Finalizer 只打开一次源 PDF 副本并按原页序串行回放已批准 Patch；透传页不执行写入，也不再发起全书样式归一、二次布局规划或二次 Judge。
10. 最终校验和原子发布产出一份页数、页序和文档特性满足 Preservation Contract 的完整目标 PDF。
...
这张图冻结八个主流程不变量：只有 Transflow 领取 PDF；分类后才绑定工具箱；全页事实与路由屏障后才冻结文档记忆；并发页只读同一记忆 hash；页级修复历史可恢复且不重复已提交动作；每轮 Repair 都必须产出可裁决候选且有上限；每页必须进入可最终化终态；全部页面终态后只能在源 PDF 副本上串行回放 Patch 和执行硬校验，不做二次全局样式重排。
...
| 9. 构建翻译单元 | Toolbox / Recovery | TranslationUnit 列表 |
| 10. 调度翻译 | Transflow | TranslationBatch |
| 11. 执行语义翻译 | AI Capability Service / Translation Capability | TranslationBundle |
| 12. 页级有效布局与排版规划 | Toolbox | PageEffectiveLayout、PagePatch / RegionPatch |
| 13. 渲染、裁决并登记 | Toolbox + Kernel + Repair Coordinator | Candidate、Finding、QualityDecision、RepairAttempt |
| 14. 有限多轮修复 | Repair Coordinator | 新候选、接受/回滚或降级决定 |
| 15. 页面终态 | Page Finalizer | 批准 Patch、产品预览 PNG、页级结果或透传决定 |
| 16. 文档最终化 | Document Finalizer | 从源 PDF 副本串行回放 Patch 后的完整临时 PDF；不执行全书样式二次归一 |
| 17. 不可变发布 | Artifact Publisher | immutable target、target_file_path、Artifact、最终状态 |
...
sequenceDiagram
    ...
    loop 每个未 FINALIZED 原始页面（有界并发）
        ...
        PC->>TB: 解析 PageEffectiveLayout 后 plan + render + judge
...
RepairContext = {PageRepairMemory, RepairRuleRegistrySnapshot, RepairBudget}
...
repair_atom + bounded parameters
acceptance_metrics + hard_regression_guards
...
3. 从不可变源页重新生成完整候选，不在上一张失败候选上累积涂改；版式类 Repair 必须复用同一份 TranslationBundle，不重复请求翻译。
...
V1 不强制引入 Prometheus。结构化日志和运行报告至少能聚合：队列等待时间、页耗时、租约丢失次数、进程池占用、翻译/分类超时率、每页修复轮数、重复动作拦截数、状态循环数、修复接受/回滚率、`degradation_rate`、`region_fallback_rate`、`page_passthrough_rate`、翻译覆盖率和工作目录字节数。
...
│     ├─ logs/
│     ├─ reports/document-repair-summary.json
│     └─ final/
├─ _page_previews/
└─ <可选固定文件名兼容副本；非权威>
```
...
4. 第二批叶完成后冻结 `DocumentLayoutMemory`、`PageRepairMemory`、`RepairRuleRegistry` 和多轮重排协调器；
5. `visual_anchored` 和各 composite 单独迁移，已知失败叶使用已冻结 Repair Memory 闭环最后处理；
6. 全叶 Catalog、Repair Memory 与 fallback 回归。
...
- `RepairRuleRegistry` 的 Schema、版本/hash、运行时只读和晋级证据完整性；
- PageOutcome；
...
- 三层 Repair Memory 合同、真实候选、多轮停止、Checkpoint 恢复和跨文档规则晋级 Gate 全部通过。
