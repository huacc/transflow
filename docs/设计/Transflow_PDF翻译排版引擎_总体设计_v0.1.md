...
> 文档状态：经 P9C 实测及 V1 最小落地复核后原文件修订，作为 P9A/P9B 后续施工基线
> 更新日期：2026-07-20
...
> P9C 后置修订说明：项目负责人明确授权直接修订当前 v0.1 文件，不另建版本文件。P9C 已完成报告及其历史锚点仍指向修订前输入哈希；本次修订不得回写 P5～P9 历史报告/Gate，也不得把当前文件的新哈希伪装成 P9C 当时输入。P9A.0 必须登记修订前后哈希、刷新当前 baseline/traceability，并保持旧阶段证据可追溯。
...
独立闭环不再把“有一个可打开 PDF”当成“翻译成功”。系统先由 SharedPdfKernel 冻结独立 PageTextInventory，再由 Toolbox 投影并核对 SemanticUnitMap，以 TranslationCompletenessDecision 验证全部语义单元；安全 FinalDeliveryArtifact 与非发布 TranslatedDiagnosticCandidate 分轨保存；页、文档和阶段分别报告 EngineeringClosure、ProductAcceptance、PromotionEligibility。源 PDF 透传可以完成工程降级，但不能获得产品验收或晋级资格。
...
- 公共 PDF 机械内核；
- 文档级协调器；
- 文档级布局记忆、当前 run 页级修复记忆、确定性 RepairAtomCatalog 与多轮重排协调器；跨文档 Registry 在 V1 仅保留静态扩展面；
- `freeform` 有界恢复；
...
- 不把某份历史 PDF 的原文、候选页、绝对坐标或失败轨迹直接拼入其他文档的运行时 Prompt；V1 不复用跨文档经验，未来若启用必须另立受控规则、反过拟合 Gate 和 baseline。
...
| SemanticUnitMap | 当前页在翻译前冻结的语义单元全集及其 owner、阅读顺序、必保留字面量和预期处置；是翻译完整性判定的分母 |
| PageTextInventory | SharedPdfKernel 在 Toolbox/TranslationPort 之前从权威 PageFacts 冻结的页级原生文字清单；每个文字对象必须预授权为 `TRANSLATE | KEEP_SOURCE | PROTECTED | NON_TRANSLATABLE`，用于证明 SemanticUnitMap 没有漏项 |
| TranslationCompletenessDecision | 对 TranslationBundle 是否逐一覆盖 SemanticUnitMap、是否存在占位/无理由原文回填/残留以及是否允许进入排版的结构化裁决 |
| FinalDeliveryArtifact | 按 Preservation 和降级合同发布的唯一可交付完整 PDF；可以是完整翻译、部分降级或整本源 PDF 透传 |
| TranslatedDiagnosticCandidate | 仅在完整有效 TranslationBundle 已形成后，为暴露布局/能力/质量失败而生成的非发布 PDF；不得以源 PDF 副本冒充，也不得写入最终交付指针；无法安全物化时必须显式记录失败状态 |
| DocumentLayoutMemory | 当前 run 在全页源事实、完整路由和公共边缘索引全部就绪后生成的文档级布局快照；由 `source_layout_baseline` 与不依赖已生成译文的 `target_layout_policy` 组成，冻结后供所有页面以同一 hash 只读共享 |
| PageEffectiveLayout | 单页在同一文档记忆下，结合本页源事实、路由、TranslationBundle 和 PageRepairMemory 解析出的页级有效布局；不同页面可以合法不同，且不得回写文档记忆 |
| PageRepairMemory | 单页在当前 run 内的修复账本；记录失败、已尝试动作、候选状态、接受/回滚和停止原因，并随 Checkpoint 恢复 |
| RepairAttempt | 一次“选择一个未尝试修复动作 → 从不可变源页尝试重排 → 记录成功候选或 `MATERIALIZATION_FAILED` → 裁决/降级”的原子记录 |
| RepairAtomCatalog | 每个叶显式登记的确定性修复原子、稳定 action identity、适用事实和有界参数；V1 RepairCoordinator 只从该目录选择单个动作 |
| RepairComparison | 当前叶版本化 comparator 的结构化比较结果；记录指标方向、硬拒绝项、精度/epsilon、tie 语义、前后值和 comparator hash |
| RepairRuleRegistry | 跨文档修复规则的版本化只读扩展面；V1 只固定一个可为空的静态快照/hash，不执行规则晋级、通用 Rule DSL 或 Repair LLM |
| PriorRepairEvidenceRef | 新 run 对同一 source/page 历史终态的审计只读引用；V1 不用它排除当前动作，不复制历史候选或替代当前页裁决 |

其中 V1 运行时真正参与决策的是 `DocumentLayoutMemory` 与当前 run 的 `PageRepairMemory`；`RepairRuleRegistry` 仅保留静态空/固定快照兼容面，`PriorRepairEvidenceRef` 仅供审计。它们均服务 PDF 布局与修复，不等同于 MerqFin 的 Translation Memory（TM）。TM 管理语义译文复用；Repair Memory 不保存或替代术语、Prompt、译文匹配和审校资产。
...
4. 等待“全页 `PageFacts` + 全页路由 + 公共边缘索引”屏障完成，再构建 `DocumentLayoutMemory`：先汇总源文档真实版式为 `source_layout_baseline`，再在该基线之上根据源/目标语言、受控字体与配置派生 `target_layout_policy`；该过程不等待译后页面，也禁止用第一张先完成的页面代表整本。记忆写成内容寻址 Artifact、提交 Checkpoint 并进入 `FROZEN` 后才可使用。
5. 只有文档记忆冻结成功后，Document Coordinator 才将页面计划和同一个记忆引用放入有界 ready queue；只按文档/页面/Provider/PDF 进程预算放行，不能一次性启动年报全部页面。
6. 页面进入独立流水线，可按资源预算并行翻译和排版；每页初始化或恢复自己的 `PageRepairMemory`，冻结 `SemanticUnitMap` 后调用 Translation Port，每个完整性决定和已提交修复尝试都成为安全点。
7. TranslationBundle 必须先通过 `TranslationCompletenessDecision`；失败只定向重译失败 unit，仍不完整则记录 `NO_TRANSLATED_CANDIDATE` 并降级，不得进入正常布局。
8. 每页把本页源事实和路由作为直接依据，把文档角色画像与目标语言策略作为软先验，再结合有效 `TranslationBundle` 和本页失败历史解析 `PageEffectiveLayout`。首轮或修复轮失败时，Repair Coordinator 只选择一个未尝试动作，从不可变源页重新规划、渲染、裁决并登记结果；局部优化只写入页级记忆。完整译文下最终仍失败时，另行物化隔离的翻译诊断候选或记录诊断物化失败。
9. 每页必须进入可最终化终态：批准 Patch、区域降级 Patch 或原页透传；不得因某一页失败漏掉该页或提前结束整本任务。
10. 全部原始页都进入终态后，Document Finalizer 只打开一次源 PDF 副本并按原页序串行回放已批准 Patch；透传页不执行写入，也不再发起全书样式归一、二次布局规划或二次 Judge。
11. 最终校验和原子发布产出一份页数、页序和文档特性满足 Preservation Contract 的完整目标 PDF；诊断候选不参与该发布链。

该模型提供全书共同的软基线，但不要求所有页面的字号、行距、段距或修复动作百分之百相同；页级事实优先，结构不同的页面可以在同一文档记忆下形成不同的有效布局。最终化仍执行可打开性和 Preservation Contract 等硬校验，它们不是二次布局检查。
...
flowchart TD
    ...
    G --> DM[全页事实与路由屏障后构建源基线/目标策略并冻结]
    DM --> H[页面计划和文档记忆引用进入有界 ready queue]
    H --> PM[初始化或恢复 PageRepairMemory]
    PM --> I[冻结 PageTextInventory 与 SemanticUnitMap]
    I --> J[调用 Translation Port]
    J --> K{TranslationCompletenessDecision PASS?}
    K -- 否 --> L[定向重译失败 unit；仍失败则记录 NO_TRANSLATED_CANDIDATE]
    K -- 是 --> EL[解析本页 PageEffectiveLayout]
    EL --> M[从不可变源页规划 Patch、渲染、Judge]
    M --> MR[原子登记候选状态、Finding 与 QualityDecision]
    MR --> N{硬约束与质量 Gate 通过?}
    N -- 是 --> Q[提交正常页面 Checkpoint]
    N -- 否 --> O{Repair 预算尚有且候选可修?}
    O -- 是 --> RM[读取页级失败历史与叶 RepairAtomCatalog]
    RM --> P[选择一个未尝试的确定性 Repair 动作]
    P --> RP[复用同一 TranslationBundle 从源页重新规划]
    RP --> M
    O -- 否 --> DG[物化隔离 TranslatedDiagnosticCandidate 或记录物化失败]
    DG --> R{区域级撤销可保持页面完整?}
    R -- 是 --> S[区域回退并提交降级页面]
    R -- 否 --> T[整页原页透传并提交降级页面]
    L --> S
    ...
    T --> U
    U -- 否 --> H
    U -- 是 --> V[按原页序回放已批准 Patch；不做二次全局样式重排]
...
这张图冻结十个主流程不变量：只有 Transflow 领取 PDF；分类后才绑定工具箱；全页事实与路由屏障后才冻结文档记忆；并发页只读同一记忆 hash；翻译前冻结 PageTextInventory 与 SemanticUnitMap 且完整性 PASS 才进入布局；完整译文下的布局失败保留隔离诊断，源副本不冒充；页级修复历史可恢复且不重复已提交动作；每个已执行 Repair 都必须有唯一终态，物化成功才要求可裁决候选，物化失败必须留证且有上限；每页必须进入可最终化终态；全部页面终态后只能在源 PDF 副本上串行回放 Patch 和执行硬校验，不做二次全局样式重排。
...
| 6. 冻结文档布局记忆 | Document Coordinator | source_layout_baseline、target_layout_policy、memory hash、版本/来源指纹 |
| 7. 绑定执行计划 | Router | PageToolbox 或 FreeformRecoveryPlan、document memory ref |
| 8. 初始化/恢复页级修复记忆 | Repair Coordinator | PageRepairMemory、预算、已尝试动作和状态哈希 |
| 9. 构建并核对语义单元分母 | SharedPdfKernel + Toolbox / Recovery | PageTextInventory、SemanticUnitMap、逐文字对象处置映射、TranslationUnit 列表 |
| 10. 调度翻译 | Transflow | TranslationBatch |
| 11. 执行语义翻译 | AI Capability Service / Translation Capability | TranslationBundle |
| 12. 翻译完整性裁决 | Page Coordinator | TranslationCompletenessDecision、有效 Bundle 或失败 unit/NO_TRANSLATED_CANDIDATE |
| 13. 页级有效布局与排版规划 | Toolbox | PageEffectiveLayout、PagePatch / RegionPatch |
| 14. 渲染、裁决并登记 | Toolbox + Kernel + Repair Coordinator | Candidate、Finding、QualityDecision、RepairAttempt |
| 15. 有限多轮修复与诊断 | Repair Coordinator | 新候选、接受/回滚、TranslatedDiagnosticCandidate/物化失败或降级决定 |
| 16. 页面终态 | Page Finalizer | 批准 Patch、产品预览 PNG、页级结果或透传决定 |
| 17. 文档最终化 | Document Finalizer | 从源 PDF 副本串行回放 Patch 后的完整临时 PDF；不执行全书样式二次归一 |
| 18. 不可变发布 | Artifact Publisher | immutable target、target_file_path、Artifact、最终状态；不含诊断候选 |
...
sequenceDiagram
    ...
    participant RM as Repair Memory
    ...
    DC->>RM: 全页事实/路由屏障后构建源基线与目标策略并冻结

    loop 每个未 FINALIZED 原始页面（有界并发）
        DC->>PC: execute(page plan + document memory ref)
        PC->>RM: 初始化或恢复 PageRepairMemory
        PC->>TB: build template + SemanticUnitMap + TranslationUnit
        TB-->>PC: typed map/request
        PC->>AI: Translation Port: translate(batch, idempotency_key)
        AI-->>PC: translations / per-unit errors
        PC->>PC: 校验 TranslationCompletenessDecision；失败 unit 定向重译或降级
        PC->>TB: PASS 后解析 PageEffectiveLayout，再 plan + render + judge
        PC->>RM: 原子登记 candidate-0 / Finding / decision
        loop 可修、预算尚有且存在未尝试动作
            PC->>RM: 读取失败历史与只读规则快照
            PC->>TB: 从源页和同一 TranslationBundle 重新规划
            TB-->>PC: repair candidate + decision
            PC->>RM: 原子登记 RepairAttempt 与接受/回滚
        end
        opt 完整译文存在但布局/能力/质量最终失败
            PC->>FS: 原子写隔离 TranslatedDiagnosticCandidate 或物化失败证据
        end
        alt 页面安全
...
```text
DocumentExecutionContext = {source_pdf_path, source_hash, config_hash, DocumentLayoutMemoryRef}
PageExecutionContext = {source_pdf_path, source_hash, page_no, PageFacts,
                        SharedRegions, DocumentLayoutMemoryRef}
RepairContext = {PageRepairMemory, RepairAtomCatalogRef, RepairBudget}

build_template(PageExecutionContext) -> PageTemplate
build_semantic_unit_map(PageTemplate, JobContext) -> SemanticUnitMap
build_translation_request(SemanticUnitMap, JobContext) -> TranslationRequest
validate_translation(SemanticUnitMap, TranslationBatchResult)
  -> TranslationCompletenessDecision + TranslationBundle?
resolve_effective_layout(PageExecutionContext, PageTemplate, TranslationBundle,
                         PageRepairMemory) -> PageEffectiveLayout
plan_layout(PageTemplate, TranslationBundle, PageEffectiveLayout) -> PagePatch
render(PageExecutionContext, PagePatch) -> CandidatePage
judge(PageExecutionContext, CandidatePage, PagePatch) -> QualityDecision
repair(PageExecutionContext, CandidatePage, QualityDecision, RepairContext)
  -> RepairProposal | Stop
```

LegacyToolboxAdapter 可以在内部一次性完成 map/request 适配，但必须向协调器交付可校验的 SemanticUnitMap 和 TranslationCompletenessDecision，并与 SharedPdfKernel 预先冻结的 PageTextInventory 逐项核对；不得因旧入口只返回 PDF 就跳过或自行缩小完整性分母。
...
**类型化语义锚点合同。** 横向定位先由叶 Toolbox 根据当前页 `owner + 语义角色 + 源几何/同组关系` 选择锚点。叶 Template / `PageEffectiveLayout` 记录源锚点与安全区域，`PagePatch` 序列化 `owner/region + text_align + target rect` 等执行字段，页级质量证据把 Patch 操作追溯回对应源锚点。`LEFT` 保持源/owner 左边缘，`RIGHT` 保持右边缘，`CENTER` 保持语义槽位或 owner 中心轴，图表、示意图、图例和表格文字还可保持基线、行列或 owner-relative 关系。源 glyph bbox 是首轮槽位和锚点事实，不是目标语言的统一最大宽度；安全扩展必须按锚点方向进行，并受所属栏、cell、节点、相邻 owner、受保护对象和页面边界约束。

共享层不得把所有文字默认成左对齐，也不得使用 `TITLE -> CENTER` 之类无条件映射；具体锚点选择、容差和局部修复仍属于叶私有 Template/Layout/Judge。Patch 解释器按声明锚点写入，Judge 以最终真实 glyph bbox 校验左边缘、右边缘、中心轴或 owner-relative 关系；纵向可在结构安全带内独立适配，但不能借纵向修复改变横向语义锚点。无法证明锚点与 owner 关系安全时必须保留真实译后诊断候选并失败，不得用源 bbox 内无溢出冒充产品通过。
...
classDiagram
    ...
    class PageCoordinator {
        ...
        +freeze_semantic_unit_map()
        +validate_translation()
        +resolve_effective_layout()
        +commit(PageOutcome)
    }
    class RepairCoordinator {
        +run_initial_candidate()
        +run_next_attempt()
        +accept_or_rollback()
        +stop_or_fallback()
    ...
    class PageEffectiveLayout {
        ...
        +page_identity
        +resolved_role_styles
        +page_local_adjustments
    }
    ...
    DocumentCoordinator --> DocumentLayoutMemory : builds and freezes
    DocumentCoordinator --> RepairRuleRegistry : pins dormant snapshot
    ...
    PageCoordinator --> PageTextInventory : freezes from PageFacts
    PageCoordinator --> SemanticUnitMap : freezes
    PageCoordinator --> TranslationCompletenessDecision : validates
    PageCoordinator --> DocumentLayoutMemory : reads pinned ref
    PageCoordinator --> PageEffectiveLayout : derives per page
    PageCoordinator --> RepairCoordinator
    PageCoordinator --> TranslationPort
    PageCoordinator --> CheckpointPort
    RepairCoordinator --> PageToolbox
    RepairCoordinator --> DocumentLayoutMemory
    RepairCoordinator --> PageEffectiveLayout
    RepairCoordinator --> PageRepairMemory
    RepairCoordinator --> RepairAtomCatalog : selects deterministic atom
    RepairCoordinator --> RepairComparison : evaluates by leaf comparator
    RepairCoordinator --> PriorRepairEvidenceRef : records audit-only link
    RepairCoordinator --> TranslatedDiagnosticCandidate : materializes audit-only
    RepairCoordinator --> CheckpointPort
...
- `DocumentCoordinator` 只构建一次文档级布局记忆并冻结；任何页面、Toolbox 或 Repair 都不得原地修改该快照。
...
- `RepairCoordinator` 拥有页级修复循环、预算、记忆提交和接受/回滚；Toolbox 只提出分类私有修复方案，不自行保存跨页或跨文档状态。
- `RepairAtomCatalog` 是 V1 Repair 动作的唯一目录；`RepairRuleRegistry` 只绑定静态版本/hash，不参与动作选择，生产运行不得把本次失败自动写成下一份 PDF 的规则。
...
生产字体策略保持最小：发布资产只冻结一套主要 CJK 字体和必要的 Latin/fallback 字体，不从宿主机系统字体目录隐式取字体。字体文件可以作为 release asset 随包部署，不要求提交进 Git；相对资产引用、版本、许可证和 SHA-256 进入配置快照。字体冻结后必须重录布局基线，并在灰度前通过字形覆盖、跨平台加载和最终 PDF 体积 Gate。V1 不预先引入 fonttools 或另一套字体子集流水线；只有体积 Gate 证明需要时，才在 T6 增加最小子集化实现并单独回归。
...
### 9.5 两层运行时 Repair Memory 与静态扩展边界

Repair Memory 的目标不是让模型自由回忆历史，而是让确定性协调器知道“本份文档的跨页软基线是什么、这一页在当前 run 已经失败过什么”。V1 固定两层运行时记忆，并为未来跨文档规则保留一个不参与当前动作选择的静态扩展面；禁止合并成无界向量库或聊天历史：

| 层 | 作用域 | 可变性 | 写入者 | 主要消费者 |
|---|---|---|---|---|
| `DocumentLayoutMemory` | 单个 source hash + 源/目标语言对 + run 配置 | 全页屏障后构建，`FROZEN` 后不可变 | Document Coordinator | 所有 PageToolbox、Repair Coordinator、Judge |
| `PageRepairMemory` | 单个 source hash + page_no + run | 当前页内追加，FINALIZED 后关闭 | Repair Coordinator 经 CheckpointPort | 当前页恢复、Repair 选择、审计 |
| `RepairRuleRegistry` | 跨文档扩展面 | V1 固定为空或静态只读快照，不在本版晋级 | 配置/发布基线 | V1 仅绑定版本/hash，不参与动作选择 |

#### 9.5.1 DocumentLayoutMemory

`DocumentLayoutMemory` 是完整源 PDF 的布局事实与目标布局政策快照，不是候选页结果缓存，也不是“第一张页面的排版经验”。它不需要等待任何译后页面，冻结时序固定为：

1. Document Coordinator 等待全部原始页的 `PageFacts`、分类路由和公共边缘索引就绪；任一页面未就绪时 Builder 返回 `NOT_READY`，不得提前发布半本画像。
2. 从完整源 PDF 汇总 `source_layout_baseline`，保留文档级分布和逐页事实，避免封面、目录或首张正文页偏置整本风格。
3. 在 `source_layout_baseline` 的角色层级和几何边界上，根据源/目标语言对、受控字体 manifest、配置和允许的调整范围派生 `target_layout_policy`；它描述目标语言排版的边界与软建议，不冒充尚不存在的译后文档事实。
4. 对两部分 canonical 序列化、计算内容 hash、原子发布 Artifact、提交 run Checkpoint，并将状态切换为 `FROZEN`。
5. 只有 `FROZEN` 引用可进入页面 ready queue；所有已放行页面在本 run 内固定使用该 hash。

因此禁止“第一次遇到某种页面就写入并持续修改全局记忆”。在页面执行前发现事实不完整可以重新构建尚未发布的草稿；任一页面已放行后则不得原地修改或混用版本。

文档记忆至少包含：

- 文档标识：`source_hash`、页数、页框/旋转摘要、源/目标语言、配置哈希、Schema 版本；
- 版本指纹：DocumentLayoutMemory Builder、Classification、Toolbox Catalog、SharedPdfKernel/Patch interpreter、字体 manifest 的版本与哈希；V1 静态 Registry hash 不参与源布局事实哈希，而由 run 配置快照单独绑定；
- `source_layout_baseline`：page identity、MediaBox/CropBox/rotation、完整 Route、权威 `PageFacts` 内容寻址引用、公共页眉页脚/页码/边缘保护区，以及正文、标题、列表、表格文字、图表/示意图标签等语义角色在全书范围内的字体族/字体 ID、字号、字重、颜色、baseline、对齐、缩进、行间距和段落距离分布；
- `target_layout_policy`：目标语言允许的字体回退链、字形覆盖要求、角色层级保持规则，以及字号、行距、段距、缩进和换行策略的受控调整范围；除明确配置的硬 guard 外，这些值是软约束，不是所有页面必须相等的固定模板；
- 页级事实单一来源：表/行/列/cell、页内 owner、anchor/slot、reading order、视觉对象几何和文字绑定的完整明细继续由 `PageFacts` 与叶私有 Template/Layout 合同持有；DocumentLayoutMemory 只保存其 hash/ref、公共跨页聚合和有真实消费者的角色分布，不复制第二份页级结构；
- 视觉边界：只聚合跨页重复视觉元素、公共保护区和角色级分布；图片内部文字仍不 OCR、不翻译，单页视觉对象身份与几何从对应 `PageFacts` ref 读取；
- 事实来源：每个值标记 `observed | inferred`、confidence 和 provenance。PDF 直接提供的 bbox/font/baseline 不得与根据行距聚类推断的段落距离混为一谈；低置信推断只能作为软约束；
- 引用而非复制：正文、表格文字及页内对象以 PageFacts/Template 内容哈希引用，不在文档记忆中重复保存无界原文、完整页级结构或 Provider 响应。

Builder 不调用依赖 DocumentLayoutMemory 的 `PageToolbox.prepare/render/judge/repair`。V1 只消费 SharedPdfKernel PageFacts、完整 Route 和公共边缘事实；某项叶级事实没有明确跨页消费者时不得加入 memory Schema。以后若真实消费者证明必须在屏障前聚合叶事实，只能增加一个窄的只读 `collect_layout_facts(route, raw_facts)` seam，且该 seam 不得依赖 DocumentLayoutMemory，不得调用 Translation、render、judge 或 repair。

同一 run 的所有页面必须读取同一个 `memory_hash`，但“同一记忆”不等于“页级结果完全相同”。每页在运行时解析自己的 `PageEffectiveLayout`，合并优先级为：

1. Preservation、对象 owner、protected region、页框和字体可用性等硬约束；
2. 当前页直接观测到的源事实、页面路由和页内 reading order；
3. 文档角色分布与 `target_layout_policy` 提供的软先验；
4. 当前页 `TranslationBundle` 的真实长度/密度和 `PageRepairMemory` 中已验证的页级调整。

文档级统计应保存分布、置信度和允许区间，而不是强迫全部页面使用单个精确值。页面候选、失败修复和译后布局不得回写文档级基线；页面特有调整写入 `PageEffectiveLayout`、RepairAttempt 和 `PageRepairMemory`，否则并发页会相互污染，后处理页也会把前面页面的失败结果误当成源文档风格。

#### 9.5.2 PageRepairMemory

`PageRepairMemory` 记录当前页怎样从失败走向接受或降级。其 identity 至少绑定：

```text
source_hash + page_no + classification_route + toolbox/version
+ config_hash + document_layout_memory_hash + repair_rule_registry_hash
```

每页至少保存：

- 初始候选的 `PageEffectiveLayout`/Patch/状态哈希、Finding、QualityDecision 和质量向量；
- `attempted_action_keys`、`seen_state_hashes`、已用预算、连续无改善次数；
- 每个 `RepairAttempt` 的 failure class、目标 owner/region、修复原子、参数、页级有效布局差异、依据规则、输入状态哈希；
- 每个 proposal 的预检结论；只有真正选择并执行的动作才形成 RepairAttempt、消耗实际轮次预算；
- 成功物化轮次的候选 PDF/PNG、Patch、Finding、前后质量向量和硬约束结果；物化失败轮次保存 action、输入 state hash、结构化异常与已产生的安全证据，不伪造候选；
- `ACCEPTED | ROLLED_BACK | REJECTED | SKIPPED_DUPLICATE | MATERIALIZATION_FAILED` 及原因；
- 最终采用的 Patch/候选引用、fallback 层级和 `PASSED | BUDGET_EXHAUSTED | NO_IMPROVEMENT | STATE_CYCLE | NO_APPLICABLE_ACTION | HARD_CONSTRAINT_FAILED` 终止原因。

页级记忆是追加式运行账本，不是让 LLM自行总结的自然语言聊天记录。相同 `action_key` 或相同候选 `state_hash` 不得再次进入已提交轮次；恢复后必须从最后一个已提交 RepairAttempt 继续。

#### 9.5.3 RepairAtomCatalog 与 RepairRuleRegistry V1 边界

V1 必须解决的是同一页面、同一 run 内不重复失败动作，而不是建设跨文档学习系统。每个 Toolbox 通过版本化 `RepairAtomCatalog` 显式登记叶私有、确定性的修复原子：

```text
atom_id / catalog_version/hash
applicable_finding_codes / required_facts / excluded_conditions
bounded_parameters / owner_scope / hard_guards
apply_adapter / comparator_version
```

RepairCoordinator 先按 required facts、硬约束、`atom_id` 稳定序过滤，再从未尝试动作中只选择一个。已有叶若只有黑盒单步 `repair()`，由窄 Legacy Repair Adapter 暴露一个或少量手工登记的稳定原子；不得反射发现工具、跨叶调用私有实现或建立通用可执行 Rule DSL。

`RepairRuleRegistry` 在 V1 只保留可为空的静态只读快照和版本/hash，以维持未来扩展接口；它不参与动作选择，不自动/半自动晋级规则，不要求“两文档 + holdout”发布流程。V1 Repair 模型调用数固定为 `0`，与 §21.3“唯一启用模型判断节点为 ClassificationDecision”保持一致。未来若真实证据需要跨文档规则或有界 Repair 模型节点，必须形成新设计、独立 Gate 和新 baseline，不能通过当前 dormant 字段静默启用。

生产运行仍禁止：

- 运行中新增、修改或覆盖 Registry；
- 保存文件名、sample_id、公司名、固定页码、历史原文或绝对坐标常量；
- 把历史失败页面、完整候选图或无界修复轨迹注入当前模型 Prompt；
- 把当前 PageRepairMemory 自动传播为其他 PDF 的规则。

#### 9.5.4 多轮重排闭环

首轮排版与后续 Repair 统一遵循以下顺序：

1. 从不可变源页、固定 hash 的 `DocumentLayoutMemory` 和已校验 `TranslationBundle` 解析本页 `PageEffectiveLayout`，生成候选 0 并登记基线状态。
2. 若 Judge 失败，依据当前 Finding、页级历史和叶的 `RepairAtomCatalog` 过滤出适用且未尝试的确定性动作；每轮只选一个主要 Repair 动作，保证因果可解释。V1 不调用 Repair 模型，不读取跨新 run 排除集。
3. 从不可变源页重新生成完整候选，不在上一张失败候选上累积涂改；版式类 Repair 必须复用同一份 TranslationBundle，不重复请求翻译。当前页内部的段落、表格组或 FlowBand 最终回流属于本轮候选生成的一部分，必须在本页 Judge 前完成并记入同一 RepairAttempt；它不等于页面 FINALIZED 后的全书二次重排。
4. 每个已选择并实际执行的动作都必须形成唯一终态 RepairAttempt。物化成功时必须产出可打开候选及 Patch/Finding/Decision 证据；字体、PDF 写入、进程或磁盘等导致无法安全物化时记录 `MATERIALIZATION_FAILED`、结构化错误和已有安全证据，消耗本轮预算但不得伪造候选成功。
5. 新候选由当前叶版本化 comparator 比较目标 Finding 与全部硬回归项；比较合同必须冻结指标方向、硬拒绝项、数值精度/epsilon、相等/tie 语义和 comparator hash。目标改善且无硬回归才接受，否则回滚到本轮前最佳候选并登记失败经验；不得使用跨叶万能质量总分，也不得把 `ACCEPTED_NEW_FAILURE` 直接提升为产品接受。
6. 若失败类别明确为译文缺失、ID 错配或语义合同失败，才允许对受影响 unit 做新的定向 TranslationBatch；布局失败不得以重新翻译掩盖。
7. 达到轮次预算、连续无改善、状态循环、无未尝试动作或硬约束失败时停止 Repair，进入区域/原子回退或整页透传；失败候选只保留为诊断 Artifact，不进入最终 PDF。

页面 FINALIZED 后不再运行一次“把所有页拉齐”的全书样式归一、布局重排或质量重判。页面之间在共同软基线下保留合理差异；Document Finalizer 只回放各页已批准 Patch，并执行 Preservation、可打开性、页数和页序等硬校验。

V1 初始默认 `max_repair_rounds=3`、`max_no_improvement=2`，两者必须进入配置快照和页级记忆；任何调整都要重跑 P9B 与受影响叶 Gate。无论 Repair 是否成功，每个可读源 PDF 最终仍按 §12 的降级顺序产出完整结果。

#### 9.5.5 生命周期、并发与失效

- 文档记忆 Builder 由 Document Coordinator 在全页事实/路由屏障后以 single-flight 方式运行；文档记忆先写为内容寻址 Artifact，再由 run Checkpoint 引用并进入 `FROZEN`，最后才允许页面出队。
- 线程并发时各任务只持有不可变只读视图；进程并发时各 Worker 按同一 Artifact hash 反序列化本进程只读副本。禁止跨线程/进程共享可变 Python 对象或已打开的 PyMuPDF `Document`，共享语义由相同 hash、Schema 和内容保证。
- 页级记忆随每个 RepairAttempt 的不可变证据一起提交；旧 Worker 的迟到提交必须被 `run_token + checkpoint_version` CAS 拒绝。
- 不存在跨页面共享的可变 RepairMemory；各页可以并发修复。文档级 `document_repair_summary` 只能在页终态后聚合用于审计和未来设计取证，不得反向影响同 run 尚未执行页面，也不得在 V1 自动形成 Registry 规则。
- 页面 ready queue 尚未放行时，发现输入事实或配置有误可以丢弃草稿并发布新 hash；任一页面已放行后，软性页面差异只写 PageRepairMemory，不修改文档记忆。若确认 source、配置、Builder 或硬事实错误，则当前 run 失效并以新 hash 启动新 run，禁止让已完成页和未执行页混用两个版本。
- `DocumentLayoutMemory` 只有 source、源/目标语言对、配置、页几何、Builder/分类器、Catalog、Kernel/Patch interpreter、字体和 memory schema 指纹全部相同才可复用；`PageRepairMemory` 还必须额外匹配 Toolbox、RepairAtomCatalog 和 comparator hash。静态 RepairRuleRegistry hash 绑定 run 配置与审计清单，但因其 V1 不参与动作选择，不得被解释为已启用跨文档规则。任一运行时所需指纹不一致均保留旧 run 审计记录并创建新 run，不偷用陈旧记忆。
- 新 run 不直接续写旧 `PageRepairMemory`。可以在 source/page/Route/Toolbox/config/document-memory/TranslationBundle 和相关 Schema/实现指纹精确一致时生成 `PriorRepairEvidenceRef`，但 V1 只用它定位旧终态审计证据，不导入失败 action/state 排除集，也不影响当前动作选择。历史候选、历史译文和自然语言总结不得注入当前模型 Prompt；同一 run 的恢复仍按当前 Checkpoint 继续并排除本 run 已提交动作。
- 记忆 Artifact 遵循 §17–§18 的敏感信息、容量和清理规则；不保存密钥、完整 Provider 响应、无界原文、历史 PDF 页面或不可追溯的自然语言“经验”。

### 9.6 迁移纠偏横切合同

P5–P9 等已经完成的阶段报告和 Gate 是历史事实，不因后续发现合同遗漏而被原地改写。后续实施应以独立纠偏阶段登记“历史证据、当前问题、前向合同、受影响回归和新的 Gate”；旧 Gate 只说明当时按旧基线完成，不自动证明新合同已满足。

#### 9.6.1 语义单元与翻译完整性

每页在调用 Toolbox 的语义分组和 Translation Port 前，SharedPdfKernel 必须先从权威 PageFacts 冻结独立的 `PageTextInventory`。它覆盖页内全部原生文字对象，并为每个对象保存稳定 identity、owner/protected 事实、源文本哈希及由配置/结构规则预先授权的唯一处置：`TRANSLATE | KEEP_SOURCE | PROTECTED | NON_TRANSLATABLE`。`KEEP_SOURCE` 必须带枚举原因和适用证据；Toolbox、Translation Provider 或 Provider 响应都无权在事后新增 KEEP_SOURCE 授权。

随后 Toolbox/Recovery 冻结 `SemanticUnitMap`，把所有 `TRANSLATE` 对象恰好映射到一个 unit，把其他对象映射到其预授权处置；它记录稳定 `unit_id/container_id`、owner、阅读顺序、源文本哈希、必保留字面量和预期处置。表格、段落、列表或复合叶可以保留各自私有分组算法，但不得新增、遗漏或吞并 PageTextInventory 对象。协调器必须在请求前完成双向覆盖核对：任一文字对象无处置、一个对象多处置、`TRANSLATE` 对象无 unit、unit 无来源对象或事后自授权 KEEP_SOURCE 都是合同失败，TranslationPort 调用数必须为 `0`；不能让某个叶用“局部请求成功”代替“整页语义单元完整”。

TranslationBundle 进入排版前必须产生 `TranslationCompletenessDecision`：

- SemanticUnitMap 中每个 unit 在响应中恰好出现一次，新增、缺失、重复或错配 ID 均失败；
- 每个 unit 只能是有效 `TRANSLATED`，或带枚举原因的显式 `KEEP_SOURCE`；空串、占位文本、异常回显、未授权原文照抄和未处理残留均失败；
- 数字、代码、专名等允许保持原文，但必须由 required-literal/KEEP_SOURCE 规则显式说明，不能用“与原文相同”逃避失败；
- 只允许对失败 unit 发起定向重译，已通过 unit 不重复请求；重试后重新对完整 SemanticUnitMap 裁决；
- 决策未 PASS 时不得进入正常布局、不得声称 `TranslationCoverage=FULL`，也不得构造伪翻译候选。

#### 9.6.2 可交付结果与翻译诊断候选双轨

“失败也必须出结果”包含两条互不冒充的 Artifact 轨道：

1. `FinalDeliveryArtifact` 负责可打开性、页数/页序、Preservation 和安全发布。它可以通过区域回退、页面透传或整本源 PDF 应急发布收敛，但其存在只证明工程闭环。
2. `TranslatedDiagnosticCandidate` 负责让布局/能力/质量错误可观察。只有 `TranslationCompletenessDecision=PASS` 后才允许生成；其中每个 SemanticUnitMap unit 必须实际物化为可提取文字，漏单元为 `0`。它可以保留溢出、碰撞或低质量等待修问题，但仍必须可打开、保持页框/旋转、遵守 owner/protected/不可修改对象和文件写入安全边界。

完整译文已形成但布局、能力或质量 Gate 失败时，必须尽力生成并标记 `TRANSLATED_DIAGNOSTIC_READY`；若完整译文存在但受字体、owner/protected、PDF 写入或完整性约束影响仍无法安全物化，必须标记 `DIAGNOSTIC_MATERIALIZATION_FAILED` 并保留实际失败证据；Provider/合同失败导致完整译文未形成时，状态必须是 `NO_TRANSLATED_CANDIDATE`。源 PDF 副本、只替换少量文字的局部样例或带占位符的 PDF 均不得登记为 TranslatedDiagnosticCandidate。诊断候选只落在 run 私有审计目录，不进入最终 Patch 回放、`target_file_path`、下载指针或产品完成产物。

页、文档和阶段报告必须分别给出三类结论：

| 结论 | 值 | 含义 |
|---|---|---|
| EngineeringClosure | `PASS` / `FAIL` | 流程是否终态、Checkpoint/Artifact/降级是否闭合 |
| ProductAcceptance | `PASS` / `FAIL` / `NOT_EVALUATED` | 是否真实完整翻译并达到已冻结质量门槛 |
| PromotionEligibility | `ELIGIBLE` / `INELIGIBLE` / `BLOCKED` | 叶、规则或版本是否具备启用/晋级条件 |

EngineeringClosure PASS 或 FinalDeliveryArtifact 存在，均不得自动推导 ProductAcceptance PASS；诊断候选存在也不得自动推导 PromotionEligibility。

#### 9.6.3 路由与能力错配

当已选 Route 的 Toolbox 无法为实际语义结构建立完整 owner/SemanticUnitMap，或真实结构证据与 Route 的能力前提冲突时，记录 `ROUTE_CAPABILITY_MISMATCH`，保存 route evidence、缺失 owner/能力和安全 fallback。Toolbox 绑定后不得在运行时擅自改 Route、调用其他叶私有工具、热改 Catalog 或自动晋级分类规则；纠正分类应进入离线分类回归和新版本发布。这样既保留当前任务的确定性降级，也把错误送回正确 owner，而不是让排版 Repair 永久重试一个分类问题。
...
| TranslatedDiagnostic | `TRANSLATED_DIAGNOSTIC_READY`、`DIAGNOSTIC_MATERIALIZATION_FAILED`、`NO_TRANSLATED_CANDIDATE`、`NOT_REQUIRED` |
...
| EngineeringClosure | `PASS`、`FAIL` |
| ProductAcceptance | `PASS`、`FAIL`、`NOT_EVALUATED` |
| PromotionEligibility | `ELIGIBLE`、`INELIGIBLE`、`BLOCKED` |

`ArtifactProduced/ArtifactIntegrity/DocumentOutcome` 描述 FinalDeliveryArtifact；`TranslatedDiagnostic` 描述独立的非发布证据。`TranslationCoverage=FULL` 只允许在 SemanticUnitMap 全覆盖、TranslationCompletenessDecision PASS 且所有 unit 已实际物化时使用。`ProductAcceptance=PASS` 还要求最终交付物自身为 FULL、Quality PASS 且未以页面/区域源文透传掩盖译文缺失；安全源 PDF 产出、局部替换或诊断候选存在都不能替代该条件。
...
- `COMPLETED` 表示每页结果和文档硬合同通过，不表示所有页面的字号、行距、段距或局部修复参数百分之百一致；结构不同页面在同一文档软基线下可以采用不同 `PageEffectiveLayout`。
- V1 不在全页终态后执行第二轮全书样式归一、重新布局或质量重判；页级 Judge 的批准结果就是布局发布输入。
- FinalDeliveryArtifact 即使是源 PDF 应急透传也必须诚实标记 TranslationCoverage 和 ProductAcceptance；源副本不得同时登记为 TranslatedDiagnosticCandidate。
- 完整有效译文已经形成但布局/能力/质量失败时，除安全 FinalDeliveryArtifact 外还应保留 TranslatedDiagnosticCandidate；实际物化仍失败时记录 `DIAGNOSTIC_MATERIALIZATION_FAILED` 和失败证据；完整译文未形成时明确记录 `NO_TRANSLATED_CANDIDATE`，不伪造可观察“翻译结果”。

`PROCESS_FAILED` 也必须保留日志、状态轨迹和已有诊断 Artifact，但不能伪称已产生可交付 PDF。

### 12.3 PDF Preservation Contract

最终 PDF 不是由页级候选 PDF 拼接出来，而是从源 PDF 副本开始，按页码串行回放已批准的 `PagePatch`。页级候选 PDF 只用于渲染、裁决和审计。Document Finalizer 不重新计算 `PageEffectiveLayout`，不调用 Toolbox/Repair/Judge 做全书样式复核；它执行的可打开性、页数/页序和 Preservation 校验属于不可取消的发布硬校验，不属于二次布局检查。
...
stateDiagram-v2
    ...
    TEMPLATE_READY --> TRANSLATION_READY: SemanticUnitMap 冻结并取得响应
    TRANSLATION_READY --> PATCH_READY: TranslationCompletenessDecision PASS
    TRANSLATION_READY --> FALLBACK_READY: 定向重译耗尽或完整性 FAIL
...
`TRANSLATION_READY` 的 Checkpoint 必须同时引用 SemanticUnitMap、TranslationBundle 和 TranslationCompletenessDecision；只有 PASS 分支能进入 PATCH_READY。`FINALIZED` 是页面可进入文档最终化的终态，具体结果由 PageOutcome 区分正常、区域降级或页面透传，TranslatedDiagnosticCandidate 作为独立审计 Artifact 不改变页面终态。

`REPAIRING` 不是一个允许丢失中间事实的黑盒状态。每次从 `REPAIRING` 返回 `CANDIDATE_READY` 前，RepairAttempt 的候选、Finding、Decision、接受/回滚结果和新版 `PageRepairMemory` 必须共同成为可恢复安全点；未提交轮次可按确定性 action key 重放，已提交轮次不得重复执行。
...
- 已冻结且指纹一致的 `DocumentLayoutMemory` 直接复用，不重新从候选页学习；
- 已提交的 `PageRepairMemory`、候选状态和 RepairAttempt 继续复用，不重复已尝试动作；
- 新 run 即使满足 §9.5.5 的完整指纹匹配，也只可通过 `PriorRepairEvidenceRef` 定位旧终态审计证据，不继承失败 action/state 排除集；只有同一 run 从已提交 Checkpoint 恢复时才继续本 run 的已尝试集合；
- 未原子提交的步骤可以重放；
- 文档最终化和最终发布必须幂等；
- 恢复前重新校验源文件哈希、配置快照、记忆 Schema/哈希、RepairAtomCatalog/叶 comparator、静态 RepairRuleRegistry、字体、Kernel/Patch interpreter 和工具箱版本。
...
sequenceDiagram
    ...
    TF2->>DB: 原子重领 PDF 任务
    TF2->>FS: 校验 source/config/toolbox/font/kernel/memory/registry hash
    TF2->>DB: 读取最后已提交 checkpoint
    TF2->>TF2: 跳过 FINALIZED 页、有效译文和已提交 RepairAttempt
    TF2->>TF2: 重放未提交原子步骤
...
| 产品修复循环 | Repair Coordinator 根据 Finding、页级历史和只读规则选择 Toolbox Repair，实际重渲染、重裁决并接受/回滚 |

禁止使用同一个通用 `retry_count` 混合三种语义。

### 15.1 产品修复循环

产品修复由 `RepairCoordinator` 统一编排，Toolbox 只提供当前叶可用的 Finding、Repair 原子和裁决。协调器必须：

1. 加载冻结的 `DocumentLayoutMemory`、当前 `PageRepairMemory`、当前叶的 `RepairAtomCatalog` 及版本化 comparator；静态 `RepairRuleRegistry` 仅核对 run 审计 hash；
2. 为初始候选和每个实际 Repair 轮次生成稳定 `state_hash/action_key`；
3. 仅排除当前 run 已提交的动作、已见候选状态和不满足 required facts 的修复原子；新 run 的 PriorRepairEvidenceRef 不影响选择；
4. 每轮只改变一个主要修复维度，从源页与同一有效 TranslationBundle 重排；成功物化候选，或以 `MATERIALIZATION_FAILED` 记录结构化失败并消耗该轮预算；
5. 用当前叶冻结的 comparator 比较前后 Finding、质量向量和硬约束，按明确的 epsilon/tie 语义接受或回滚；
6. 在选择下一动作前原子提交本轮证据和页级记忆；
7. 达到停止条件后立即进入确定性 fallback，不再把相同失败发送给相同动作。

文档级记忆提供跨页软参照，不决定某一页的修复动作；当前 run 的页级记忆负责“这一页已试过什么”；叶 RepairAtomCatalog 提供确定性动作。静态 Registry 与 PriorRepairEvidenceRef 在 V1 仅供版本绑定和审计，不得进入动作选择，职责不得互换。
...
SharedPdfKernel 与 PageToolbox/Freeform 在发送请求前先按 §9.6.1 冻结 PageTextInventory、投影 SemanticUnitMap 并完成双向覆盖核对；Translation Port 响应仍先执行 ID/Schema 校验，再执行内容完整性裁决。完整性裁决至少检查缺失/重复/新增 ID、空串/占位/异常回显、无预授权 KEEP_SOURCE 的原文照抄、必保留字面量、残留源语言和 unit/container owner。失败时只允许对失败 unit 组成新的定向 TranslationBatch；所有 unit 通过后形成内容寻址 TranslationBundle 和 TranslationCompletenessDecision，二者共同进入候选生成与 Checkpoint。

该门禁不把语言质量判断简化为“必须与原文不同”：数字、代码、缩写、专名和约定不翻译内容可以显式 KEEP_SOURCE。反之，Provider 返回 HTTP 200、JSON 可解析或 ID 齐全也不等于翻译完整。没有 PASS Decision 的响应不能进入正常排版，也不能生成 TranslatedDiagnosticCandidate。
...
- stage/state；
- attempt；
- document_memory_hash / page_memory_version / repair_registry_hash；
- repair_action_key / candidate_state_hash；
- semantic_unit_map_hash / translation_bundle_hash / completeness_decision；
- diagnostic_candidate_status；
- duration_ms；
- outcome；
- engineering_closure / product_acceptance / promotion_eligibility；
- fallback；
- error_code；
- artifact_ref。
...
- 任务配置快照；
- 分类路由和证据摘要；
- 文档级布局记忆及其事实来源、置信度、版本和哈希；
- 工具箱版本与实现指纹；
- 翻译请求/响应审计摘要；
- SemanticUnitMap、TranslationCompletenessDecision、失败 unit 定向重译和实际物化/可提取统计；
- Patch 和 Finding；
- 页级修复记忆、每轮候选、修复提议、Finding、Decision、接受/回滚和终止原因；
- RepairAtomCatalog 与 comparator 的版本/hash、选中 atom、比较结果、epsilon/tie 语义；
- RepairRuleRegistry 的静态版本、哈希和“V1 命中规则数为 0”证据；
- PriorRepairEvidenceRef 的来源 run、完整指纹匹配结果、审计 Artifact 引用和“对当前动作选择影响数为 0”证据；
- 路由能力错配、TranslatedDiagnosticCandidate 状态及与 FinalDeliveryArtifact 的隔离证明；
- 页级最终结果；
- Patch 回放清单；
- 最终质量和降级摘要。
...
V1 不强制引入 Prometheus。结构化日志和运行报告至少能聚合：队列等待时间、页耗时、租约丢失次数、进程池占用、翻译/分类超时率、每页修复轮数、重复动作拦截数、状态循环数、修复接受/回滚率、`degradation_rate`、`region_fallback_rate`、`page_passthrough_rate`、语义单元/物化翻译覆盖率、诊断候选产出率、三类结论分布和工作目录字节数。
...
│  └─ <run_id>/
│     ├─ job/
│     │  └─ document-layout-memory-<hash>.json
│     ├─ pages/<page_no>/
│     │  ├─ repair/
│     │  │  ├─ page-repair-memory-<checkpoint>-<hash>.json
│     │  │  └─ attempts/<round>/{proposal,patch,findings,decision,candidate.pdf,preview.png}
│     │  └─ diagnostic/{semantic-unit-map.json,translation-completeness.json,translated-candidate.pdf}
│     ├─ previews/page-<page_no>-<checkpoint>-<hash>.png
│     ├─ logs/
│     ├─ reports/{document-repair-summary.json,translation-completeness-summary.json}
│     ├─ diagnostic/<可选整本文档 translated-diagnostic-<hash>.pdf>
│     └─ final/
├─ _page_previews/
└─ <可选固定文件名兼容副本；非权威>
```
...
`diagnostic/` 与 `final/` 是硬隔离边界。页级 TranslatedDiagnosticCandidate 只有在完整译文实际物化、PDF 可打开且 owner/protected/page geometry 校验通过后才登记；仅当全书每页均具备完整 TranslationBundle 且物化成功时，才允许额外组装整本 translated diagnostic。任何诊断文件都不得进入最终 Patch 清单、产品预览/下载指针或 `target_file_path`。完整译文存在但物化失败时只写 `DIAGNOSTIC_MATERIALIZATION_FAILED` 证据；完整译文不存在时只写 `NO_TRANSLATED_CANDIDATE` 决策 JSON；两者都不复制源 PDF 填充该目录。
...
RepairAttempt 使用相同协议，但其 `action_key`、输出路径和终态哈希必须确定性生成。成功物化时，候选、Patch、Finding、Decision 和新版 `PageRepairMemory` 均校验并原子 rename 后，Checkpoint 才能引用该轮次；物化失败时，原子提交 `MATERIALIZATION_FAILED`、结构化异常、action/input-state 和已有安全证据，不得伪造 candidate/Patch 引用。若进程在 Artifact rename 后、Checkpoint 前崩溃，恢复器可以按 action key 和内容哈希复用孤儿证据并补交同一轮，不得把它计成新的尝试；只有 Checkpoint 成功后才视为“已尝试”。
...
| Repair 候选已写、页级记忆未提交 | 按确定性 action key/hash 复用候选并补交同一 RepairAttempt，不增加预算、不重复翻译 |
| Repair 物化失败证据已写、页级记忆未提交 | 按确定性 action key/hash 复用失败证据，补交同一 `MATERIALIZATION_FAILED` RepairAttempt，不伪造候选、不重复执行 |
| RepairAttempt 与页级记忆已提交 | 恢复时排除其 action/state，直接选择下一未尝试动作或停止 |
...
│  │  └─ repair_rule_registry.json      # V1 可为空的静态只读快照，仅绑定版本/hash
│  ├─ schemas/
│  │  ├─ model_decision.v1.schema.json
│  │  ├─ page_translation.v1.schema.json
│  │  ├─ page_text_inventory.v1.schema.json
│  │  ├─ document_layout_memory.v1.schema.json
│  │  ├─ page_repair_memory.v1.schema.json
│  │  └─ repair_atom_catalog.v1.schema.json
...
│  │  ├─ translation.py              # PageTextInventory、Unit、Batch、Bundle 合同
│  │  ├─ toolbox.py                  # Template、Patch、Finding、Decision
│  │  ├─ memory.py                   # DocumentLayoutMemory、PageRepairMemory、RepairAttempt
│  │  ├─ artifacts.py
│  │  └─ errors.py
│  ├─ application/
│  │  ├─ pdf_job_worker.py
...
│  │  ├─ page_coordinator.py
│  │  ├─ repair_coordinator.py        # 页级多轮重排、接受/回滚、停止与记忆提交
│  │  ├─ page_router.py
...
│     └─ logging.py
└─ tests/
   ├─ unit/{domain,classification,pdf_kernel,toolboxes,freeform,memory}/
...
| `RepairCoordinator`（`application`） | 页级 Repair 选择、预算、候选比较、记忆提交和 fallback | 发明分类私有布局规则、修改文档级事实、自动晋级跨文档规则 |
...
已完成迁移阶段后来暴露横切合同遗漏时，不原地改写旧报告、旧 Gate 或旧产物。应新增有明确输入、前向合同、真实回归和独立 Gate 的纠偏阶段；它可以复用旧证据定位问题，但必须说明旧结论适用的历史基线，以及新合同从哪个下游阶段开始生效。
...
4. 第二批叶完成后先建立迁移纠偏基线，补齐 SemanticUnitMap、翻译完整性、诊断候选、路由错配和三类结论合同，不回写此前 Gate；
5. 冻结窄化的 `DocumentLayoutMemory`、当前 run 的 `PageRepairMemory`、叶 `RepairAtomCatalog`、版本化 comparator、静态 Registry 边界和多轮重排协调器；
6. `visual_anchored` 和各 composite 单独迁移，已知失败叶使用已冻结 Repair Memory 闭环最后处理；
7. 全叶 Catalog、Repair Memory、翻译完整性、诊断候选与 fallback 回归。
...
- 当前已知失败不被伪装成 PASS；
- 工具箱可以在文档协调器中稳定运行；
- 文档级布局事实只构建一次且页面间无可变记忆串扰；
- 每个已执行 Repair 轮次都有唯一终态：物化成功时有真实候选，物化失败时有 `MATERIALIZATION_FAILED` 与可恢复证据；重复动作/状态循环被确定性拦截；
- V1 Repair 模型调用数和 Registry 运行时命中/写入数均为 `0`；跨文档规则学习留待新设计和新 baseline；
- 只有 Gate 通过的叶在 Catalog 中 `enabled=true`，其余叶明确 disabled 并走确定性 fallback。
- 源 PDF 透传、TranslatedDiagnosticCandidate 和 ProductAcceptance 三者严格区分，旧阶段问题由前向纠偏 Gate 承接而非改写历史。
...
- `PageTextInventory/SemanticUnitMap/TranslationCompletenessDecision` 的独立分母、双向覆盖、逐文字对象预授权处置、逐 unit 唯一处置、占位/事后 KEEP_SOURCE/残留拒绝、定向重译和内容寻址；
...
- 类型化语义锚点的叶级选择与共享执行门禁：正文/标签、数值、左/右/居中标题及 owner-relative 标签分别保持已声明的左边缘、右边缘、中心轴或 owner 关系；以最终 glyph bbox 检查锚点漂移，并包含 `TITLE` 角色不自动居中的正反例；
- `DocumentLayoutMemory` 的 Schema、内容哈希、`source_layout_baseline/target_layout_policy`、事实 provenance、全页屏障、构建确定性和冻结后只读；
- 首张或部分页面事实就绪时 Builder 必须返回 `NOT_READY`，完整事实/路由就绪后才构建一次；冻结前 TranslationPort 调用数必须为 `0`；
- 同一文档记忆下不同页面可解析不同 `PageEffectiveLayout`，页级调整不得回写文档快照；
- `PageRepairMemory/RepairAttempt` 的 identity、追加顺序、action/state 去重、接受/回滚和终止原因；
- `PriorRepairEvidenceRef` 仅记录来源 run、完整指纹和审计引用；无论指纹是否一致，对新 run 动作选择影响数均为 `0`；
- `RepairAtomCatalog` 的稳定 atom/action identity、required facts、有界参数和叶 comparator 的指标方向、硬拒绝、精度、epsilon/tie 与版本/hash；Repair 模型调用数为 `0`；
- `RepairRuleRegistry` 固定为空或静态只读快照、运行时命中/写入数为 `0`，不得通过 dormant 字段启用 Rule IR、模型裁决或晋级流程；
- FinalDeliveryArtifact 与 TranslatedDiagnosticCandidate 的类型、目录、发布指针和状态隔离；可物化、物化硬失败、无完整译文三种状态必须可区分，源 PDF 副本登记为诊断候选的数量必须为 `0`；
- `ROUTE_CAPABILITY_MISMATCH` 的证据、owner 和离线反馈合同，运行时热改 Route/Catalog 或跨叶偷用工具数为 `0`；
- PageOutcome；
...
- 某页能力缺失；
- 某页质量失败；
- 同一完整 PDF 的权威 PageFacts 引用、重复边缘事实及正文/标题/表格文字/图表标签等跨页角色分布能以同一 hash 被跨页读取，且候选页不能污染源布局记忆；表/cell/owner/anchor 等页内明细只从 PageFacts/叶合同读取，文档记忆不得复制第二份；文档画像是软先验，不要求不同结构页面解析出完全相同的页级参数；
- 至少两个结构不同页面在同一文档记忆下形成各自 `PageEffectiveLayout` 并通过页级 Gate，Document Finalizer 的全书样式归一/二次 Judge 调用数为 `0`；
- 一页连续多轮失败后，每个已执行动作都有唯一终态；物化成功轮次有真实候选与失败记录，物化失败轮次有 `MATERIALIZATION_FAILED` 证据，下一轮不重复当前 run 已提交动作，最终接受安全候选或明确 fallback；
- 对正文、表格、图表/图标标签、旋转文字和复合页的真实样本冻结 PageTextInventory 与 SemanticUnitMap，逐文字对象处置覆盖率、完整 Bundle 的 unit 物化/可提取率均为 `100%`、漏单元为 `0`；布局失败仍产出隔离的 TranslatedDiagnosticCandidate，Provider/完整性失败则明确 `NO_TRANSLATED_CANDIDATE`；
- 对 CJK 标点、拉丁词断裂、旋转字形、CID/Wingdings/受控字体回退和图标/图例/数值/单位绑定执行实际字体/字形和数据关系探针，不用仅检查配置名称代替真实 PDF 结果；
- 最后一页失败；
- 全页透传；
...
- Worker 进程中断；
- 文档记忆写入后、run Checkpoint 引用前中断；
- Repair 候选写入后、页级记忆 Checkpoint 前中断，以及页级记忆提交后重启；
- 陈旧/损坏的文档记忆、页级记忆或 Registry hash 被恢复器拒绝；
- `BrokenProcessPool` 和子进程异常退出；
- 线程池和进程池并发读取同一文档记忆 hash，验证各 Worker 只持有本地不可变视图、构建调用数为 `1`、跨页写入为 `0`；
- 租约过期和重领；
...
- 文本替换但结构不变；
- 页面顺序变化；
- 文档级画像必须随当前源 PDF 的事实重建，不得命中历史文件名、公司名、页码或绝对坐标规则；
- V1 Registry 命中/写入和 Repair 模型调用数均为 `0`；任何跨文档规则或模型 Repair 必须以新设计、新 baseline 和独立反过拟合 Gate 启用；
- 同 source 新 run 即使完整指纹一致也不导入失败 action/state 排除集；PriorRepairEvidenceRef 对当前选择影响数恒为 `0`，同一 run Checkpoint 恢复则继续本 run 已提交集合；
...
- 两层运行时 Repair Memory、静态扩展边界、确定性 RepairAtomCatalog/叶 comparator、候选或物化失败终态、多轮停止、Checkpoint 和新 run 历史审计隔离 Gate 全部通过。
- 至少一份未经拆页的完整真实 PDF 达到 SemanticUnitMap/实际物化覆盖率 `100%`、漏单元 `0`、Quality PASS 和 ProductAcceptance PASS；源 PDF 透传或诊断候选不能代替该结果。
- 所有验收页/文档同时报告 EngineeringClosure、ProductAcceptance 和 PromotionEligibility，工程闭环不得冒充产品验收。
...
- 页面或 Toolbox 不能修改 `DocumentLayoutMemory`；跨页可变 Repair 状态和运行时 Registry 写入命中均为 `0`。
- `DocumentFinalizer` 调用全书样式归一、二次布局规划或二次质量 Judge 的次数为 `0`；Preservation 和 ArtifactIntegrity 硬校验必须完整执行。
- 历史 PDF 原文、候选图、sample_id、文件名、固定页码或绝对坐标不得进入跨文档 Registry 或当前模型 Prompt。
- TranslatedDiagnosticCandidate 不得进入最终 Patch 回放、产品预览/下载指针或 `target_file_path`；源 PDF 副本、局部样例和占位 PDF 不得冒充诊断候选。
...
| 全页事实/路由屏障增加首张译后页面的启动延迟 | PageFacts、边缘分析和分类可在预算内并发并流式聚合；屏障前不放行排版页，以稳定单一文档记忆版本，耗时纳入 P13 容量证据 |
...
| Toolbox/Provider 自行缩小分母，或只校验响应成功/ID 对齐而遗漏占位、原文回填、未翻译单元或 PDF 中未实际物化的译文 | Kernel PageTextInventory 先冻结全部原生文字及预授权处置；SemanticUnitMap 双向核对，TranslationCompletenessDecision 与 PDF 物化/可提取探针继续验收 |
| 用源 PDF 副本或局部替换样例冒充“失败时的翻译候选” | FinalDeliveryArtifact 与 TranslatedDiagnosticCandidate 类型/目录/指针隔离；完整 Bundle 不存在时只记录 `NO_TRANSLATED_CANDIDATE` |
| 工程闭环、产品质量和叶子晋级混成一个 PASS | 分别记录 EngineeringClosure、ProductAcceptance、PromotionEligibility；G14 要求真实完整 PDF 产品验收证据 |
...
| 文档级“记忆”被页面修复原地改写，造成并发串扰和后页漂移 | DocumentLayoutMemory 内容寻址并冻结；页面只读同一 hash，修改尝试直接拒绝 |
| 从首张或首类页面提前冻结文档记忆，导致封面/目录/正文样式偏置 | 等待全页 PageFacts、完整 Route 和公共边缘索引屏障；部分输入只返回 `NOT_READY` |
| 并发过程中热更新文档记忆，导致不同页面消费不同版本 | 页面放行前 single-flight 发布；放行后固定同一 hash，硬错误新开 run，软差异只写页级记忆 |
| 把文档画像当成精确模板并做全书二次归一，破坏特殊页面 | 文档画像保存分布/区间并作为软先验；页级有效布局允许不同，FINALIZED 后不做二次全局样式重排 |
| 页级修复不保存失败历史，重启或下一轮重复同一动作 | PageRepairMemory 随每轮候选原子提交；action/state 去重、无改善和循环停止 |
| 同一 source 新 run 可能重复历史 action/state | V1 接受该有限成本以保证新 run 独立确定性；PriorRepairEvidenceRef 只供审计，同一 run 的 Checkpoint 恢复仍严格排除已提交动作 |
| 分类错路由被当成布局问题反复修，或 Toolbox 运行时偷用其他叶工具 | 记录 ROUTE_CAPABILITY_MISMATCH 并安全降级；分类纠正只经离线回归和版本发布，运行时不热改 Route/Catalog |
| 在失败候选上累积修改导致版式漂移 | 每轮从不可变源页和同一有效 TranslationBundle 重新生成完整候选 |
| 单一样本经验自动传播到其他 PDF，形成隐蔽过拟合 | V1 Registry 固定为空或静态快照且不参与动作选择；跨文档学习必须另立设计、baseline 和反过拟合 Gate |
| Repair Memory 保存无界原文、候选或 Provider 响应导致泄密/磁盘失控 | 只保存稳定 ID、哈希、结构化事实和受控 Artifact 引用，沿用 manifest 保留策略 |
| 降级只写日志、用户误以为完整成功 | 产品状态映射、降级报告、警告和聚合指标共同展示 |
| 低对比度源样式被实现者静默“增强”导致品牌/版式漂移 | V1 默认保持源视觉属性并标记 Quality/降级；未经独立政策和 Gate 不自动改色或增强对比度 |
...
35. V1 Repair Memory 由文档级跨页软基线和当前 run 页级修复账本两层运行时记忆组成；跨文档 Registry 仅为 dormant 静态扩展面，它们都不等同于语义 Translation Memory。
36. `DocumentLayoutMemory` 的源基线只从完整不可变源 PDF 事实构建，目标策略只从源基线、语言对、受控字体和配置派生；整体冻结后所有页面只读，候选页和修复结果不得回写。
37. `PageRepairMemory` 是单页、单 run、可 Checkpoint 恢复的追加账本；每轮记录真实候选、失败、动作、接受/回滚和停止原因。
38. `RepairRuleRegistry` 在 V1 固定为空或静态只读快照，仅绑定版本/hash，运行时命中、写入和晋级数均为 `0`；跨文档规则须另立设计和 baseline。
39. 版式 Repair 复用同一有效 TranslationBundle，并从不可变源页重新生成候选；只有翻译合同/语义失败才允许定向重译。
40. 每个已选择并执行的 Repair 动作都必须形成唯一终态和证据；成功物化才要求可裁决候选，物化失败记录 `MATERIALIZATION_FAILED` 并消耗预算，预算耗尽后仍通过区域/页面/文档 fallback 产出完整 PDF。
41. V1 不引入向量数据库、长期聊天记忆或历史 PDF 页面 Prompt 注入实现 Repair Memory。
42. `DocumentLayoutMemory` 必须等待全页 `PageFacts`、完整分类路由和公共边缘索引屏障后构建；禁止由第一张或首批页面提前冻结。
43. 文档记忆由 `source_layout_baseline` 和 `target_layout_policy` 组成；后者在源基线之上由语言对、受控字体和配置于翻译前派生，不要求先有译后文档。
44. 同一 run 的线程/进程并发页固定读取同一不可变 memory hash；文档画像是软先验，不同页面可以形成不同 `PageEffectiveLayout`，页级优化不得回写全局记忆。
45. V1 不在页面 FINALIZED 后执行全书样式二次归一、重新布局或二次 Judge；Document Finalizer 的 Preservation、可打开性、页数和页序硬校验仍必须执行。
46. 已完成阶段的报告和 Gate 不因后续发现横切遗漏而回写；用独立纠偏阶段冻结前向合同和受影响回归，历史结论只对其原基线有效。
47. 每页翻译前由 SharedPdfKernel 冻结独立 PageTextInventory，再由 Toolbox 投影并双向核对 SemanticUnitMap；TranslationCompletenessDecision PASS 是进入正常排版和生成翻译诊断候选的前置条件，Provider 无权事后授权 KEEP_SOURCE，ID 齐全或 HTTP 成功本身不代表翻译完整。
48. FinalDeliveryArtifact 与 TranslatedDiagnosticCandidate 是两条隔离轨道：前者保证安全交付，后者暴露完整译文下的布局/能力/质量错误；源 PDF 副本不得冒充后者，后者不得发布为最终产物。
49. 页、文档和阶段结论分别记录 EngineeringClosure、ProductAcceptance、PromotionEligibility；任何一种不得自动推导另外两种。
50. `PriorRepairEvidenceRef` 在 V1 只定位同源同页历史终态审计证据，不影响新 run 的动作选择；同一 run 从 Checkpoint 恢复时才继续已提交动作/状态集合。
51. V1 RepairCoordinator 只消费叶 `RepairAtomCatalog` 和版本化 comparator，Repair 模型调用数为 `0`；不得通过静态 Registry 字段静默启用 Rule IR、坐标/参数生成或模型工具选择。
52. Route 与 Toolbox 能力错配记录为 ROUTE_CAPABILITY_MISMATCH 并安全降级；运行时不自动改路由、不热改 Catalog、不跨叶偷用私有工具。
53. 当前页内部段落/组/FlowBand 的最终回流可以作为 RepairAttempt 的候选生成步骤，但必须在本页 Judge 和 FINALIZED 前完成；禁止的仍是全页终态后的全书二次重排。
54. V1 对低对比度内容默认保持源视觉设计并诚实标记质量/降级；未经独立批准的视觉政策和回归 Gate，不静默调整颜色或对比度。
...
- 分类、翻译和 PDF 进程池默认并发数；
- P9B 实测后对默认 `max_repair_rounds=3`、`max_no_improvement=2` 的调整结论，以及网络、原子任务、Provider 快速降级和文档 run/attempt 的具体预算；
- 若未来启用跨文档 RepairRuleRegistry：另立设计后再确定维护人、规则废弃窗口、独立语料与 holdout 阈值；V1 不预先实现晋级流程；
- ProductAcceptance 除“语义单元/实际物化 `100%`、漏单元 `0`、Quality PASS、无源文透传掩盖”硬条件外的分层人工质量阈值，由 P14 基于纠偏/全叶真实证据冻结；冻结前只能记 `NOT_EVALUATED`，不能记 PASS；
- PriorRepairEvidenceRef 的审计保留期限、同源多 run 索引和失效清理策略；若未来要让其影响新 run 动作选择，必须另立安全性与反过拟合 Gate；
...
- `spikes/page_classification_engine_puncture_v1/reports/deep_audits/current_classification_20260711/深度分类审计报告.md`
