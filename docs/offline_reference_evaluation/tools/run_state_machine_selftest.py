...
def find_process_doc() -> Path | None:
    docs_root = WORKSPACE_ROOT / "docs"
    if not docs_root.exists():
        return None
    candidates = sorted(path for path in docs_root.glob("*/*.md") if path.is_file())
...
def run_one(
    ...
    decisions.append(
        decision(
            ...
            "S9_VerifyProcessContract",
            "Adjudicate process-document feedback and previously recorded failure modes.",
            [{"path": rel(process_doc) if process_doc else "process_doc_not_supplied", "kind": "process_doc"}],
...
def main() -> int:
    ...
    parser.add_argument("--process-doc", default=None)
    ...
    process_doc = resolve_workspace_path(args.process_doc) if args.process_doc else find_process_doc()
    out_base = (
        Path(args.out_dir)
        if args.out_dir
        else WORKSPACE_ROOT
    ...
    for mode in args.modes:
        ...
        for item in manifest["inputs"]:
            results.append(run_one(item, mode, out_base, args.generator, semantic_translations_dir, process_doc))
