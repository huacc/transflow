...
3. 若执行了变更，必须回到 `S5`、`S6` 或 `S7` 重新物化译文、布局或候选 PDF。
4. 必须重新进入 `S8_VerifyProductQuality`，用变更后的候选和质量证据验证目标 gate。

如果只生成 `visual_repair_plan.json` 或只重新评估原候选，不能把本轮记为“修复循环已执行”；最终报告必须写 `repair_loop_execution=NOT_RUN` 或 `PARTIAL_NOT_FULL_LOOP`。`validate_process_artifacts.py` 必须把“进入 `Lx_RepairLoop` 但缺少 `repair_loop_<n>.json`”判为 `S_FAIL_PROCESS_CONTRACT`。

`run_semantic_product_quality_round.py` 是当前通用执行引擎。它负责把 S3-S9 原子工具串联起来，并在 `Lx_RepairLoop` 被进入时强制写 `repair_loop_<n>.json`。如果当前失败类没有已实现的通用 repair atom 执行器，执行引擎必须：
